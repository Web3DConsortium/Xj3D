    ...
    implements WindowListener
{
    ...
    private void setupSceneGraph(int nSpheres)
    {
        ...
        Vector3f trans = new Vector3f(0, 100, 600);
        ...
        for (File files_in_dir1 : files_in_dir) {
            if (files_in_dir1.isDirectory()) {
                continue;
            }
            System.out.println("loading " + files_in_dir1);
            TextureComponent2D img_comp = loadImage(files_in_dir1);
            if (img_comp != null)
            {
     ...
     * @param
     */
    private TransformGroup createPole(Vector3f viewPos,
                                      float[] colour)
    {
     ...
     * @param
     */
    private TransformGroup createArrow(Vector3f viewPos,
                                       float[] colour)
    {
        float[] normals =
        {
    ...
    private String[] loadShaderSource(String file)
    {
        ...
        if(!f.exists())
        {
            System.err.println("Can't find shader " + file);
            return null;
        }
