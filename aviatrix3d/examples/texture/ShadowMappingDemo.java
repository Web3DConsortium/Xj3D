...
// None
/**
 * Main class that creates a hard shadowing scene using shadow mapping
 * technique.
 * <p>
 *
 * Shadow mapping technique is an image space based algorithm, which means
 * unlike stencil shadow volume technique, there is no need for the knowledge of
 * scene's geometry to carry out necessary computation.
 *
 * To create a shadow in the scene using the shadow mapping technique, first
 * step is to render the whole scene from the light's point-of-view. From this
 * rendering, depth value is saved into a texture. Second, and final step is to
 * render the scene from camera's viewpoint. By using a depth comparison between
 * the depth texture and the rendered scene, object is either drawn in shadow or
 * in light.
 * <p>
 *
 * This example code demonstrates how the shadow mapping effects can be
 * implemented using the aviatrix3d scene graph.
 *
 * Since every geometries needs to be represented in scene graph hierarchy in
 * aviatrix3d two scene graph scenes are created. As described above about the
 * implementation details of shadow mapping, first, scene from the light's
 * point-of-view is created. After the creation of the scene, that scene is then
 * copied onto a OffscreenTexture2D which copies an rendered image into a
 * texture buffer.
 *
 * The next step is then to create a scene from camera's viewpoint. Each
 * geometries in the scene will use the depth values which is stored inside of
 * the OffscreenTexture2D to compare their depths with the depths stored in the
 * depth texture, thus determining whether they are in shadow or outside of
 * shadow.
 * <p>
 *
 ...
 *
 * LPOV __________ / \ / \ <- View frustum / \ / \ / O \ / O <- Objects \ / \ /
 * _ _ <- Shadows \ / \ ----------------------------
 *
 * @author Sang Park
...
public class ShadowMappingDemo extends JFrame
    ...
    private static final String APP_NAME = "Shadow Mapping Demo";

    /**
     * Panel that consists of aviatrix3d scene on the left side and java panel
     * on the right side
     */
    ...
    // screen first.
    /**
     * Surface containing aviatrix3d scene
     */
    ...
    /**
     * Texture coordinate generator that generates eye linear texture
     * coordinates from the light's point of view
     */
    ...
    /**
     * Setup view settings from the light's point of view and also camera's
     * view.
     */
     ...
     * <p>
     *
     * In this render pass, model view matrix and the projection matrix of this
     * render pass is set relative to the light's position and direction. Before
     * rendering the scene from the light's perspective, front face culling is
     * enabled to draw only the back facing polygons.
     *
     * @return SimpleScene containing a scene that is rendered from the light's
     * point of view
     */
    ...
    TextureUnit createShadowTexture() {
        ...
        GLCapabilities cap = new GLCapabilities(getDefault());
