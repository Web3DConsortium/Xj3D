...
public abstract class BaseStateSortStage extends BaseSortStage
{
    ...
    protected Map<VisualDetails, Integer> lightIdMap;
    ...
    protected HashSet<VisualDetails> currentClipPlanes;

    /** Set for holding the difference between the current and new sets */
    protected HashSet<VisualDetails> oldClipPlanes;

    /** Set to hold onto clip planes that are common between last node and this */
    protected HashSet<VisualDetails> keepClipPlanes;

    /** Mapping of the VisualDetails instance to the global ID */
    private Map<VisualDetails, Integer> clipIdMap;
    ...
    static
    {
        ...
        for(int i = 0; i < 32; i++)
            TEX_IDS[i] = GL.GL_TEXTURE0 + i;
    }

    /**
     * Create an empty sorting stage that initialises the internal structures
     * to assume that there is a minimum number of surfaces, both on and
     * offscreen.
     *
    ...
    protected int appendLights(GraphicsCullOutputDetails node,
                               ...
                               int offset)
    {
        if(node.numLights == 0 && currentLights.isEmpty())
            return offset;
        ...
        if(new_count != 0)
        {
            for(int i = 0; i < new_count; i++)
            {
                ...
                lightIdMap.put(ld, lastGlobalId);
    ...
    /**
     * Append the clip planes onto the existing list. Checks versus existing
     * state and starts and stops as required.
     *
     * @param node The node instance to look at appending
     * @param offset The distance into instr array of values to start
    ...
    protected int appendClipPlanes(GraphicsCullOutputDetails node,
                                   ...
                                   int offset)
    {
        if(node.numClipPlanes == 0 && currentClipPlanes.isEmpty())
            return offset;
        ...
        if(num_old != 0)
        {
            ...
            oldClipPlanes.toArray((VisualDetails[]) clipTmp);
            oldClipPlanes.clear();
                               ...
                               int offset)
    {
        ...
        int idx = offset;
        ShaderComponentRenderable comp;
    ...
    protected int cleanupObjects(GraphicsInstructions instr, int offset)
    {
        ...
        if(num_lights != 0)
        {
            ...
            {
                ld = currentLights.pop();
                l_id = lightIdMap.remove(ld);
        ...
        if(num_clips != 0)
        {
            ...
            currentClipPlanes.toArray((VisualDetails[]) clipTmp);
