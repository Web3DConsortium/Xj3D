...
public abstract class BaseCullStage implements GraphicsCullStage
{
    ...
    protected HashSet<OffscreenCullable> processedPBufferTextures;

    /** Collection of textures we've found for the current parent texture */
    private HashSet<OffscreenCullable> visitedNodes;
    ...
    protected List<Cullable> currentViewpointPath;
    ...
    /**
     * Register a receiver for the output of the sorter. If the value is null,
    ...
    {
        for (ViewportCollection[] workLayer : workLayers) {
            for (ViewportCollection wLayer : workLayer) {
                if (wLayer == null) {
                    break;
                }
                for (int k = 0; k < wLayer.numViewports; k++) {
                    ViewportLayerCollection c = wLayer.viewports[k];
