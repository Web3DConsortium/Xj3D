...
/**
 * Raster is an abstract class that specifies 2D pixel data required by a
 * Pixmap node.
 * <p>
 *
 * Raster may take one of two forms: Single byte bitmaps and multibyte
 * images. When doing line intersections, rasters will only return true
 * if the start point is within the bounds of the raster and the direction is
 * along the -Z axis.
 * <p>
 *
 * <p><b>Coordinate System</b></p>
 *
 * Rasters use OpenGL's coordinate system that defines the position as the
 * lower left corner of the object. Width extends to the right, height is
 * upwards. Rasters are always rendered in screen space coordinates - 2D
 * integer pixel locations. Fractional values don't exist.
 * <p>
 *
 * Note that OpenGL treats raster objects differently to polygonal data. If the
 * origin is outside of the current clip bounds, the entire object is not shown
 * on screen, even if part of its bounds lie inside the current viewport.
 * <p>
 *
 ...
 *     in something that is not part of the transform hierarchy.</li>
 * </ul>
 *
 * @author Justin Couch
 * @version $Revision: 2.6 $
 */
public abstract class Raster extends NodeComponent
    implements PickableObject, LeafPickTarget, GeometryRenderable
{
    ...
    private static final String PICK_TIMING_PROP =
        "org.j3d.aviatrix3d.Raster.pickTimingMsg";
    ...
    /** Listing of all the parents of this node in lieu of the super's parentList */
        ...
        implements InternalNodeUpdateListener
    {
        ...
        /**
         * Notify this node to update it's bounds now and propagate those new
     ...
     * bounds. Used to propagate bounds changes from the leaves of the tree
