    ...
    implements AppearanceAttributeRenderable, DeletableRenderable
{
    ...
    private Map<GL, Integer> setDisplayListMap;

    /** Map of display contexts to maps for the restore */
    private Map<GL, Integer> clearDisplayListMap;
    ...
    public PolygonAttributes()
    {
        ...
        setDisplayListMap = new HashMap<>(1);
        clearDisplayListMap = new HashMap<>(1);
    }
    ...
    public void render(GL2 gl)
    {
        ...
        if(stateChanged)
        {
            synchronized (setDisplayListMap) {
                if (!setDisplayListMap.isEmpty()) {
                    Integer listName = setDisplayListMap.remove(gl);

                    if (listName != null) {
                        gl.glDeleteLists(listName, 1);
                    }
                }
                setDisplayListMap.clear();
                if (!clearDisplayListMap.isEmpty()) {
        ...
        Integer listName = setDisplayListMap.get(gl);
     ...
     * @return the draw mode for the specified face
     */
     ...
     *
     * @param factor The offset factor multiplier
