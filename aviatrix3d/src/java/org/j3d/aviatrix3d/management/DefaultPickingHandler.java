 ...
 * <ul>
 * <li>unknownPickTypeMsg: Error message when an unknown pick type is
 * requested</li>
 * <li>unknownSortTypeMsg: Error message when an unknown sort type is
 * requested</li>
 * <li>unknownProxyTypeMsg: Error message for an unknown proxy geometry
 * type</li>
 * </ul>
 *
 * @author Justin Couch
...
class DefaultPickingHandler
        ...
        switch (target_node.getPickTargetType()) {
            ...
            case PickTarget.CUSTOM_PICK_TYPE:
                ...
                if (pickCustom((CustomPickTarget) target_node, req)) {
                    ...
                    PickTarget[] kids
                            = pickInstructions.children.clone();
    ...
    /**
     * Recurse a single-child node tree looking for intersections with a single
     * point. Shared nodes are different to normal because we don't bother doing
     * bounds tests as the bounds will be identical to their child. Skip the
     * test and just head straight to the child node to test against.
     *
    ...
    private void pickAllPoint(PickTarget root, PickRequest req) {
        int found = 0;
        List<SceneGraphPath> output_list;

        if (req.foundPaths instanceof ArrayList) {
            output_list = (List<SceneGraphPath>) req.foundPaths;
            output_list.clear();
        } else {
     ...
     * @param pickType flags to compare against for picking
     */
    private int pickAllPoint(PickTarget root,
            PickRequest req,
            float[] loc,
            List<SceneGraphPath> paths,
            int currentPath,
            boolean needTransform) {
        if (!root.checkPickMask(req.pickType)) {
            return 0;
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllPoint((PickTarget) target_node,
                        req,
                        loc,
                        paths,
                        currentPath,
        ...
        int found = 0;
        int num_kids = ((GroupPickTarget) root).numPickableChildren();
        ...
        if (num_kids != 0) {
            ...
            pickPath[lastPathIndex] = root;
            lastPathIndex++;
            PickTarget[] kids = ((GroupPickTarget) root).getPickableChildren();
    ...
    /**
     * Recurse a single-child shared node tree looking for intersections with a
     * single point. Shared nodes are different to normal because we don't
     * bother doing bounds tests as the bounds will be identical to their child.
     * Skip the test and just head straight to the child node to test against.
     *
     * @param root The shared node to test against
     * @param pickType flags to compare against for picking
     * @param loc The location of the point to pick with in local coords
     ...
     * @param useGeom true if this should check the results based on geometry
     * rather than just bounds
     * @param needClosest true if every triangle should be searched to find the
     * real one that is closest. Only used if useGeom is true
     */
    ...
    private void pickAllLineSegment(PickTarget root, PickRequest req) {
        ...
        switch (target_node.getPickTargetType()) {
            case PickTarget.GROUP_PICK_TYPE:
                ...
                if (num_kids != 0) {
                    ...
                    for (int i = 0; i < num_kids; i++) {
                        ...
                        switch (kids[i].getPickTargetType()) {
                            case PickTarget.GROUP_PICK_TYPE:
                                found += pickAllLineSegment(kids[i],
                                        req,
                                        start,
                                        end,
                                        output_list,
            ...
            case PickTarget.LEAF_PICK_TYPE:
                ...
                found += pickAllLineSegment(target_node,
                        req,
                        start,
                        end,
                        output_list,
     ...
     * rather than just bounds
     */
    private int pickAllLineSegment(PickTarget root,
            PickRequest req,
            float[] p1,
            float[] p2,
            List<SceneGraphPath> paths,
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllLineSegment((PickTarget) target_node,
                        req,
                        p1,
                        p2,
                        paths,
    ...
    private int pickAllLineSegment(SinglePickTarget root,
        ...
        switch (child.getPickTargetType()) {
            case PickTarget.GROUP_PICK_TYPE:
                found = pickAllLineSegment(child,
                        req,
                        p1,
                        p2,
                        paths,
     ...
     * calculating
     */
    private int pickAllLineSegment(PickTarget geom,
            PickRequest req,
            float[] p1,
            float[] p2,
            List<SceneGraphPath> paths,
        ...
        if (bounds.checkIntersectionSegment(p1, p2)) {
            if (useGeom) {
                if (((LeafPickTarget) geom).pickLineSegment(p1, p2, needClosest, vertexPickData, 0)) {
                    resizePath();
                    pickPath[lastPathIndex] = geom;
                    validTransform[lastPathIndex] = false;
                    lastPathIndex++;
     ...
     * rather than just bounds
     */
    private int pickAllRay(PickTarget root,
            PickRequest req,
            float[] p1,
            float[] p2,
            List<SceneGraphPath> paths,
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllRay((PickTarget) target_node,
                        req,
                        p1,
                        p2,
                        paths,
     ...
     * @param needTransform true if we should calc vworld information
     */
    private int pickAllCylinder(PickTarget root,
            PickRequest req,
            float[] center,
            float[] axis,
            float radius,
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllCylinder((PickTarget) target_node,
                        req,
                        center,
                        axis,
                        radius,
     ...
     * @param needTransform true if we should calc vworld information
     */
    private int pickAllCone(PickTarget root,
            PickRequest req,
            float[] vertex,
            float[] axis,
            float angle,
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllCone((PickTarget) target_node,
                        req,
                        vertex,
                        axis,
                        angle,
     ...
     * @param needTransform true if we should calc vworld information
     */
    private int pickAllBox(PickTarget root,
            PickRequest req,
            float[] min,
            float[] max,
            List<SceneGraphPath> paths,
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllBox((PickTarget) target_node,
                        req,
                        min,
                        max,
                        paths,
    ...
    /**
     * Recurse a single-child shared node tree looking for intersections with a
     * frustum. Shared nodes are different to normal because we don't bother
     * doing bounds tests as the bounds will be identical to their child. Skip
     * the test and just head straight to the child node to test against.
     *
     * @param root The shared node to test against
     * @param pickType flags to compare against for picking
     * @param path The place to put the results in
     ...
     * @param needTransform true if we should calc vworld information
     */
    private int pickAllFrustum(PickTarget root,
            PickRequest req,
            List<SceneGraphPath> paths,
            int currentPath,
            boolean needTransform) {
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllFrustum((PickTarget) target_node,
                        req,
                        paths,
                        currentPath,
                        needTransform);
    ...
    private void pickAllSphere(PickTarget root, PickRequest req) {
        ...
        switch (target_node.getPickTargetType()) {
            ...
            case PickTarget.SINGLE_PICK_TYPE:
                ...
                found += pickAllSphere(target_node,
                        req,
                        start,
                        req.additionalData,
                        output_list,
     ...
     * @param needTransform true if we should calc vworld information
     */
    private int pickAllSphere(PickTarget root,
            PickRequest req,
            float[] min,
            float radius,
            List<SceneGraphPath> paths,
        ...
        if (bounds instanceof BoundingGeometry) {
            ...
            if (target_node instanceof GroupPickTarget) {
                return pickAllSphere((PickTarget) target_node,
                        req,
                        min,
                        radius,
                        paths,
    ...
    /**
     * Process a custom pick. All this does is call the appropriate custom
     * picking method and then return. The values are left in the class variable
    ...
    /**
     * Walk the transform stack and compute the total transform to get from the
     * world root to here. The forward matrix is placed in vworldMatrix and the
     * inverse is placed in invertedMatrix. Both are class-scope variables.
     */
    ...
    /**
     * Ensure that bounding box extents are properly ordered after they have
     * been transformed
     *
