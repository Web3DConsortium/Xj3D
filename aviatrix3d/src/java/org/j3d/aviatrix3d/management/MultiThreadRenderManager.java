...
/**
 * Implementation of the {@link RenderManager} that uses separate threads for
 * each of the pipelines that it is managing, allowing it to run multiple output
 * surfaces simultaneously (eg Powerwall, CAVE etc).
 * <p>
 *
 ...
 * <li>boundsSetTimingMsg: Error message when attempting to call boundsChanged()
 * during bounds callback</li>
...
public class MultiThreadRenderManager
    ...
    /**
     * Message when the data change listener callback attempts to add another
     * data listener during processing.
     */
    private static final String DATA_TIMING_PROP
            = "org.j3d.aviatrix3d.management.MultiThreadRenderManager.dataSetTimingMsg";

    /**
     * Message when the bounds change listener callback attempts to add another
     * data listener during processing.
     */
    ...
    /**
     * Message when there was an exception generated in the internal code that
     * implements the updateBoundsAndNotify() callback.
     */
    ...
    /**
     * Message when there was an exception generated in the internal code that
     * implements the disableActiveAudioState() callback.
     */
     ...
     */
    private HashSet<Object> dataChangeListenerSet;

    /**
     * HashSet to determine duplicates in the change List
     */
    private HashSet<Object> dataChangeSrcSet;

    /**
     * HashSet to determine duplicates in the change List
     */
    private HashSet<Object> boundsChangeListenerSet;

    /**
     * HashSet to determine duplicates in the change List
     */
    private HashSet<Object> boundsChangeSrcSet;
     ...
     */
    private HashSet<Object> shaderInitSet;

    /**
     * Set of shaders that require log fetch processing
     */
    private HashSet<Object> shaderLogSet;

    /**
     * The collection of display collections to manage
     */
    private List<DisplayCollection> displays;
     ...
     */
    private Map<DisplayCollection, DisplayCollectionThread> displayThreadMap;
    ...
    /**
     * Lock object to prevent frame callbacks from finishing before we've called
     * all the other threads this frame.
     */
    private final Object frameFinishLock;
     ...
     */
    private final Object renderWaitLock;
     ...
     * Starts by allocating space for 2 render pipes and 1 audio pipe to be
     * handled.
     */
    ...
    /**
     * Construct a new render manager with no pipelines or renderers registered,
     * but with internal arrays setup for the given number of displays to be
     * added.
     *
    ...
    public MultiThreadRenderManager(int numDisplays) {
        ...
        displays = new ArrayList<>(numDisplays);
        displayThreadMap = new HashMap<>();
     ...
     * Setting a value of null will clear the currently set reporter. If one is
     * already set, the new value replaces the old.
     *
    ...
    /**
     * Tell render to start or stop management. If currently running, it will
     * wait until all the pipelines have completed their current cycle and will
     * then halt.
     *
    ...
    /**
     * Force a single render of all pipelines now. Ignores the enabled and cycle
     * time settings to cause a single render at this point in time. If a render
     * is currently in progress, an exception is generated
     *
     ...
     * reasons, a render manager may elect to not run or update some portions of
     * the scene graph. This method requests that the next frame only should
     ...
     * milliseconds that should be the minimum between frames and can be used to
     * throttle the management loop to a maximum frame rate should other systems
     * require CPU time.
     *
    ...
    /**
     * Add a dislay collection to be managed. A duplicate registration or null
     * value is ignored.
     *
    ...
    public void removeDisplay(DisplayCollection collection)
        ...
        DisplayCollectionThread th
                = displayThreadMap.remove(collection);
    ...
    /**
     * Set the picking handler to use. Overrides the default implementation that
     * is used. If null is passed, the code reverts to the default
     ...
     * safe to update the scene graph. A value of null will remove the currently
     * set value.
     *
     ...
     * application to make sure the {@link #shutdown()} method is called to turn
     * off the OpenGL management system. If it does not, there is a good
    ...
    /**
     * Notify the handler that you have updates to the SG that might alter a
     * node's bounds.
     *
     * @param l The change requestor
     * @param src The object that is passing this listener through.
     * @param intL Internal listener for making callbacks at a later time to
     * propagate the bounds changes.
    ...
    /**
     * Notify the handler that you have updates to the SG that will not alter a
     * node's bounds.
     *
     * @param l The change requestor
     * @param src The object that is passing this listener through.
     ...
     * sound rendering. Note that updating the active sound node means that the
     * other sound node is disabled. This will be called on the data change
     * callback normally. The source object will be an instance of either Layer
     * or ViewportLayer, depending on the circumstances.
     *
     * @param intL Internal listener for making callbacks at a later time to
     * propagate when the target is no longer the active listener.
     */
    @Override
    ...
    /**
     * The shader object passed requires updating the log info. Queue the shader
     * up for processing now so that at the next oppourtunity it can call
     * glGetLogInfoARB.
     *
     * @param shader The shader instance to queue
     * @param updateResponse true if this is being made as a response to a
     ...
     * texture object ID. The reasons why this object is now marked for deletion
     * are not defined - that is the sole discretion of the calling code.
     * <p>
     *
     * This renderable instance will begin the deletion processing as soon as
     * the start of the next culling pass begins. Once it hits the output
     ...
     * ignored.
     *
     * @param deletable The renderable that will handle the cleanup at the
     * appropriate time
     */
    @Override
    ...
    /**
     * Woops, we were in error, so please rescind that deletion request. For
     * example, during the data update change processing a texture was
