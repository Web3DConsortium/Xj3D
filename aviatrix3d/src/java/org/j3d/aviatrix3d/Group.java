 ...
 * <p>
 * Lights are automatically scoped and culled to the parent group unless
 * the global flag is set to true on them.
 * <p>
               ...
               GroupCullable
{
    ...
    private static final String CHILD_IDX_ERR_PROP =
        "org.j3d.aviatrix3d.Group.invalidChildIndexMsg";
    ...
    public void requestBoundsUpdate()
    {
        ...
        // want in here. It would mean that if we remove all the children and
        // the node is not live, then we would never get recalculated back to
        // BOUNDING_VOID and keep whatever the last bounds were.
        ...
        for(int i = 0; i < lastChild; i++)
        {
            if(childList[i] != null)
            {
                if(childList[i] instanceof MultiParentNode)
                    ((MultiParentNode)childList[i]).setLive(this, state);
                else
        ...
        throws AlreadyParentedException, InvalidWriteTimingException
    {
        ...
        // Moved to after so updBounds will do something
        if(newChild != null)
        {
        ...
        throws InvalidWriteTimingException
    {
        ...
        int move_amt = lastChild - idx;
        lastChild--;

        Node old_child = childList[idx];

        System.arraycopy(cullList, idx + 1, cullList, idx, move_amt);
        System.arraycopy(childList, idx + 1, childList, idx, move_amt);
        System.arraycopy(pickableList, idx + 1, pickableList, idx, move_amt);
