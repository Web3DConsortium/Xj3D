 ...
 * that depend on getting some resize information (e.g. using our utility
...
public class PbufferSurface extends BaseSurface
{
        ...
        throws PbufferUnavailableException
    {
        ...
        if(isShared())
            shared_context = sharedSurface.canvas.getContext();

        GLDrawableFactory factory = GLDrawableFactory.getFactory(GLProfile.getDefault());

        if(!factory.canCreateGLPbuffer(factory.getDefaultDevice(),GLProfile.getDefault()))
        {
            I18nManager intl_mgr = I18nManager.getManager();
        ...
        canvas = factory.createOffscreenAutoDrawable(factory.getDefaultDevice(), caps, chooser, surfaceWidth, surfaceHeight);
        ...
        if (isShared())
            ((GLSharedContextSetter)canvas).setSharedContext(shared_context);
