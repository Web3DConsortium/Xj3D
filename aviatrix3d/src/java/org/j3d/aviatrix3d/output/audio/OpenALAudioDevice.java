    ...
    implements AudioOutputDevice
{
    ...
    /** Listener Position */
    private final Point3f listenerPoint;

    /** Scratch point */
    private final Point3f tmpPoint;
    ...
    static
    {
        // Need to allocate a byte buffer of 24 bytes, to equate to 6 floats
...
//                alc.alcMakeContextCurrent(context);
                ...
                int[] buffer = null;
                int[] source = null;
                AudioDetails[] details = commands.renderList;
                for (AudioDetails detail : details) {
                    sound = (Sound) detail.renderable;

                    if (sound != null) {
                        buffer = new int[] {sound.getBuffer()};
                        source = new int[] {sound.getSource()};
                        break;
                    }
                }
