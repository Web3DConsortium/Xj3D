...
/**
 * Flat, polygonalised 2D text representation.
 * <p>
 *
 ...
 * such that the ray is parallel to the local X-Y plane and does not change in Z
 * depth, then no intersection will be generated. The implementation attempts to
 * optimise the testing based on the bounds of the individual character before
 * checking polygons.
 * <p>
 *
...
public class Text2D extends Geometry {
    ...
    /** Message when the user provides a dodgy justification flag */
    ...
    private List<CharacterData>[] textCharacters;

    /** The characters adjusted for leftToRight & topToBottom */
    private List<CharacterData>[] renderedCharacters;
    ...
    /** Computed minimum bounds for the character set */
    private float[] boundsMin;

    /** Computed maximum bounds for the character set */
    private float[] boundsMax;

    /** Reused List of CharacterData in the render method */
    private List<CharacterData> renderedCharacter;

    /**
     * Create a new, default text representation. No font information is set, so
     * a plain, 12 point font is used.
     */
    public Text2D() {
        this(null);
    }
     ...
     * {@link org.j3d.geom.CharacterCreator}. If the argument is null, then the
    ...
    public Text2D(CharacterCreator charGen) {
        ...
        sizeScale = 1;
        spacingScale = 1;
        hJustification = JUSTIFY_BEGIN; // defaults 15.4.1 FontStyle and 15.2.2.3 Direction and justification
        vJustification = JUSTIFY_FIRST;
        maxExtents = 0;
    ...
    public void render(GL2 gl) {
        ...
        if (horizontal)
        {
            ...
            for (int i = 0; i < text.length; i++) {
                renderedCharacter = renderedCharacters[i];

                // A little bias to get the spacing looking better
                y -= (lineSpacing[i]+0.25);

                gl.glPushMatrix();

                // Using the "y" component of the bbox and a bias to
                // "help" the vertical justification look right
                gl.glTranslatef(lineStartPos[i], y+(boundsMax[1]/2), 0);
                gl.glScalef(sizeScale, sizeScale, 1);

                for (CharacterData data : renderedCharacter) {
        ...
        else // (horizontal false) means vertical
        {
            float x = 0, y;
            ...
            for (int i = 0; i < text.length; i++) {
                ...
                // Using the "y" component of the bbox to
                // "help" the vertical justification look right.  This does not
                // work as well for horizontal=false, but is a step in the right
                // direction
                y = lineStartPos[i]+boundsMax[1]; // boundsmAX[I]
     ...
     */
    @Override
    public int compareTo(Renderable o) throws ClassCastException {
        Text2D txt = (Text2D) o;
     ...
     * the live state of the nodes represents the same state as the parent scene
     * graph.
     *
    ...
    protected void recomputeBounds() {
        ...
        if (horizontal) {
            ...
            List<CharacterData> textCharacter;
            int size;
            float local_y, local_x;
            CharacterData data;
            LineMetrics line_m;
            for (int i = 0; i < text.length; i++) {
                textCharacter = textCharacters[i];
                size = textCharacter.size();
                ...
                for (CharacterData d : textCharacter) {
                    local_x += d.bounds.width;
                }

                data = textCharacter.get(0);
//System.out.println("Normal y = " + local_y);

                line_m = font.getLineMetrics(text[i], frc);
...
//System.out.println("line [" + i + "] " + local_y + " " + sizeScale + " " + spacingScale);
        ...
        else // vertical
        {
            ...
            for (int i = 0; i < text.length; i++) {
                ...
                for (CharacterData data : textCharacter) {
                    local_y += data.bounds.width;
                    x = data.bounds.height;
                    if (x > local_x) {
                        local_x = x;
                    }
                }
            ...
            switch (vJustification) {
                ...
                case JUSTIFY_MIDDLE:
                    for (int i = 0; i < text.length; i++) {
                        lineStartPos[i] *= -0.5f; // hack?  TODO computation expected
                    }
                    break;
        ...
        switch (hJustification) {
            ...
            case JUSTIFY_FIRST:
                start_x = -max_x * 0.3f; // TODO why not 0.5f?
                end_x =  max_x;
                break;
        ...
        switch (vJustification) // TODO fix
        // the following start_y end_y values are all defined by trial and error, likely erroneous, original design unclear
        // not clear that end_y has an effect
        // most important is JUSTIFY_MIDDLE
        {
            case JUSTIFY_BEGIN:
                start_y = 2.0f * max_y;
                end_y = 0;
            ...
            case JUSTIFY_FIRST:
                start_y =  1.8f * max_y; // TODO why this factor?
                end_y = -max_y;
                break;
        ...
        bounds.getExtents(boundsMin, boundsMax);              // ??? apparently unused?
        ...
        if (leftToRight) {
            ...
            List<CharacterData> this_line;
            int num_chars;
            for (int i = 0; i < textCharacters.length; i++) {
                renderedCharacters[i].clear();
                this_line = textCharacters[i];
                num_chars = this_line.size();
    ...
    /**
     * Replace the internal character generator with another. If the argument is
     * null, then the default is used again.
     *
     * @param charGen The creator of characters to use, or null
     */
    ...
    public void setText(String[] text, int numText) {
        ...
        if (numText == 0) {
            ...
            CharacterData data;
            for (int i = 0; i < numText; i++) {
                characters = text[i].toCharArray();
                generator.createCharacterTriangles(characters,
                        characters.length,
                        textCharacters[i]);
                len = textCharacters[i].size();

                for (int j = 0; j < len; j++) {
                    data = textCharacters[i].get(j);
...
//        recomputeBounds();
    ...
    /**
     * Get a reference to the currently set collection of strings. If no strings
     * are set, then this returns null.
     *
    ...
    /**
     * Change whether the text should be placed from top to bottom or bottom to
     * top. Top to bottom is the default setting.
     *
     ...
     * between lines - such as double spaced text. The value must be a non-zero,
     * non-negative value.
     *
     ...
     *
     * @param size raw text size in units of the text, default 1.0
     * @throws IllegalArgumentException The value was not positive
     * @throws InvalidWriteTimingException An attempt was made to write outside
     * of the NodeUpdateListener bounds callback method
     */
    ...
    public void setVerticalJustification(int justify)
        ...
        if ((justify < JUSTIFY_FIRST) || (justify > JUSTIFY_END)) { // bounds checking
            I18nManager intl_mgr = I18nManager.getManager();
            String msg_pattern = intl_mgr.getString(INVALID_JUSTIFY_PROP);
