...
class HAnimShader extends Shader
{
    ...
    static
    {
        ...
        {
            if(is[i] == null)
                is[i] = cldr.getResourceAsStream(SHADER_FILES[i]);
        }
        ...
        SHADER_PROGRAM = new HAnimShaderProgram();
        ...
        for(int i = 0; i < is.length; i++)
        {
            ...
            StringBuilder str_buf = new StringBuilder(2_048);
            char[] buf = new char[1_024];
            int read;

            try
            {
                while((read = rdr.read(buf, 0, 1_024)) != -1)
            ...
            SHADER_PROGRAM.addShaderObject(shader);
        }

        SHADER_PROGRAM.bindAttributeName("weight", WEIGHT_ATTRIB_INDEX);
        SHADER_PROGRAM.bindAttributeName("matrixIndex", MATRIX_ATTRIB_INDEX);

        SHADER_PROGRAM.link();
        SHADER_PROGRAM.requestLinkConfirmation(true);
        SHADER_PROGRAM.requestInfoLog();
    }
    ...
    {
        if(SHADER_PROGRAM == null || !SHADER_PROGRAM.isValid(gl))
            return;

        SHADER_PROGRAM.render(gl);

        int id = SHADER_PROGRAM.getProgramId(gl);

        Integer i_id = idMap.get(id);
        if(i_id == null)
        {
            i_id = id;
            idMap.put(id, i_id);
        ...
        if(!shaderLogRecieved)
        {
            ...
            int num_objs = SHADER_PROGRAM.getNumShaderObjects();
            ShaderObject[] objs = new ShaderObject[num_objs];
            SHADER_PROGRAM.getShaderObjects(objs);
            ...
            msg = SHADER_PROGRAM.getLastInfoLog();
            if(msg != null && msg.length() != 0)
    ...
    protected void setUpdateHandler(NodeUpdateHandler handler)
    {
        ...
        if(SHADER_PROGRAM != null)
            SHADER_PROGRAM.setUpdateHandler(updateHandler);
    }
    ...
    boolean equals(HAnimShader sh)
    {
        ...
        return !((arguments == null) || !arguments.equals(sh.arguments));
    }
}
