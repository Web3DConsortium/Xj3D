...
public class MarkerGroup extends BaseGroup implements CustomCullable
{
    ...
    private List<TransformGroup> pathList;
    ...
    private void getLocalToVworld(Node node, Matrix4f mat)
	{
        ...
        Node lParent = node.getParent();
        while (lParent != null) {
            if (lParent instanceof SharedGroup) {
                SharedGroup sg = (SharedGroup) lParent;
                ...
                if (lParent instanceof TransformGroup) {
                    pathList.add((TransformGroup) lParent);
                }
