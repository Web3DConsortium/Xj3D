...
public class DEMParser implements HeightMapSource
{
    ...
    private static final CharHashMap<Integer> processCodes;

    /** The map of ground reference planes codes */
    private static final Map<String, Integer> groundRefCodes;

    /** Map of the unit of measure code character to the value */
    private static final CharHashMap<Integer> unitCodes;
     ...
     * any methods in this base class. Either the stream or the reader will
     * be set, not bother.
     */
    ...
    /** Working buffer to stave needing to reallocate all the time */
     ...
     * @throws java.io.IOException An error parsing the file
     */
    ...
    {
        if(inputStream != null)
            inputStream.read(buffer, 0, 1_024);
        else
        {
            inputReader.read(charBuffer, 0, 1_024);
    ...
    private void parseARecord() throws IOException
    {
        ...
        Integer code_int = processCodes.get(code);
        header.processCode = code_int != null ? code_int : 0;
        ...
        code_int = groundRefCodes.get(str);

        if(code_int != null)
            header.groundReferenceSystem = code_int;
        else
...
System.out.println("projection parameter parsing not handled yet");
        ...
        code_int = unitCodes.get(readChar(529, 6, '0'));
        header.groundUnitOfMeasure = code_int;

        code_int = unitCodes.get(readChar(535, 6, '0'));
        header.elevationUnitOfMeasure = code_int;
    ...
    private void parseBRecords() throws IOException
    {
        ...
        List<DEMTypeBRecord> records = new LinkedList<>();
     ...
     * @return the number of elevation values read
     */
    private void parseBRecordProfile(List<DEMTypeBRecord> allRecords) throws IOException
    {
     ...
     * to a double array of height values.
     *
     ...
     * in the stream, just characters that are turned into string values.
    ...
    {
        return (float) readDouble(start, len, defaultValue);
    }
