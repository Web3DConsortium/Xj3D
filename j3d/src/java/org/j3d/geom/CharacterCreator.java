...
public class CharacterCreator {
    ...
    /**
     * The flatness of the tessellation.
     */
     ...
     */
    private final TriangulationUtils triangulator;
     ...
     */
    private final CharHashMap<CharacterData> charDataMap;

    /**
     * Convenience array used to fetch coordinates from the PathIterator
     */
    private final float[] newCoords;
    ...
    /**
     * Place to put the triangulated index value output
     */
    ...
    /**
     * Convenience method that creates the glyph information for the specified
     * character.
     *
...
//System.out.println("final ascent " + ch_data.ascent);
        ...
        List<int[]> polyIndexList = new ArrayList<>();
        int[] poly_index;
//System.out.println("processing " + character + " flatness " + (flatness / scale));
        ...
        while (!glyph_path.isDone()) {
            switch (glyph_path.currentSegment(newCoords)) {
                ...
                case PathIterator.SEG_CLOSE:
                    poly_index = polyIndexList.get(polyIndexList.size() - 1);
        ...
        int parent_id, parent_start_idx, parent_end_idx, parent_num_vertices,
                num_children, child_id, child_start_idx, child_end_idx,
                child_array_length, child_num_vertices;
        int[] parent_poly_indices, children_poly_ids, child_poly_indices;
        float parent_start_x, parent_start_y;
        for (int i = 0; i < num_parent_polys; i++) {
            parent_id = parent_poly_ids[i];
            parent_poly_indices = polyIndexList.get(parent_id);
            parent_start_idx = parent_poly_indices[0];
            parent_end_idx = parent_poly_indices[1];
            parent_num_vertices = (parent_end_idx - parent_start_idx) / 3;
	     ...
	     // order the interior poly vertices such that the starting vertex
            ...
            children_poly_ids = (int[]) polyIDMap.get(parent_id);
            num_children = children_poly_ids.length;
            if (num_children > 0) {
                parent_start_x = charCoords[parent_start_idx];
                parent_start_y = charCoords[parent_start_idx + 1];
                for (int j = 0; j < num_children; j++) {
                    child_id = children_poly_ids[j];
                    child_poly_indices = polyIndexList.get(child_id);
                    child_start_idx = child_poly_indices[0];
                    child_end_idx = child_poly_indices[1];
                    child_array_length = child_end_idx - child_start_idx;
                    child_num_vertices = child_array_length / 3;
        ...
        for (int i = 0; i < num_parent_polys; i++) {
            ...
            if (num_triangles > 0) {
                ...
                System.err.println("can't tesselate \'" + character + "\'");
            }
            start_vtx += length;
        }
    ...
    /**
     * Convenience method that goes through a list of characters and creates the
     * needed glyph information.
     *
    ...
    /**
     * Point is inside check using the odd-even winding rule. Think of standing
     ...
     * make will be odd, whereas if you were outside the jumps will be even.
     *
    ...
    /**
     * Find the closest point on the previous curve that is closest to the given
     * point. Uses a simple radial distance calc.
     *
     * @param x The x coordinate of the point to test against
     * @param y The y coordinate of the point to test against
     * @param polygon The set of points of the polygon
    ...
    /**
     * Reorder the vertices of a polygon, with the specified index as the new
     * starting index
     *
     * @param coord The set of points of the polygon
     * @param start The index of the current start vertex in the coord array
     * @param num The number of vertices in the polygon
     * @param newStart The index in the coord array to make the new starting
     * index
     */
    ...
    /**
     * Determine whether the ordering of the vertices of a polygon is clockwise.
     * <p>
     ...
     * @param start The index of the start vertex in the coord array
     * @param num The number of vertices in the polygon
     * @return true if the ordering is clockwise, false if the ordering is
     * counter-clockwise.
     */
    ...
    /**
     * Return the bounds of the geometry. An array containing [x_min, x_max,
     * y_min, y_max].
     *
     * @param coord The set of points of the polygon
     * @param start The index of the start vertex in the coord array
     ...
     * @return true if internal is contained within external, false otherwise.
     */
    private boolean contains(float[] external, float[] internal) {
        return ((internal[0] > external[0]) && (internal[1] < external[1])
                && (internal[2] > external[2]) && (internal[3] < external[3]));
    }

    /**
     * Group the polygons together based on their bounds. Polygons that are
     * contained within the bounds of another polygon are included into that
     * polygon's group.
     *
     * @param coord The set of points of the polygon
     ...
     * the polyIndexList.
     */
    private IntHashMap group(float[] coord, List<int[]> polyIndexList) {
        int num_poly = polyIndexList.size();
        IntHashMap<int[]> map = new IntHashMap<>(num_poly);

        // get the bounds of each polygon for comparison
        float[][] bounds = new float[num_poly][];
        int[] poly_idx;
        for (int i = 0; i < num_poly; i++) {
            poly_idx = polyIndexList.get(i);
        ...
        for (int i = 0; i < num_poly; i++) {
            ...
            children = new int[idx];
            System.arraycopy(set, 0, children, 0, idx);
            map.put(i, children);
        }
     ...
     * @param polyIndexList The list containing the beginning and ending indices
     * of each polygon.
     * @param polyIDMap A map containing arrays of indices within the
     * polyIndexList that are to be grouped together. The map is keyed by
     * indices to polygons from the polyIndexList.
     * @param id The polygon id (map key) that identifies the exterior polygon
     * to combine with it's interior polygons.
     * @return The combined vertices of the polygons
     */
    private float[] combine(float[] coord, List<int[]> polyIndexList,
            IntHashMap polyIDMap, int id) {
        float[] combined_coord;
	    ...
	    // determine the insertion index for each interior poly
            ...
            int child_poly_id, child_start_idx, child_end_idx, child_array_length;
            int[] child_poly_indices;
            float child_start_x, child_start_y;
            for (int i = 0; i < num_children; i++) {
                child_poly_id = children_poly_ids[i];
                child_poly_indices = polyIndexList.get(child_poly_id);
                child_start_idx = child_poly_indices[0];
                child_start_x = coord[child_start_idx];
                child_start_y = coord[child_start_idx + 1];
                child_end_idx = child_poly_indices[1];
                child_array_length = child_end_idx - child_start_idx;
            ...
            int idx, min_pass, point;
            for (int i = 0; i < num_children; i++) {
                idx = 0;
                min_pass = Integer.MAX_VALUE;
                for (int j = 0; j < num_children; j++) {
                    if (!used[j]) {
                        point = insertion_point[j];
                        if (point < min_pass) {
                            idx = j;
                            min_pass = point;
                        }
