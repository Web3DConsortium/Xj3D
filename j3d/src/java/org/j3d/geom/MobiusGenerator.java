...
public class MobiusGenerator extends GeometryGenerator
{
    ...
    private boolean coordsDimensionsChanged;

    private boolean normalsDimensionsChanged;

    private boolean coordsCountChanged;

    private boolean normalsCountChanged;
    ...
    private void generateUnindexedCoordinates(GeometryData data)
    {
        ...
        int vertexCount = (strips + 1) * this.divisions * 4;

        coordsDimensionsChanged = false;
        if((data.coordinates == null) ||
           (data.coordinates.length != vertexCount * 3))
        {
            data.coordinates = new float[vertexCount * 3];
        }
        ...
        while (uu <= umax)                //outer loop
        {
            ...
            while (vv <= vmax)            //inner loop
            {
                ...
                z = (float)(vv * sin_uu_2);
                data.coordinates[count++] = x;
                data.coordinates[count++] = y;
                data.coordinates[count++] = z;
