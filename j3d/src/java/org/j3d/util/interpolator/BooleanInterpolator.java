...
public class BooleanInterpolator extends Interpolator
{
    ...
    /**
     * Create a new sequencer instance with the default size for the
     ...
     */
    public BooleanInterpolator(int size, int type)
    {
        super(size, type);

        keyValues = new boolean[size];
    }
     ...
     */
    public void addKeyFrame(float key, boolean value)
    {
    ...
    public boolean booleanValue(float key)
    {
        ...
        else
        {
            ...
            {
                case LINEAR:
                    boolean p1 = keyValues[loc + 1];
                    boolean p0 = keyValues[loc];
...
*/
    ...
    private void realloc()
    {
        if(currentSize == allocatedSize)
        {
            ...
            // Don't acutally allocate the space for the values as the
            // arraycopy will set these. Just make sure we allocate after that
            // the remaining new, empty, places.
            boolean[] new_values = new boolean[new_size];
    ...
    {
        StringBuilder buf = new StringBuilder("<boolean interpolator>\n");
        ...
        for(int i = 0; i < currentSize; i++)
        {
            ...
            buf.append(keys[i]);
            buf.append(" value: ");
            buf.append(keyValues[i]);
            buf.append("\n");
        }

        buf.append("</boolean interpolator>");
        return buf.toString();
    }
}
