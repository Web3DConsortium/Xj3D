...
public abstract class tController implements BrowserListener, X3DFieldEventListener {
    ...
    /**
     * Synchronization variable, indicating the browser setup has completed and
     * the event system is ready.
     */
    boolean browserInitialized;

    /**
     * Synchronization variable, indicating the last event cascade has
     * completed.
     */
    ...
    /**
     * Signal the controller that all pending updates shall be released to the
     * browser. This method blocks until notification is received that the
    ...
    public void flushUpdate() {
        synchronized (this) {
            ...
            try {
                while (!this.eventCascadeOccured) {
                    wait();
                }
                this.eventCascadeOccured = false;
