...
/**
 * Test wrapper for the MFFloat X3DField type
 */
public class tMFFloat implements tX3DField {

    /**
     * Default smoke test write value
     */
    public final static float[] smoke_value = new float[]{0.0f, 1.0f};
     ...
     * @param control
     */
    public tMFFloat(final X3DNode node, final X3DFieldDefinition def, final tController control) {
        this.nodeName = node.getNodeName();
        this.fieldName = def.getName();
        this.field = (MFFloat) node.getField(fieldName);
    ...
    public boolean smoke() {
        ...
        switch (access) {
            ...
            case X3DFieldTypes.OUTPUT_ONLY:
                r_value = new float[field.getSize()];
