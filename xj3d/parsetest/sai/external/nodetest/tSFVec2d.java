...
/**
 * Test wrapper for the SFVec2d X3DField type
 */
public class tSFVec2d implements tX3DField {

    /**
     * Default smoke test write value
     */
    public final static double[] smoke_value = new double[]{0.1, 0.1};
     ...
     * @param control
     */
    public tSFVec2d(final X3DNode node, final X3DFieldDefinition def, final tController control) {
        this.nodeName = node.getNodeName();
        this.fieldName = def.getName();
        this.field = (SFVec2d) node.getField(fieldName);
    ...
    public boolean smoke() {
        final double[] r_value = new double[2];
                ...
                for (int i = 0; i < r_value.length; i++) {
                    if (w_value[i] != r_value[i]) {
                        control.logMessage(tMessageType.ERROR, new String[]{
                            nodeName + ":" + fieldName,
                            "\twrote [ " + w_value[0] + " " + w_value[1] + " ]",
                            "\tread  [ " + r_value[0] + " " + r_value[1] + " ]"});
                        return (FAIL);
                    }
                }
                break;
