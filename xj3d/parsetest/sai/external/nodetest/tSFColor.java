...
/**
 * Test wrapper for the SFColor X3DField type
 */
public class tSFColor implements tX3DField {

    /**
     * Default smoke test write value
     */
    public final static float[] smoke_value = new float[]{0.1f, 0.1f, 0.1f};
     ...
     * @param control
     */
    public tSFColor(final X3DNode node, final X3DFieldDefinition def, final tController control) {
        this.nodeName = node.getNodeName();
        this.fieldName = def.getName();
        this.field = (SFColor) node.getField(fieldName);
    ...
    public boolean smoke() {
        final float[] r_value = new float[3];
