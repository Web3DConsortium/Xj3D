...
/**
 * Test wrapper for the MFRotation X3DField type
 */
public class tMFRotation implements tX3DField {

    /**
     * Default smoke test write value
     */
    public final static float[] smoke_value = new float[]{
        0.707f, 0.707f, 0.0f, (float) Math.PI,
        0.0f, 0.0f, 0.707f, (float) Math.PI / 2};

    /**
     * The field
     */
    final MFRotation field;
     ...
     * @param control
     */
    public tMFRotation(final X3DNode node, final X3DFieldDefinition def, final tController control) {
        this.nodeName = node.getNodeName();
        this.fieldName = def.getName();
        this.field = (MFRotation) node.getField(fieldName);
    ...
    public boolean smoke() {
        ...
        switch (access) {
            ...
            case X3DFieldTypes.OUTPUT_ONLY:
                r_value = new float[field.getSize() * 4];
