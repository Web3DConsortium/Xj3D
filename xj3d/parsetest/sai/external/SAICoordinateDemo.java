...
/**
 * A test of Coordinate, IndexedLineSet, and set1Value on MFVec3f's. The test is
 * using IndexedLineSets because I couldn't come up with a simple manipulation
 * that wouldn't end up shredding polygons.
 * <P>
 */
public class SAICoordinateDemo {
    ...
    public static void main(String[] args) {
        ...
        for (X3DNode x3dNode : tempNodes) {
            tempScene.removeRootNode(x3dNode);
        }

        ((MFNode) nodes[0].getField("set_children")).setValue(tempNodes.length, tempNodes);

        X3DNode children[] = new X3DNode[((MField) (tempNodes[0].getField("children"))).getSize()];
        ...
        MFVec3f coordOutput = (MFVec3f) (coord.getField("point"));
        MFVec3f coordInput = (MFVec3f) (coord.getField("point"));
...
class BasicSAICoordinateMutatorOne implements X3DFieldEventListener {
    ...
    public void readableFieldChanged(X3DFieldEvent evt) {
        ...
        buffer[0] -= (0.5f + r.nextFloat());
        buffer[1] -= (0.5f + r.nextFloat());
        buffer[2] -= (0.5f + r.nextFloat());
