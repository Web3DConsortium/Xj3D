...
/**
 * ***************************************************************************
 * Web3d.org Copyright (c) 2006 - 2007 Java Source
 *
...
/**
 * A testcase for SFImage that will exercise setValue(), setImage(), getImage()
 * and setSubImage().
 *
 * @author Rex Melton
...
public class SFImageTest3 extends JFrame implements ActionListener, ChangeListener {
    ...
    public SFImageTest3() {
        ...
        spinner.addChangeListener(SFImageTest3.this);
        ...
        readButton.addActionListener(SFImageTest3.this);
        ...
        writeButton.addActionListener(SFImageTest3.this);
    ...
    private void loadScene() {
        ...
        try {
            ...
            scene = browser.createX3DFromURL(new String[]{file.toURI().toURL().toExternalForm()});
            X3DNode pixelTex = scene.getNamedNode("pixelTex");
            sfimage = (SFImage) pixelTex.getField("image");
        } catch (MalformedURLException | InvalidBrowserException | InvalidURLException | InvalidX3DException | InvalidFieldException | InvalidNodeException e) {
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (source == readButton) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    if (useImageIO) {
                        InputStream in = image_file.toURI().toURL().openStream();
                        bImage = ImageIO.read(in);
                    } else {
                        Object content = image_file.toURI().toURL().openConnection().getContent();

                        if (content instanceof BufferedImage) {
                            bImage = (BufferedImage) content;
                        } else if (content instanceof ImageProducer) {
                            bImage = ImageUtils.createBufferedImage((ImageProducer) content);
                        }
                    }
                    System.out.println("Setting image " + bImage);
                    sfimage.setImage(bImage);
    ...
    /**
     * Change the pixel texture to have the number of components from the
     * spinner
     *
     * @param ce
     */
     ...
     */
    private void setPixelTexture(int i) {
        switch (i) {
            case 1:
