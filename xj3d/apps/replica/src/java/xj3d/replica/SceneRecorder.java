...
public class SceneRecorder implements ActionListener, RecorderListener {
    ...
    public SceneRecorder ( String[] arg ) {
        ...
        if ( !success ) {
            ...
            System.exit( ReplicaConstants.INVALID_ARGUMENTS );
        }
        ...
        if ( showWindow || interactive ) {
            if ( headless ) {
                ...
                if ( interactive ) {
                    ...
                    openItem.addActionListener( SceneRecorder.this );
        ...
        if ( !interactive ) {
            ...
            try {
                ...
                logger.errorReport("Unable to create InputSource: ", ex);
            }
            success = loader.load( is );
            // TODO wait until load complete
            if ( success ) {
                // start the recording, the completion will be handled
                // by the RecorderListener method
                recorder.start( SceneRecorder.this );
            } else {
                logger.fatalErrorReport(
                    LOG_NAME +": Error loading source file", null );
                System.exit( ReplicaConstants.INPUT_FILE_NOT_FOUND );
            }
        }
    }
    ...
    private boolean parseArgs( String[] arg ) {
        ...
        if ( encoding_string == null ) {
            ...
            for (String format : formats) {
                if (encoding_string.equals(format)) {
        ...
        if ( frame_rate_string == null ) {
            ...
            try {
                frameRate = Integer.valueOf(frame_rate_string );
            } catch( NumberFormatException e ) {
                logger.warningReport(
