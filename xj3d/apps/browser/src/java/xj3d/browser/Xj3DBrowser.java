...
public class Xj3DBrowser extends JFrame
    ...
    private static final String USAGE_MSG =
      ...
      "  -anisotropicDegree n    Forces anisotropic filtering of nTH degree\n" +
      ...
      "  -title                  Set a title to show in the browser window frame\n" +
    ...
    private final ErrorReporter console;
    ...
    private final Map<String, VRMLNode> viewpointDefMap;
    ...
    private final ViewpointManager viewpointManager;
    ...
    /** Frame state manager */
    private FrameStateManager state_manager;
    ...
    public void resetSurface() {
        ...
        if (useJoglNewt || surface instanceof SimpleNEWTSurface)
            canvas = new NewtCanvasAWT((com.jogamp.newt.Window) surface.getSurfaceObject());
        else
    ...
    protected void processWindowEvent(WindowEvent e) {
        ...
        switch (e.getID()) {
            case WindowEvent.WINDOW_CLOSING:
                ...
                // Better to call this than harsh sys exit. Let the last thread
                // running determine JVM shutdown.
                dispose();
...
//                System.exit(0);
    ...
    public void deviceActivated(int type) {
        statusBar.setStatusText("Activate: " + activeSensor);
    }
    ...
    public void linkActivated(String[] urls, String[] params, String desc) {
        ...
        for (String url1 : urls) {
            if (url1.charAt(0) == '#') {
                ...
                // load the world.
                try {
                    InputSource is = new InputSource(url1);
                    if(success = load(is)) {
                        break;
                    }
...
//        route_manager.setRouterFactory(new SimpleRouterFactory());
     ...
     * currently in use.
     * @param shutDownThread if the RuntimeHook was activated
     */
    private void shutdownApp(boolean shutDownThread)
    {
        OriginManagerFactory.removeInstance(state_manager);
        ...
        for(int i = 0; i < args.length; i++) {
            if(args[i].startsWith("-")) {
                if(args[i].equals("-fullscreen")) {
                    ...
                    try {
                        String val = args[i+1];
                        fullscreen = Integer.parseInt(val);
                        lastUsed = i + 1;
                    ...
                    try {
                        String val = args[i+1];
                        screenSize[0] = Integer.parseInt(val);
                        val = args[i+2];
                        screenSize[1] = Integer.parseInt(val);
                        lastUsed = i + 2;
                    } catch(NumberFormatException e) {
                    ...
                    String val = args[++i];
                    anisotropicDegree = Integer.parseInt(val);
                    if (anisotropicDegree > 1)
                        useMipMaps = true;
                    lastUsed = i;
                } else if (args[i].equals("-antialias")) {
                    String val = args[++i];
                    desiredSamples = Integer.parseInt(val);
                    lastUsed = i;
