...
public class Xj3DFilter extends JFrame implements ActionListener, WindowListener {
     ...
     */
    private final List<String> argument_list;
    ...
    public void windowClosing(WindowEvent we) {
        ...
        for (int i = 0; i < num_custom_filter; i++) {
            fp = (FilterParam) custom_filters.get(i);
            ...
            if (filesValid) {
                ...
                if (num_filter > 0) {
                    ...
                    for (int i = 0; i < num_filter; i++) {
                        fp = (FilterParam) filters.getElementAt(i);
    ...
    private void loadFilterDoc(Document doc) {
        ...
        if (chain_element.getTagName().equals("filter_chain")) {
            ...
            String name, options;
            for (int i = 0; i < num_filter; i++) {
                filter_element = (Element) filter_list.item(i);
                name = filter_element.getAttribute("name");
                options = filter_element.getAttribute("options");
                fp = new FilterParam(name, name);
	...
	//----------------------------------------------------------
        ...
        private FilterParam[] getCustomFilterParams() {
            ...
            try {
                ...
                String key_name, className;
                int num_filter;
                for (int i = 0; i < num_key; i++) {
                    key_name = keys[i];
                    if (key_name.startsWith("CustomFilter")) {
                        className = prefs.get(key_name, null);
                        if (className != null) {
                ...
                num_filter = filter_list.size();
                fp = new FilterParam[num_filter];
                for (int i = 0; i < num_filter; i++) {
                    className = filter_list.get(i);
