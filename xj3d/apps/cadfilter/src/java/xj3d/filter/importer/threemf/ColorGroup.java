 ...
 * @author Alan Hudson
 */
public class ColorGroup implements ModelResource, ThreeMFElement, PropertySource {
    public static final int ELEMENT_ID = ThreeMFElementFactory.COLOR_GROUP;
    ...
    public ColorGroup(Attributes atts) {
    ...
    /**
     * Get the color as a float array(rgba)
     * @return
     */
    public float[][] getColor4Components() {
        return colors;
    }

    public float[][] getColor3Components() {
        float[][] color3 = new float[colors.length][3];
        int len = colors.length;
        for(int i=0; i < len; i++) {
            color3[i][0] = colors[i][0];
            color3[i][1] = colors[i][1];
            color3[i][2] = colors[i][2];
        }
