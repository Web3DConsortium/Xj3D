 ...
 */
public class STLBinaryFileExporter extends Exporter
        implements BinaryContentHandler {

    private static final String NAME = "STLBinaryExporter";
    ...
    /**
     * Have we issued the triangle count
     */
    private boolean issuedCount;
     ...
     * @param tc A stream triangle counter
     * @param reporter The error reporter to use
     */
    public STLBinaryFileExporter(OutputStream os, int major, int minor,
            TriangleCounter tc, ErrorReporter reporter) {
     ...
     * <CODE>#</CODE> start. The type string contains the representation of the
     * first few characters of the file after the #. This allows us to work out
     * if it is VRML97 or the later X3D spec.
     * <p>
     * Version numbers change from VRML97 to X3D and aren't logical. In the
     * first, it is <code>#VRML V2.0</code> and the second is
     * <code>#X3D V1.0</code> even though this second header represents a later
     * spec.
     *
     * @param uri The URI of the file.
     * @param url The base URL of the file for resolving relative URIs contained
     * in the file
     ...
     * @param comment Any trailing text on this line. If there is none, this is
     * null.
    ...
    public void startDocument(String uri,
        ...
        // TODO: anything we want to put here, just can't be solid
        byte[] b = new byte[80];
     ...
     * of the parser for this document. The name is the name of the profile for
     * the document to use.
     *
    ...
    /**
     * A component declaration has been found in the code. There may be zero or
     * more component declarations in the file, appearing just after the profile
     * declaration. The textual information after the COMPONENT keyword is left
     * unparsed and presented through this call. It is up to the user
    ...
    /**
     * An EXPORT declaration has been found in the document. Both paramters will
     * always be provided regardless of whether the AS keyword has been used.
     * The parser implementation will automatically set the exported name as
     * needed.
     *
    ...
    public void startNode(String name, String defName)
        ...
        if (name.equals("IndexedTriangleSet") || name.equals("TriangleSet")) {
     ...
     */
    @Override
    @SuppressWarnings("unchecked") // casts from home rolled Map derivative
        ...
        switch (nodeName) {
            case "IndexedTriangleSet": {
                if (!issuedCount) {
                    issuedCount = true;
                    try {
                        writeInt(triangleCounter.getTriangleCount());
                    } catch (IOException ioe) {
                        // Comes from FilterExitCodes.CANNOT_WRITE_OUTPUT_FILE
                        lastErrorCode = 5;
                        ExporterException ee = new ExporterException(NAME, 5, "Failed to write an STL Binary Value.  msg: " + ioe.getMessage());
            ...
            case "TriangleSet": {
                if (!issuedCount) {
                    ...
                    try {
                        ...
                        errorReporter.errorReport("Failed to write an STL Binary Value", ioe);
                    }
                }
                insideITS = false;
                fieldValues = (Map<String, String>) fieldValuesStack.pop();
...
//System.out.println("    Exporter.startField: " + name);
     ...
     * {@link StringContentHandler#fieldValue(String)}. or any of the fieldValue
     * methods in {@link BinaryContentHandler}. This will only ever be called if
     * there have been nodes declared. If no nodes have been declared (ie "[]")
     * then you will get a <code>fieldValue()</code>. call with the parameter
     * value of null.
     *
     * @throws SAVException This call is taken at the wrong time in the
     * structure of the document.
...
//System.out.println("    Exporter.endField: " + name);
     ...
     * <p>
     * There are times where we have an MFField that is declared in the file to
     * be empty. To signify this case, this method will be called with a
    ...
    public void fieldValue(float value)
        ...
        if (insideITS) {
            ...
            @SuppressWarnings("unchecked") // cast from home rolled Map derivative
            Map<String, String> fieldValues = (Map<String, String>) fieldValuesStack.peek();
            fieldValues.put(nodeName + "." + fieldName, Float.toString(value));
        }
...
//System.out.println("    value: " + value.toString().trim());
    ...
    public void fieldValue(long value)
        ...
        if (insideITS) {
            ...
            @SuppressWarnings("unchecked") // cast from home rolled Map derivative
            Map<String, String> fieldValues = (Map<String, String>) fieldValuesStack.peek();
            fieldValues.put(nodeName + "." + fieldName, Long.toString(value));
        }
    ...
    public void fieldValue(double value)
        ...
        if (insideITS) {
            ...
            @SuppressWarnings("unchecked") // cast from home rolled Map derivative
            Map<String, String> fieldValues = (Map<String, String>) fieldValuesStack.peek();
            fieldValues.put(nodeName + "." + fieldName, Double.toString(value));
        }
     ...
     * with an IS statement then the value returned here is null. There will be
     * a subsequent call to the ProtoHandlers <CODE>protoIsDecl()</CODE> method
     * with the name of the field included.
     *
     * @param access The access type (eg exposedField, field etc)
     * @param type The field type (eg SFInt32, MFVec3d etc)
     * @param name The name of the field
     * @param value The default value of the field as either String or String[].
     * Null if not allowed.
     * @throws SAVException This call is taken at the wrong time in the
     * structure of the document.
     * @throws VRMLException This call is taken at the wrong time in the
    ...
    /**
     * Notification of the end of an ordinary proto declaration statement. This
     * is called just after the closing bracket of the declaration and before
     * the opening of the body statement. If the next thing called is not a
     * {@link #startProtoBody()} Then that method should toss an exception.
     *
     * @throws SAVException This call is taken at the wrong time in the
     * structure of the document.
     * @throws VRMLException This call is taken at the wrong time in the
     ...
     * the body and not the normal scenegraph information.
     *
     * @throws SAVException This call is taken at the wrong time in the
     * structure of the document.
     * @throws VRMLException This call is taken at the wrong time in the
    ...
    /**
     * Notification of the end of an EXTERNPROTO declaration. This is called
     * just after the closing bracket of the declaration and before the opening
     * of the body statement. If the next thing called is not a
     * {@link #externProtoURI} Then that method should toss an exception.
     *
     * @throws SAVException This call is taken at the wrong time in the
     * structure of the document.
     * @throws VRMLException This call is taken at the wrong time in the
     ...
     * as an array of URI strings. The calling application is required to
     * interpret the incoming string. Even if the externproto has no URIs
     * registered, this method shall be called. If there are none available,
     * this will be called with a zero length list of values.
     *
     ...
     * @throws java.io.IOException
     */
    protected void writeFloat(float val) throws IOException {
        prepareBuffer(Float.floatToRawIntBits(val));
        writer.write(buf, 0, 4);
    }
}
