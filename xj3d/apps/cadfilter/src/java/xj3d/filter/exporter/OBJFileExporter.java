 ...
 */
public class OBJFileExporter extends Exporter
    implements BinaryContentHandler {

    private static final String NAME = "OBJFileExporter";

    /** The maximum number of digits for an fraction (float or double) */
//    private final static int MAX_FRACTION_DIGITS = 6;
    private final static int MAX_FRACTION_DIGITS = 10;
     ...
     */
    public OBJFileExporter(OutputStream os, int major, int minor,
        ...
        try {
            ...
            lastErrorCode = 5;
            ExporterException ee = new ExporterException(NAME,5,"Failed to write an OBJ value.  msg: " + ioe.getMessage());
            throw ee;
        }
    ...
    public void endNode() throws SAVException, VRMLException {
        ...
        if (nodeName.equals("IndexedTriangleSet")) {
            ...
            for(int i=0; i < len; i++) {
                writeln("v " + numberFormater.format(coord[i*3]) + " " +
                        numberFormater.format(coord[i*3+1]) + " " +
                        numberFormater.format(coord[i*3+2]));
            }
            ...
            if (normal != null) {
                ...
                for(int i=0; i < len; i++) {
                    writeln("vn " + numberFormater.format(normal[i*3]) + " " +
                            numberFormater.format(normal[i*3+1]) + " " +
                            numberFormater.format(normal[i*3+2]));
                }
            }
            ...
            if (texture == null && normal == null) {
                ...
                for(int i=0; i < len; i++) {
                    ...
                    // write vertices
                    writeln("f " +
                            numberFormater.format(coordIndex[i*3] + 1) + " " +
                            numberFormater.format(coordIndex[i*3+1] + 1) + " " +
                            numberFormater.format(coordIndex[i*3+2] + 1));
