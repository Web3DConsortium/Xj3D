...
public class CDFFilter {
    ...
    public CDFFilter() {
        ...
        content_map.registerType("3mf", "application/vnd.ms-3mfdocument");
    ...
    private static void initFilters() {
        ...
        filters.put("AppearanceReplacer",
    ...
    private int load(String[] filterNames,
        ...
        //
        String currentArg, lvl, compMethod, exportTypeStr, parsingTypeStr;
        int[] v;
        Float maxTime;
        try {
            for (int i = 0; i < filterArgs.length; i++) {

                currentArg = filterArgs[i];
                ...
                switch (currentArg) {
                    case "-loglevel":
                        ...
                        break;
                    case "-exportVersion":
                        v = parseVersionString(filterArgs[++i]);
                    ...
                    case "-maxRunTime":
                        ...
                        //
                        maxTime = Float.valueOf(filterArgs[++i]);
        ...
        for (int i = 0; i < filterNames.length; i++) {
            filter_name = filters.get(filterNames[i]);
            ...
            filter = loadFilterClass(filter_name);

            if (filter == null) {
                System.out.println("Invalid filter: " + filter_name);
...
//            filter.setLocator();
        ...
        if (url != null) {
            ...
            try {
                is = new InputSource(inFile.getPath());
            } catch (MalformedURLException ex) {
                if (url != null)
                    System.err.println("*** CDFFilter url.getPath()=" + url.getPath()); // debug diagnostic
                System.err.println("*** CDFFilter inFile.getPath()=" + inFile.getPath()); // debug diagnostic
                System.err.flush();
                errorReporter.errorReport("Unable to create an InputSource: ", ex);
        ...
        if ((out == null && outStream == null) || (encoding == null)) {
        ...
        reader.registerImporter("application/x-ac3d",
                ...
                "ac3d");
        reader.registerImporter("application/vnd.ms-3mfdocument",
        ...
        try {
            ...
            return FilterExitCodes.ABNORMAL_CRASH;
