...
public abstract class NewAbstractFilter extends AbstractFilter {
    ...
    protected SimpleStackInterface parentTypeStack;
    ...
    *  means that the parent field of the node has ended as well */
     ...
     */
    protected NewAbstractFilter(boolean debug) {
        if (debug) {
            fieldStack = new SimpleStackLogged("Field");
            nodeStack = new SimpleStackLogged("Node");
            parentTypeStack = new SimpleStackLogged("ParentType");

            ((SimpleStackLogged)fieldStack).setDebugCalls(true);
            ((SimpleStackLogged)nodeStack).setDebugCalls(true);
        } else {
    ...
    public void startDocument(String uri,
        ...
        // TODO: Why is version hardcoded here?
        ...
        //parentTypeStack.push(FieldConstants.MFNODE);
    ...
    public void endDocument() throws SAVException, VRMLException {
        ...
        if (!nodeStack.isEmpty())
            node = (String) nodeStack.peek();

        String field = null;

        if (!fieldStack.isEmpty())
            field = (String) fieldStack.peek();

        if (node == null || !node.equals("Group")) {
            System.out.println("Node stack incorrect at end document  size: " + nodeStack.size());
        }

        if (field == null || !field.equals("children")) {
            System.out.println("Field stack incorrect at end document: " + fieldStack.size());
            if (field != null) System.out.println("   " + field);
        }

/*
System.out.println(this.getClass().getName() + " nodeStack: " + nodeStack.size() + " fieldStack: " + fieldStack.size());

System.out.println("Field history: \n");
System.out.println(fieldStack.toStringHistory());
System.out.println("Node history: \n");
System.out.println(nodeStack.toStringHistory());
*/
        nodeStack.clear();
        fieldStack.clear();
    }
    ...
    public void startNode(String name, String defName)
        ...
        try {
            ...
            if (fieldStack instanceof SimpleStackLogged) {
System.out.println("Invalid field: ");
    ...
    public void endNode() throws SAVException, VRMLException {
        ...
        try {
            nodeName = (String) nodeStack.pop();
            fieldType = (int) parentTypeStack.pop();
            fieldName = (String) fieldStack.peek();
...
System.err.println("Stack Exception: ");
        ...
        if (fieldName.equals("appearance") && fieldType != FieldConstants.SFNODE) {
            System.out.println("***Appearance not flagged as SFNode");
        }
    ...
    public void useDecl(String defName) throws SAVException, VRMLException {
        ...
        int fieldType = fieldHandler.getFieldType(nodeName, fieldName);


        // Do not pop fieldStack on MFNode its handled in endField
        if (fieldType == FieldConstants.SFNODE) {
            fieldStack.pop();
        }
    ...
    public void fieldValue(String value) throws SAVException, VRMLException {
        ...
        if(contentHandler != null && !suppressCalls) {
            ...
            fieldHandler.setFieldValue(nodeName,
                                         fieldName,
                                         value);
        }
    }
