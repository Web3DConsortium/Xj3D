...
import org.web3d.x3d.jaxp.X3DSAVAdapterTolerant;
...
class FileParserReader extends BaseReader {
     ...
     */
    private static final Set<String> standardWeb3DTypes;
    ...
    /**
     * How to style non web3d formats.  Specific to a parser
     */
    private String[] style;

    /** Should we parse strict or tolerant */
    private ParsingType parsing;
     ...
     */
    private Map<String, String> mimeToParserMap;

    /**
     * For the custom parsers, file extension to their mime type
     */
     ...
     *
     * @param style How to style non-web3d formats.
     */
    FileParserReader(String[] style) {
        this(style, ParsingType.STRICT);
    }
     ...
     *
     * @param style How to style non-web3d formats.
     * @param parsing Should we parse strict or tolerant
     */
    FileParserReader(String[] style, ParsingType parsing) {
        this.style = style;
    ...
    public void parse(InputSource input)
            throws IOException, VRMLException, FilterProcessingException {
        parse(input, null);
    }
     ...
     *
     * @param input The input source to be used
     * @param style The style or null for no styling
     * @return Null if no parsing issues or a messages detailing the issues
     * @throws IOException               An I/O error while reading the stream
     * @throws VRMLParseException        A parsing error occurred in the file
     * @throws FilterProcessingException The input file is processable
     */
    public List<String> parse(InputSource input, String[] style)
        ...
        // JC: Use our explicit filename map attempt first. It seems that some
        // versions of the JDK have the .x3d extension already mapped to
        // application/xml. THat buggers things up badly because if we
        // query the stream first, we think we have an XML mime type and
        // immediately go off to Collada land, rather than to the X3D
        // parser.
        ...
        if (realURL != null && (contentType == null || contentType.equals("content/unknown"))) {
            int dot_idx = realURL.lastIndexOf(".");
            String ext = realURL.substring(dot_idx + 1);
            contentType = extToMimeMap.get(ext);
        }

        if (standardWeb3DTypes.contains(contentType))
            return parseX3D(input, contentType);
        else {
            String classname = mimeToParserMap.get(contentType);
            ...
            return parser.parse(input, style);
        }
    }
     ...
     *
     * @param ch The node handler instance to use
     */
    @Override
     ...
     * @param extension The file extension to use for this importer
     */
    void registerImporter(String mimetype, String classname, String extension) {
    ...
    String getRegisteredImporter(String mimetype) {
        return mimeToParserMap.get(mimetype);
    }
     ...
     * @throws VRMLParseException A parsing error occurred in the file
     */
    private List<String> parseX3D(final InputSource input, String contentType)
            throws IOException, VRMLException, FilterProcessingException {
        ...
        if (contentType != null && contentType.equals("model/x3d+binary")) {
            ...
            try {
                Reader is = AccessController.doPrivileged((PrivilegedExceptionAction<Reader>) () -> input.getCharacterStream());
                pbr = new PushbackReader(is, 2);
            ...
            while (!type_found) {
                ...
                switch (ch) {
                    case '<':
                        // Xara3D is using .x3d extension so make sure it isn't that
                        int ch2 = pbr.read();
                        if (ch2 != '?') {
                            throw new VRMLParseException(0, 0, "Unsupported Format");
                        }
                        pbr.unread(ch2);
    ...
    private void parseVRMLWeakly(Reader rdr,
        ...
        if (rdr == null) {
            ...
            if ((header[1] == null) ||
                    ...
                    (!vrml_97 && !(header[1].equals("V3.0") || header[1].equals("V3.1") || header[1].equals("V3.2") || header[1].equals("V3.3") || header[1].equals("V4.0"))))
        ...
        if (vrml_97) {
            ...
            documentLocator = relaxed97.getDocumentLocator();
        }
        ...
        if (binary) {
            ...
            documentLocator = binary03.getDocumentLocator();
        } else {
            if (relaxed03 == null) {
            ...
            documentLocator = relaxed03.getDocumentLocator();
        }
        ...
        try {
        ...
        } catch (org.web3d.parser.vrml97.ParseException | org.web3d.parser.x3d.ParseException pe1) {
            Throwable cause = pe1.getCause();
            pe1.printStackTrace(System.err);
            if (cause instanceof FilterProcessingException)
                throw (FilterProcessingException) cause;
            else
                throw new VRMLParseException(documentLocator.getLineNumber(),
            ...
            tme1.printStackTrace(System.err);
            throw new VRMLParseException(documentLocator.getLineNumber(),
                    documentLocator.getColumnNumber(),
                    tme1.getMessage());
    ...
    private NonWeb3DFileParser loadParserClass(final String classname) {
        ...
        try {
            ret_val = AccessController.doPrivileged((PrivilegedExceptionAction<NonWeb3DFileParser>) () -> {
                Class<?> cls = Class.forName(classname);
                return (NonWeb3DFileParser) cls.getDeclaredConstructor().newInstance();
            });
