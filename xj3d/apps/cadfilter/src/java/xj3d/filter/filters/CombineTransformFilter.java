...
/**
 * Filter for combing transform hierarchies. The transformational components of
 * the Transforms are combined and when possible the coordinates of the children
 * nodes are transformed to retain the information. Will respect DEFed nodes to
 * indicate that a node is to be used by ROUTING or SCRIPS. Run
 * removeUnneededDEF to preprocess the file.
 *
 * Differs a fair bit from FlattenTransform. Might eventually combine them but
 * right now not certain this filter is a superset of the other.
 *
 * This filter makes use of the MatrixTransform node that some X3D browsers
 * have. If your target browser does not have that node then don't use this
 * filter. Or send me a piece of code that can convert an arbitrary 4x4 matrix
 * back into Transform fields.
 *
 * @author Alan Hudson
...
//System.out.println("containsUSE: " + containsUSE);
        ...
        for (Encodable node : enc) {
            ...
            if (node instanceof Shape) {
                shape = (Shape) node;
                def = ((BaseEncodable) node).defName;
                ...
                inTransform = ejectTransform(matrix);
                ...
                cnode = (Collision) node;
                children = cnode.getChildren();

                if (inTransform) {
                    super.startField("children");   // MatrixTransform.children
                }
                super.startNode("Collision", cnode.defName);
                super.startField("enabled");
                super.fieldValue(cnode.enabled);
                super.startField("children");
                mat = new Matrix4f();
                mat.setIdentity();
                ...
                child_matrix = transform.getMatrix();
                ...
                mTransform = (MatrixTransform) node;
                children = mTransform.getChildren();
                analyze(children, matrix, false);
            } else {
                ejectNode(node, matrix);
            }
    ...
    /**
     * Send any collected Matrix into the stream. Leaves the node open, caller
     * must call endNode.
     *
     * @param matrix The current transform stack
     */
    ...
    private void flatten(Encodable[] enc, Matrix4f matrix) {
        ...
        Point3f p, pos;
        Normal normal;
        int idx, num_vector, num_point;
        float[] point, point_xfrm, vector, vector_xfrm, vPos, o;
        Vector3f v;
        Group group;
        Encodable[] children;
        Switch snode;
        ...
        for (Encodable node : enc) {
            if (node instanceof Shape) {
                shape = (Shape) node;
                geometry = (BaseGeometry) shape.getGeometry();
                if (geometry != null) {
                    point = null;
                    coord = (Coordinate) geometry.getCoordinate();
                    if (coord != null) {
                        point = coord.point;
                        if (point != null) {
                            num_point = coord.num_point;
                            point_xfrm = new float[num_point * 3];
                            p = new Point3f();
                            idx = 0;
                    ...
                    if (geometry instanceof BaseComposedGeometry) {
                        ...
                        if (normal != null) {
                            ...
                            if (vector != null) {
                                num_vector = normal.num_vector;
                                vector_xfrm = new float[num_vector * 3];
                                v = new Vector3f();
                                idx = 0;
                ...
                mTransform = (MatrixTransform) node;
                children = mTransform.getChildren();
                child_matrix = mTransform.getMatrix();
                child_matrix.mul(matrix, child_matrix);
                flatten(children, child_matrix);
                ...
                vPos = viewpoint.position;
                pos = new Point3f();
                pos.set(vPos[0], vPos[1], vPos[2]);
    ...
    private boolean isIdentity(Matrix4f mat) {
        return mat.m00 == 1 && mat.m11 == 1 && mat.m22 == 1 && mat.m33 == 1
