...
public class TransformFilter extends NewAbstractFilter {
    ...
    private final Map<Integer, float[]> scaleMap;

    /** This hash map contains the set of all rotations we found */
    private final Map<Integer, float[]> rotateMap;

    /** This hash map contains the set of all translations we found */
    private final Map<Integer, float[]> translateMap;
    ...
    private final SimpleStack suppressStack;
    ...
    public void startNode(String name, String defName)
        ...
        if (!transformsInserted) {
            ...
            float[] scale, rotate, translate;
            for(int i=0; i < transforms; i++) {
                super.startNode("Transform", DEF_BASE+i);

                scale = scaleMap.get(i);
    ...
    public void setArguments(String[] args) {
        ...
        //
        String argument;
        int scaleIndex, rotateIndex;
        float widthScalar, heightScalar, depthScalar, x, y, z, aRadians;
        float[] scalar, rotation, translation;
        try {
            for( int i = 0; i< args.length; i++) {
                ...
                if (argument.equals("-resetTransforms")) {
                    ...
                    scaleIndex = Integer.parseInt(argument.substring(6));
                    if( scaleIndex > transforms ){
                        transforms = scaleIndex;
                    }

                    widthScalar = Float.parseFloat(args[++i]);
                    heightScalar = Float.parseFloat(args[++i]);
                    depthScalar = Float.parseFloat(args[++i]);

                    scalar = new float[]{widthScalar, heightScalar, depthScalar};
                    ...
                    rotateIndex = Integer.parseInt(argument.substring(7));
                    ...
                    z = Float.parseFloat(args[++i]);
                    aRadians = Float.parseFloat(args[++i]);

                    rotation = new float[]{x, y, z, aRadians};
