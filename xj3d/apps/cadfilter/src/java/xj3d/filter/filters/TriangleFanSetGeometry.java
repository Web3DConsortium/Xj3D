...
/**
 * Geometry holder that represents the X3D IndexedTriangleFanSet node.
 * <p>
 *
 ...
 */
class TriangleFanSetGeometry extends TriangulationGeometry  {
    ...
    void generateOutput(ContentHandler ch,
        ...
        ch.startNode("TriangleFanSet", defName);
        ch.startField("coord");
        ...
        if(ccw) {
            ...
            if(normals != null) {
                ...
                if(normalPerVertex) {
                    for(int i = 0; i < polygonCount; i++) {
                        ...
                        output_normals[output_idx] = normals[input_idx];
                        output_normals[output_idx + 1] = normals[input_idx + 1];
                        output_normals[output_idx + 2] = normals[input_idx + 2];
                        output_idx += 3;

                        for(int j = 1; j < setCount[i]; j++) {
                            output_normals[output_idx] = normals[input_idx + ((setCount[i] - j) * 3)];
                            output_normals[output_idx + 1] = normals[input_idx + (((setCount[i] - j) * 3) + 1)];
                            output_normals[output_idx + 2] = normals[input_idx + (((setCount[i] - j) * 3) + 2)];
                            output_idx += 3;
                        }

                        input_idx += setCount[i] * 3;
                    ...
                    for(int i = 0; i < polygonCount; i++) {
                        ...
                        output_normals[output_idx + 3] = normals[input_idx];
                        output_normals[output_idx + 4] = normals[input_idx + 1];
                        output_normals[output_idx + 5] = normals[input_idx + 2];

                        output_normals[output_idx + 6] = normals[input_idx];
                        output_normals[output_idx + 7] = normals[input_idx + 1];
                        output_normals[output_idx + 8] = normals[input_idx + 2];
                        ...
                        for(int j = 1; j < setCount[i]; j++) {
                            ...
                            output_normals[output_idx + 3] = normals[input_idx + ((setCount[i] - j) * 3)];
                            output_normals[output_idx + 4] = normals[input_idx + (((setCount[i] - j) * 3) + 1)];
                            output_normals[output_idx + 5] = normals[input_idx + (((setCount[i] - j) * 3) + 2)];

                            output_normals[output_idx + 6] = normals[input_idx + ((setCount[i] - j) * 3)];
                            output_normals[output_idx + 7] = normals[input_idx + (((setCount[i] - j) * 3) + 1)];
                            output_normals[output_idx + 8] = normals[input_idx + (((setCount[i] - j) * 3) + 2)];
            ...
            if(colors != null) {
                ...
                for(int i = 0; i < polygonCount; i++) {
                    ...
                    output_colors[output_idx] = colors[input_idx];
                    output_colors[output_idx + 1] = colors[input_idx + 1];
                    output_colors[output_idx + 2] = colors[input_idx + 2];
                    output_idx += 3;

                    for(int j = 1; j < setCount[i]; j++) {
                        output_colors[output_idx] = colors[input_idx + ((setCount[i] - j) * 3)];
                        output_colors[output_idx + 1] = colors[input_idx + (((setCount[i] - j) * 3) + 1)];
                        output_colors[output_idx + 2] = colors[input_idx + (((setCount[i] - j) * 3) + 2)];
                        output_idx += 3;
                    }
            ...
            if(texCoords != null) {
                ...
                for(int i = 0; i < polygonCount; i++) {
                    ...
                    output_texcoords[output_idx] = texCoords[input_idx];
                    output_texcoords[output_idx + 1] = texCoords[input_idx + 1];
                    output_idx += 2;

                    for(int j = 1; j < setCount[i]; j++) {
                        output_texcoords[output_idx] = texCoords[input_idx + ((setCount[i] - j) * 2)];
                        output_texcoords[output_idx + 1] = texCoords[input_idx + (((setCount[i] - j) * 2) + 1)];
                        output_idx += 2;
                    }

                    input_idx += setCount[i] * 2;
                }
            }
        } else {
            ...
            for(int i = 0; i < polygonCount; i++) {
                ...
                output_coords[output_idx] = coordinates[input_idx];
                output_coords[output_idx + 1] = coordinates[input_idx + 1];
                output_coords[output_idx + 2] = coordinates[input_idx + 2];

                output_idx += 3;

                for(int j = 1; j < setCount[i]; j++) {
                    output_coords[output_idx] = coordinates[input_idx + ((setCount[i] - j) * 3)];
                    output_coords[output_idx + 1] = coordinates[input_idx + (((setCount[i] - j) * 3) + 1)];
                    output_coords[output_idx + 2] = coordinates[input_idx + (((setCount[i] - j) * 3) + 2)];
                    output_idx += 3;
                }
