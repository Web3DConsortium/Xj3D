...
/**
 * Geometry holder that represents the X3D TriangleSet node.
 * <p>
 *
 ...
 */
class TriangleSetGeometry extends TriangulationGeometry  {
    ...
    /** The number of valid coordinates in the coordinates array */
    ...
    void generateOutput(ContentHandler ch,
        ...
        if(ccw) {
            ...
            output_idx = numCoords;

            if(normals != null) {
                output_normals = new float[numCoords * 3];

                if(normalPerVertex) {
                    for(int i = 0; i < (poly_count / 3); i++) {

                        output_normals[output_idx] = normals[i * 9];
                        output_normals[output_idx + 1] = normals[(i * 9) + 1];
                        output_normals[output_idx + 2] = normals[(i * 9) + 2];

                        output_normals[output_idx + 3] = normals[(i * 9) + 3];
                        output_normals[output_idx + 4] = normals[(i * 9) + 4];
                        output_normals[output_idx + 5] = normals[(i * 9) + 5];

                        output_normals[output_idx + 6] = normals[(i * 9) + 6];
                        output_normals[output_idx + 7] = normals[(i * 9) + 7];
                        output_normals[output_idx + 8] = normals[(i * 9) + 8];
                        output_idx += 9;
                    }
                } else {
                    for(int i = 0; i < (poly_count / 3); i++) {

                        output_normals[output_idx] = normals[i * 3];
                        output_normals[output_idx + 1] = normals[i * 3 + 1];
                        output_normals[output_idx + 2] = normals[i * 3 + 2];

                        output_normals[output_idx + 3] = normals[i * 3];
                        output_normals[output_idx + 4] = normals[i * 3 + 1];
                        output_normals[output_idx + 5] = normals[i * 3 + 2];

                        output_normals[output_idx + 6] = normals[i * 3];
                        output_normals[output_idx + 7] = normals[i * 3 + 1];
                        output_normals[output_idx + 8] = normals[i * 3 + 2];
            ...
            if(colors != null) {
                output_colors = new float[numCoords * 3];

                for(int i = 0; i < (poly_count / 3); i++) {

                    output_colors[output_idx] = colors[i * 9];
                    output_colors[output_idx + 1] = colors[(i * 9) + 1];
                    output_colors[output_idx + 2] = colors[(i * 9) + 2];

                    output_colors[output_idx + 3] = colors[(i * 9) + 3];
                    output_colors[output_idx + 4] = colors[(i * 9) + 4];
                    output_colors[output_idx + 5] = colors[(i * 9) + 5];

                    output_colors[output_idx + 6] = colors[(i * 9) + 6];
                    output_colors[output_idx + 7] = colors[(i * 9) + 7];
                    output_colors[output_idx + 8] = colors[(i * 9) + 8];
                    output_idx += 9;
                }
            }

            if(texCoords != null) {
                output_texcoords = new float[numCoords * 2];

                for(int i = 0; i < (poly_count / 3); i++) {

                    output_texcoords[output_idx] = texCoords[i * 6];
                    output_texcoords[output_idx + 1] = texCoords[(i * 6) + 1];

                    output_texcoords[output_idx + 2] = texCoords[(i * 6) + 2];
                    output_texcoords[output_idx + 3] = texCoords[(i * 6) + 3];

                    output_texcoords[output_idx + 4] = texCoords[(i * 6) + 4];
                    output_texcoords[output_idx + 5] = texCoords[(i * 6) + 5];
                    output_idx += 6;
                }
            }
        } else {
            // Not CCW so reverse everything.
            output_coords = new float[numCoords];

            for(int i = 0; i < (poly_count / 3); i++) {

                output_coords[output_idx] = coordinates[i * 9];
                output_coords[output_idx + 1] = coordinates[i * 9 + 1];
                output_coords[output_idx + 2] = coordinates[i * 9 + 2];

                output_coords[output_idx + 3] = coordinates[(i * 9) + 6];
                output_coords[output_idx + 4] = coordinates[(i * 9) + 7];
                output_coords[output_idx + 5] = coordinates[(i * 9) + 8];

                output_coords[output_idx + 6] = coordinates[(i * 9) + 3];
                output_coords[output_idx + 7] = coordinates[(i * 9) + 4];
                output_coords[output_idx + 8] = coordinates[(i * 9) + 5];
                output_idx += 9;
            }
            ...
            if(normals != null) {
                ...
                if(normalPerVertex) {
                    for(int i = 0; i < (poly_count / 3); i++) {
                        ...
                        output_normals[output_idx + 3] = normals[(i * 9) + 6];
                        output_normals[output_idx + 4] = normals[(i * 9) + 7];
                        output_normals[output_idx + 5] = normals[(i * 9) + 8];

                        output_normals[output_idx + 6] = normals[(i * 9) + 3];
                        output_normals[output_idx + 7] = normals[(i * 9) + 4];
                        output_normals[output_idx + 8] = normals[(i * 9) + 5];
                        output_idx += 9;
                    }
                } else {
                    for(int i = 0; i < (poly_count / 3); i++) {
            ...
            if(colors != null) {
                ...
                for(int i = 0; i < (poly_count / 3); i++) {
                    ...
                    output_colors[output_idx + 3] = colors[(i * 9) + 6];
                    output_colors[output_idx + 4] = colors[(i * 9) + 7];
                    output_colors[output_idx + 5] = colors[(i * 9) + 8];

                    output_colors[output_idx + 6] = colors[(i * 9) + 3];
                    output_colors[output_idx + 7] = colors[(i * 9) + 4];
                    output_colors[output_idx + 8] = colors[(i * 9) + 5];
                    output_idx += 9;
                }
            }
            ...
            if(texCoords != null) {
                ...
                for(int i = 0; i < (poly_count / 3); i++) {
                    ...
                    output_texcoords[output_idx + 2] = texCoords[(i * 6) + 4];
                    output_texcoords[output_idx + 3] = texCoords[(i * 6) + 5];

                    output_texcoords[output_idx + 4] = texCoords[(i * 6) + 2];
                    output_texcoords[output_idx + 5] = texCoords[(i * 6) + 3];
                    output_idx += 6;
                }
            }
        }
