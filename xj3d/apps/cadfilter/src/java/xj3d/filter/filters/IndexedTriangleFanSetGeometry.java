 ...
 */
class IndexedTriangleFanSetGeometry extends TriangulationGeometry  {
    ...
    void generateOutput(ContentHandler ch,
        ...
        ch.startNode("IndexedTriangleFanSet", defName);
        ch.startField("coord");
        ...
        if(ccw) {
            ...
            for(int i = 0; i < polygonCount; i++) {
                ...
                for(int j = 1; j < rawVerticesPerFace[i]; j++)
                    output_indices[output_idx++] =
                        coordIndices[input_idx + rawVerticesPerFace[i] - j];
