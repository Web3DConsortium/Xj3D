...
public class USERedundantNodeFilter extends CommonEncodedBaseFilter {
    ...
    public void setArguments(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (arg.equals(NODE_NAME)) {
                ...
                results = nodeNames.split("\\s|[,]");

                num_node = results.length;
    ...
    private void filter() {
        ...
        int num_nodes, rem_nodes, last_idx, num_ignore, x, d_idx;
        int[][] dup;
        int[] num_dup, ignore;
        String name, def_name_base, def_name;
        List<CommonEncodable> node_list;
        CommonEncodable src, tgt;
        boolean skip, isAppearance;
        for (int n = 0; n < num_list; n++) {
            num_nodes = 0;
            rem_nodes = 0;
            name = nodeName.get(n);
            node_list = node_list_list.get(n);
            if (node_list != null) {
                num_nodes = node_list.size();
                rem_nodes = num_nodes;
                if (num_nodes > 1) {
                    last_idx = num_nodes - 1;
                    dup = new int[last_idx][];
                    num_dup = new int[last_idx];
                    ignore = new int[num_nodes];
                    num_ignore = 0;
                                ...
                                if (!skip) {
                                    ...
                                    if ((tgt.getDefName() == null) && (tgt.getUseName() == null)) {
                                        if (src.deepEquals(tgt)) {
                                            isAppearance = name.equals("Appearance");
                                            if (isAppearance) {
                    ...
                    def_name_base = name.toUpperCase() + "_";
                    def_inc = 0;
                    for (int i = 0; i < last_idx; i++) {
                        if (num_dup[i] > 0) {
                            rem_nodes -= num_dup[i];
                            def_name = getDefName(def_name_base);
