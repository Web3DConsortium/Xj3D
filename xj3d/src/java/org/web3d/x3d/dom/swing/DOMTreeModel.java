...
public class DOMTreeModel implements TreeModel, EventListener {
    ...
    public DOMTreeModel(Node root) {
        ...
        if(hasDOMEvents) {
            ...
            tgt.addEventListener("DOMNodeInserted", DOMTreeModel.this, false);
            tgt.addEventListener("DOMNodeRemoved", DOMTreeModel.this, false);
            tgt.addEventListener("DOMAttrModified", DOMTreeModel.this, false);
        }
    }
    ...
    private TreePath buildTreePath(Node dest) {
        List<Node> rev_list = new ArrayList<>();
        ...
        for(i = 0; i < size; i++) {
            node_path[i] = rev_list.get(size - i - 1);
        }
    ...
    private void fireTreeNodesChanged(TreeModelEvent evt) {
        Iterator<TreeModelListener> itr = listeners.iterator();
        TreeModelListener l;
