 ...
 * The initialization process should provide an application name as a
 ...
 */
public class I18nManagerMultiResource {
    ...
    private static I18nManagerMultiResource instance;
     ...
     */
    private I18nManagerMultiResource() {
        Preferences prefs = Preferences.userNodeForPackage(I18nManagerMultiResource.class);
     ...
     * given root class.
     * @return the global instance of the internationalisation manager for the
     * given root class
     */
    public static I18nManagerMultiResource getManager() {

        if(instance == null)
            instance = new I18nManagerMultiResource();
     ...
     */
    public void setApplication(String appName, String[] resourceFiles) {

        Preferences main_prefs =
            Preferences.userNodeForPackage(I18nManagerMultiResource.class);
        ...
        if (resourceFiles != null && resourceFiles.length > 0) {
            ...
            resourceFileNames = new String[resourceFiles.length];
            stringResources = new ResourceBundle[resourceFiles.length];

            for (int i = 0; i < resourceFiles.length; i++) {
                    resourceFileNames[i] = resourceFiles[i];
                    stringResources[i] = ResourceBundle.getBundle(resourceFiles[i], locale);
            }
        }
    }

    /**
     * Get the currently set application string that language settings are
    ...
    public void changeLocale(String language, String country, String variant)
        ...
        if (stringResources != null) {
        	for (int i = 0; i < stringResources.length; i++) {
                stringResources[i] = ResourceBundle.getBundle(resourceFileNames[i], locale);
        	}
        }
    ...
    public Locale getFoundLocale() {
        return stringResources[0].getLocale();
    }
     ...
     * @param property the localised string for the given property name
     * @return
     */
    public String getString(String property) {
    	String val = null;

        for (ResourceBundle stringResource : stringResources) {
            try {
                val = stringResource.getString(property);
            }catch (MissingResourceException mre) {
                // Ignore and move on to next resource bundle
            }
        }
