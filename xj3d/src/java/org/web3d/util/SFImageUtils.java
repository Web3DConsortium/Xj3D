...
public class SFImageUtils {
    ...
    /**
     * Utility method for MFImageWrapper and SFImageWrapper to convert a
     * RenderedImage to the SFImage integer format
     *
            ...
            case 3:
                ...
                for (int y = height - 1; y >= 0; y--) {
                    for (int x = 0; x < width; x++) {
                        ...
                        if (pixel_index < targetArray.length)
    ...
    /**
     * Perform a bitBlt from a source image to an SFImage field. Does not do
     * wrapping.
     *
    ...
    public static void convertSubRenderedImageToData(
        ...
        switch (components) {
            case 1:
                ...
                int image_index, pixel_index;
                for (int y = destYOffset; y < destYOffset + srcHeight; y++) {
                    image_index = img_y_inv * imgWidth + srcXOffset;
                    pixel_index = y * destinationWidth + destXOffset + pixel_offset;
    ...
    /**
     * Utility method to convert SFImage data representation to a
	...
	// identify the image type by number of components and setup
        ...
        int upper_row_offset, lower_row_offset, upper_offset, lower_offset, upper_pixel;
        for (int y = 0; y < iterations; y++) {
            upper_row_offset = y * width;
            lower_row_offset = lower_row * width;
            for (int x = 0; x < width; x++) {
                // transpose the rows, a pair at an iteration
                upper_offset = upper_row_offset + x;
                lower_offset = lower_row_offset + x;
                upper_pixel = pixels[upper_offset];
