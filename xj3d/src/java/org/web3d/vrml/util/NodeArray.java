...
public class NodeArray {
    ...
    public NodeArray(int initialSize) {
        array = new VRMLNode[initialSize];
        valueCount = 0;
    }
    ...
    public void add(VRMLNode newNode) {
        ...
        if(valueCount == array.length) {
            VRMLNode[] newArray =
                new VRMLNode[array.length + INCREMENT_SIZE];
    ...
    public void add(VRMLNode[] values) {
        ...
        if(req_size >= array.length) {
            VRMLNode[] newArray = new VRMLNode[req_size];
            System.arraycopy(array, 0, newArray, 0, array.length);
            array = newArray;
        }
     ...
     */
    public void add(VRMLNode[] values, int offset, int len) {
     ...
     * @throws IndexOutOfBoundsException The index was not legal
     */
    public void set(int index, VRMLNode value) {
        if((index < 0) || (index >= valueCount))
            throw new IndexOutOfBoundsException();
    ...
    /**
     * Find the index of the given node in this list. If the node is not in the
     * list a value of -1 will be returned. The comparison uses the .equals()
    ...
    public VRMLNode[] toArray() {
        VRMLNode[] ret_val = new VRMLNode[valueCount];
    ...
    public VRMLNode[] toArray(VRMLNode[] values) {
        ...
        if(values.length >= valueCount)
            ...
            ret_val = new VRMLNode[valueCount];
