...
public class OGLTransform extends BaseTransform
    ...
    public void updateNodeBoundsChanges(Object src) {
        ...
        for(int i = 0; i < remSize; i++) {
            kid = removedChildren.remove(0);
            implGroup.removeChild(kid);
        }

        for(int i = 0; i < addSize; i++) {
            kid = addedChildren.remove(0);

            if (kid == null) {continue;}

            // NOTE: Here is the missing call to get ecmascript functions to
            // finally work (tdn)
            if (kid.getParent() != null) {
                ((Group)kid.getParent()).removeChild(kid);
            }
    ...
    public void setupFinished() {
        ...
        for(int i = 0; i < childCount; i++) {
            ...
            if(ogl_node != null) {
                ...
                // As in line 188 above, if this node has a parent, then prevent
                // an org.j3d.aviatrix3d.AlreadyParentedException
                if (ogl_node.getParent() != null) {
                    ((Group)ogl_node.getParent()).removeChild(ogl_node);
                }
    ...
    protected void removeChildNode(VRMLNodeType node) {
        ...
        // Check to see if it is a sensor node and therefore needs to be
