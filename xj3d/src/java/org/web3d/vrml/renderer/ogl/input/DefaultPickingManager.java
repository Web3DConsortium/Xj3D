...
public class DefaultPickingManager
    ...
    private static final int ALL_BITMASK = 0xFFFF_FFFF;
    ...
    private static final int EXTENDED_BITMASK = 0x7FFF_FFFF;
    ...
    private List<SceneGraphPath> selectionArray;
    ...
    public void loadScene(BasicScene scene) {
        ...
        for(VRMLNode node : nodes) {
            sensorList.add(node);
            ((OGLPickingSensorNodeType)node).setTypeConvertor(this);
        }

        nodes = scene.getBySecondaryType(TypeConstants.PickTargetNodeType);

        for(VRMLNode node : nodes) {
            ((OGLPickableTargetNodeType)node).setTypeConvertor(this);
        }
    }
    ...
    public int addObjectType(String type) {
        ...
        if(mask != null) {
            ...
            bitmaskCount.put(type, count + 1);

            ret_val = mask;
        } else if(availableBitmasks.size() == 0) {
            assignedBitmasks.put(type, EXTENDED_BITMASK);
            bitmaskCount.put(type, 1);

            ret_val = EXTENDED_BITMASK;
        } else {
            ret_val = availableBitmasks.remove(0);
            assignedBitmasks.put(type, ret_val);
            bitmaskCount.put(type, 1);
        }
    ...
    /**
     * Process a LineSet or IndexedLineSet option.
     *
     * @param geom The node to do the picking with
     * @param targets The items to pick against
     */
    ...
    /**
     * Process a LineSet picking option.
     *
     * @param geom The node to do the picking with
     * @param targets The items to pick against
     */
    private void processLineSetPicking(int intersectType,
        ...
        int[] index_list = field_data.intArrayValues;
            ...
            for(int j = 1; !search_complete && j < index_list[i]; j++) {
                ...
                for(int k = 0; !search_complete && k < targets.length; k++) {
                    ...
                    if(singlePicker.foundPaths instanceof List)
                        ...
                        selectionArray.add((SceneGraphPath) singlePicker.foundPaths);
                    }
    ...
    private void processIndexedLineSetPicking(int intersectType,
        ...
        List<SceneGraphPath> potentials;
        ...
        for(int i = 0; i < num_index - 1; i++) {
            ...
            for (PickableObject target : targets) {
                target.pickSingle(singlePicker);
                if(singlePicker.pickCount == 0)
                    continue;
                if(singlePicker.foundPaths instanceof List)
                    potentials = (List<SceneGraphPath>) singlePicker.foundPaths;
                else {
                    potentials = selectionArray;
                    selectionArray.clear();
                    selectionArray.add((SceneGraphPath) singlePicker.foundPaths);
                ...
                if(intersectType != VRMLPickingSensorNodeType.INTERSECT_BOUNDS) {
                    ...
                    for(int j = 0; j < size; j++) {
                        SceneGraphPath path = potentials.get(j);
                        int len = path.getNodeCount();
                        terminate = false;
    ...
    private void processBoxPicking(int intersectType,
        ...
        VRMLFieldData field_data = geom.getFieldValue(field_idx);
        float x = field_data.floatArrayValues[0] * 0.5f;
        float y = field_data.floatArrayValues[1] * 0.5f;
        float z = field_data.floatArrayValues[2] * 0.5f;
    ...
    private void calculatePickMatrix(OGLPickingSensorNodeType sensor) {
        ...
        while(parent != null) {
            if(parent instanceof TransformGroup) {
                ...
                ((TransformPickTarget)parent).getTransform(matrixStack[last_matrix]);
                last_matrix++;
            }
