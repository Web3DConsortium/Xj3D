...
class ViewportResizeManager
    ...
    void sendResizeUpdates() {
        ...
        for (Viewport vp : fullscreenViewports) {
            vp.setDimensions(viewX, viewY, viewWidth, viewHeight);
        }
        for (FixedViewportData data : fixedViewports) {
            processViewportChange(data);
        }
        for (ProportionalViewportData data : proportionalViewports) {
            processViewportChange(data);
        }
        for (CustomViewportData data : customViewports) {
            processViewportChange(data);
        }
