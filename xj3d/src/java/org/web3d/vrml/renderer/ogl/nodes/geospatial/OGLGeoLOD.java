...
public class OGLGeoLOD extends BaseGeoLOD implements
     ...
     */
    protected List<VRMLNode> childExternalList;
    ...
    public void allEventsComplete() {
        ...
        if (levelChanged) {
            ...
            for (VRMLNode childExternalList1 : childExternalList) {
                VRMLSingleExternalNodeType node = (VRMLSingleExternalNodeType) childExternalList1;
...
//System.out.println("visibilityStateChange: "+ position +", "+ visible);
    ...
    public void viewPositionChanged(Point3f position,
        ...
        if (inside != childrenShown) {
            ...
            if (childrenShown) {
                ...
                for (VRMLNode externalChild : childExternalList) {
                    VRMLSingleExternalNodeType node = (VRMLSingleExternalNodeType) externalChild;
                    if (node != null) {
    ...
    private void init() {
        ...
        childExternalList = new ArrayList<>();
    }
    ...
    private void getExternalNodeTypes(VRMLScene scene) {
        childExternalList.addAll(scene.getBySecondaryType(TypeConstants.SingleExternalNodeType));
    }
}
