...
public class OGLLineSet extends BaseLineSet
    ...
    public void updateNodeDataChanges(Object src) {
        if(vfColor == null) {
            ...
            boolean isRGBA = (vfColor.getNumColorComponents() == 4); // otherwise RGB
            ...
            if (num_points * vfColor.getNumColorComponents() == num_colors) {
                implGeom.setColors(isRGBA, tmpColors);
            } else {
                // TODO: What to do here.  Crashes currently on 0 length
                // Don't want to issue a warning each time
                if (num_colors != 0) {
                    errorReporter.warningReport("OGLLineSet has invalid number of color values.  Expected: " + 
							num_points + "*" + vfColor.getNumColorComponents() + " = " + (num_points * vfColor.getNumColorComponents()) +
                                                " but found: " + num_colors, null);
                }
            }
        }
    }
