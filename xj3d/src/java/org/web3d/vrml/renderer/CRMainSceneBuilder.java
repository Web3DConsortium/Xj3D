...
public abstract class CRMainSceneBuilder
     ...
     * should not be (e.g. proto callbacks)
     */
    ...
    protected DocumentLocator locator;
    ...
    /** Counter when we discover a StaticGroup node */
    ...
    private boolean isVrmlVersion2;
    private boolean isVrmlVersion3;
    private boolean isVrmlVersion4;
    ...
    public void startDocument(String uri,
        ...
        if (type.charAt(1) == 'V')
        {
            ...
            isVrmlVersion2 =  (version.charAt(0) == '2') || (version.charAt(1) == '2');
            isVrmlVersion3 =  (version.charAt(0) == '3') || (version.charAt(1) == '3');
            isVrmlVersion4 =  (version.charAt(0) == '4') || (version.charAt(1) == '4');

            if      (isVrmlVersion2)
                major = 2;
            else if (isVrmlVersion3)
                major = 3;
            else if (isVrmlVersion4)
                major = 4;
            else System.err.println ("[CRMainSceneBuilder] incorrect VRML major version number: " + version);
            ...
            metaData = new WriteableSceneMetaData(isVrmlVersion2 ? "2.0" : "1.0",
                                                  true,
                                                  real_enc);
        ...
        else // XML
        {
            ...
            // The version string is everything after #X3D. TODO check, both VRML and X3D have utf8 encoding
            real_enc = encoding.equals("utf8") ?
                       SceneMetaData.VRML_ENCODING : // 2
                       SceneMetaData.XML_ENCODING;   // 3
            ...
            if (maj_num.substring(0,1).equalsIgnoreCase("V")) {
...
//          fieldParser.setCaseSensitive(true); // TODO this seems wrong, mixup with utf8
            fieldParser.setCaseSensitive(false);
    ...
    public void endDocument() throws SAVException, VRMLException {
        // Tell the scene graph to go build itself now. This will be a
    ...
    private boolean reportBaseIssue = true;
    @Override
    public void startField(String name) throws SAVException, VRMLException {
        ...
        // TODO indicate how to fix this
        if (index == -1)
        {
            badFieldName          = true;
            boolean criticalError = true;
            String xj3dIssue      = new String();
            if  (name.equalsIgnoreCase("retainUserOffsets") ||
                 name.equalsIgnoreCase("id")      || // same as DEF
                 name.equalsIgnoreCase("class")   || name.equalsIgnoreCase("style") ||
                 name.equalsIgnoreCase("shadows") || name.equalsIgnoreCase("visible") ||
                 name.equalsIgnoreCase("ambientIntensity") )
            {
                // for valid content, likely additional field in X3D4; report encoding and version to aid diagnosis
                if (isVrmlVersion2)
                         xj3dIssue = "(VRML97)";
                else if (isVrmlVersion3)
                         xj3dIssue = "(ClassicVrml 3)";
                else if (isVrmlVersion4)
                         xj3dIssue = "(ClassicVrml 4)";
                else     xj3dIssue = "(X3D " + majorVersion + "." + minorVersion + ")";
            ...
            String errorMessage =   "[CRMainSceneBuilder] Unknown field name '" + 
                                    ...
                                    "' (found in X3D model on line " + locator.getLineNumber() +
                                    " column " + locator.getColumnNumber() + ")";
            System.err.println(errorMessage);
            if (reportBaseIssue)
            {
                System.err.println(xj3dIssue);
    ...
    public void useDecl(String defName) throws SAVException, VRMLException {
        ...
        if(!inStatic && staticNodeSet.contains(defName))
            throw new InvalidFieldValueException(
                " Line: " + locator.getLineNumber() +
                " Column: " + locator.getColumnNumber() +
                " Cannot USE a node that is under a StaticGroup. \"" +
                defName + '\"');
