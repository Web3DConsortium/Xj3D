...
public abstract class CRExternProtoBuilder
    ...
    private final Map<String, VRMLNodeType> defMap;

    /** The mapping of proto names (key) to node instances (value) */
    private final Map<String, PrototypeDecl> protoMap;

    /** The mapping of externproto names (key) to node instances (value) */
    private final Map<String, ExternalPrototypeDecl> externProtoMap;

    /** The working stack of currently defined PROTO declarations */
    private final SimpleStack protoDeclStack;

    /** The working stack of currently building scripts. */
    private final SimpleStack scriptStack;

    /** The working stack of def maps */
    private final SimpleStack defMapStack;

    /** The working stack of proto decls maps */
    private final SimpleStack protoMapStack;
     ...
     * default factory for non-renderable nodes.
     *
     * @param fac
    ...
    public void externProtoURI(String[] values) throws SAVException, VRMLException {
        ...
        ((VRMLSingleExternalNodeType)currentProto).setUrl(values, values.length);
        ((VRMLExternalNodeType)currentProto).setWorldUrl(worldURL);
        currentProto = null;
    }
