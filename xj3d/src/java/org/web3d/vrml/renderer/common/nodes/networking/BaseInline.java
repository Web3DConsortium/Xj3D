...
public class BaseInline extends AbstractNode
    ...
    /** Index of the Bounding box size bboxDisplay field */
    protected static final int FIELD_BBOX_DISPLAY = LAST_NODE_INDEX + 4;
    
    /** Index of the Bounding box size visible field */
    protected static final int FIELD_VISIBLE = LAST_NODE_INDEX + 5;

    /** Index of the Load field */
    protected static final int FIELD_LOAD = LAST_NODE_INDEX + 6;
    ...
    public BaseInline(VRMLNodeType node) {
        ...
        try {
            ...
            vfBboxCenter[0] = field.floatArrayValues[0];
            vfBboxCenter[1] = field.floatArrayValues[1];
            vfBboxCenter[2] = field.floatArrayValues[2];

            index = node.getFieldIndex("bboxSize");
            field = node.getFieldValue(index);
            vfBboxSize[0] = field.floatArrayValues[0];
            vfBboxSize[1] = field.floatArrayValues[1];
            vfBboxSize[2] = field.floatArrayValues[2];
            
            index = node.getFieldIndex("bboxDisplay");
            field = node.getFieldValue(index);
            vfBboxDisplay = field.booleanValue;
            
            index = node.getFieldIndex("visible");
            field = node.getFieldValue(index);
            vfVisible = field.booleanValue;
     ...
     * use this to walk back up the scene graph because of protos.
     */
    @Override
...
// as clearing the import mappings.
     ...
     */
    @Override
    public void setBboxCenter(float[] newBboxCenter) throws InvalidFieldAccessException {
     ...
     */
    @Override
    public void setBboxSize(float[] newBboxSize) throws InvalidFieldAccessException {
        if(!inSetup)
    ...
    /** Convenience method to set a new value for the vfBoxDisplay field.
     * 
     * @param newBboxDisplay the new display for the bounding box
     */
    public void setBboxDisplay(boolean newBboxDisplay) {
        if(!inSetup)
            throw new InvalidFieldAccessException("bboxDisplay is initializeOnly");
        
        vfBboxDisplay = newBboxDisplay;
    }
