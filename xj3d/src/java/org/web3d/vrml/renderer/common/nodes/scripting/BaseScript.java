...
public abstract class BaseScript extends AbstractDynamicFieldNode
     ...
     * that.
     */
    private IntHashMap<Object> fieldValueMap;

    /** The list of sizes for each field */
    private IntHashMap<Integer> fieldSizeMap;

    /** Map of whether the field index has been changed since last check */
    private IntHashMap<Boolean> fieldChangedMap;
    ...
    /** Flags for the local, predefined fields */
    private boolean[] hasChanged;

    /** Change time for the local, predefined fields */
    ...
    static {
        ...
        for (String VRML97_MIME_TYPES1 : VRML97_MIME_TYPES) {
            validVrml97Types.add(VRML97_MIME_TYPES1);
        }

        validX3DTypes = new HashSet<>();

        for (String X3D_MIME_TYPES1 : X3D_MIME_TYPES) {
            validX3DTypes.add(X3D_MIME_TYPES1);
        }
...
System.err.println(getClass().getName() + ".notifyExternProtoLoaded() not yet implemented, ignored" );
     }
    ...
    public void initialize(double timestamp) {
        try {
            ...
            if (className.equals("org.mozilla.javascript.EcmaError")) { // Rhino
                String msg;
                ...
                // output entire script into console to assist author debug efforts
                try {
    ...
    public void setupFinished() {
        ...
        for(int i = 0; i < num_fields; i++) {
            ...
            if(value instanceof VRMLNodeType) {
                ...
                int num_nodes = fieldSizeMap.get(i);
    ...
    public boolean hasFieldChanged(int index) {
        ...
        boolean ret_val;
        Boolean bool = fieldChangedMap.get(index);
        ret_val = bool;
    ...
    public void sendRoute(double time,
        ...
        Integer size = fieldSizeMap.get(srcIndex);

        try {
            sendValue(destNode, destIndex, value, size);
    ...
    public void setValue(int index, int[] value, int numValid)
        ...
        fieldSizeMap.put(index, numValid);
    ...
    public void setValue(int index, VRMLNodeType child)
        ...
        if(inSetup && decl.getFieldType() == FieldConstants.MFNODE) {
            ...
            if(cur_node == null) {
                ...
                fieldSizeMap.put(index, 2);
            } else {
                // already existing array so just elongate
                Integer cs = fieldSizeMap.get(index);
                int cur_size = cs;
                ...
                fieldSizeMap.put(index, cur_size + 1);
            }
        } else {
    ...
    public final int appendField(VRMLFieldDeclaration field)
        ...
        switch(field.getFieldType()) {
            case FieldConstants.SFINT32:
                fieldValueMap.put(field_index, 0);
            ...
            case FieldConstants.SFFLOAT:
                fieldValueMap.put(field_index, (float) 0);
                fieldSizeMap.put(field_index, SINGLE_SIZE);
                break;

            case FieldConstants.SFDOUBLE:
                fieldValueMap.put(field_index, (double) 0);
                fieldSizeMap.put(field_index, SINGLE_SIZE);
                break;

            case FieldConstants.SFLONG:
                fieldValueMap.put(field_index, (long) 0);
                fieldSizeMap.put(field_index, SINGLE_SIZE);
                break;
...
// defined, even if it is a default.
    ...
    private void sendValue(VRMLNodeType destNode,
        ...
        switch(decl.getFieldType()) {
            case FieldConstants.SFINT32:
                destNode.setValue(index, ((Integer) value));
                break;
            ...
            case FieldConstants.SFFLOAT:
                destNode.setValue(index, ((Float) value));
                break;
            ...
            case FieldConstants.SFDOUBLE:
                destNode.setValue(index, ((Double) value));
                break;
            ...
            case FieldConstants.SFLONG:
                destNode.setValue(index, ((Long) value));
                break;
            ...
            case FieldConstants.SFBOOL:
                destNode.setValue(index, ((Boolean)value));
                break;
    ...
    private boolean fetchValue(int index) throws InvalidFieldException {
        ...
        Integer i_size = fieldSizeMap.get(index);
        int num_elements = i_size == null ? 0 : i_size;

        switch(decl.getFieldType()) {
            case FieldConstants.SFINT32:
                fieldData.intValue = ((Integer) value);
                fieldData.dataType = VRMLFieldData.INT_DATA;
                break;

            case FieldConstants.MFINT32:
                fieldData.intArrayValues = (int[])value;
                fieldData.numElements = num_elements;
                fieldData.dataType = VRMLFieldData.INT_ARRAY_DATA;
                break;

            case FieldConstants.SFFLOAT:
                fieldData.floatValue = ((Float) value);
                fieldData.dataType = VRMLFieldData.FLOAT_DATA;
                break;

            case FieldConstants.MFFLOAT:
                fieldData.floatArrayValues = (float[])value;
                fieldData.numElements = num_elements;
                fieldData.dataType = VRMLFieldData.FLOAT_ARRAY_DATA;
                break;

            case FieldConstants.SFDOUBLE:
                fieldData.doubleValue = ((Double) value);
                fieldData.dataType = VRMLFieldData.DOUBLE_DATA;
                break;

            case FieldConstants.MFDOUBLE:
                fieldData.doubleArrayValues = (double[])value;
                fieldData.numElements = num_elements;
                fieldData.dataType = VRMLFieldData.DOUBLE_ARRAY_DATA;
                break;

            case FieldConstants.SFLONG:
                fieldData.longValue = ((Long) value);
                fieldData.dataType = VRMLFieldData.LONG_DATA;
                break;

            case FieldConstants.MFLONG:
                fieldData.longArrayValues = (long[])value;
                fieldData.numElements = num_elements;
                fieldData.dataType = VRMLFieldData.LONG_ARRAY_DATA;
                break;

            case FieldConstants.SFBOOL:
                fieldData.booleanValue = ((Boolean)value);
                fieldData.dataType = VRMLFieldData.BOOLEAN_DATA;
                break;

            case FieldConstants.MFBOOL:
                fieldData.booleanArrayValues = (boolean[])value;
                fieldData.numElements = num_elements;
            ...
            case FieldConstants.MFSTRING:
                if (value instanceof String[]) {
                    fieldData.stringArrayValues = (String[])value;
                    fieldData.numElements = num_elements;
                    fieldData.dataType = VRMLFieldData.STRING_ARRAY_DATA;
                }
                else {
            ...
            case FieldConstants.SFNODE:
                fieldData.nodeValue = (VRMLNode)value;
                fieldData.dataType = VRMLFieldData.NODE_DATA;
                break;
            ...
            case FieldConstants.MFNODE:
                ...
                if(value instanceof VRMLNodeType) {
                    fieldData.nodeArrayValues = new VRMLNodeType[1];
                    fieldData.nodeArrayValues[0] = (VRMLNodeType)value;
                } else {
                    fieldData.nodeArrayValues = (VRMLNodeType[])value;
                }
                fieldData.numElements = num_elements;
            ...
            case FieldConstants.SFVEC2F:
                fieldData.floatArrayValues = (float[])value;
                if((value == null) || fieldData.floatArrayValues.length < 2)
            ...
            case FieldConstants.SFVEC3F:
                fieldData.floatArrayValues = (float[])value;
                if((value == null) || fieldData.floatArrayValues.length < 3)
                    fieldData.numElements = 0;
                else
                    fieldData.numElements = 1;
                fieldData.dataType = VRMLFieldData.FLOAT_ARRAY_DATA;
            ...
            case FieldConstants.SFVEC4F:
                fieldData.floatArrayValues = (float[])value;
                if((value == null) || fieldData.floatArrayValues.length < 4)
                    fieldData.numElements = 0;
                else
                    fieldData.numElements = 1;
                fieldData.dataType = VRMLFieldData.FLOAT_ARRAY_DATA;
            ...
            case FieldConstants.SFVEC2D:
                fieldData.doubleArrayValues = (double[])value;
                if((value == null) || fieldData.doubleArrayValues.length < 2)
                    fieldData.numElements = 0;
                else
                    fieldData.numElements = 1;
                fieldData.dataType = VRMLFieldData.DOUBLE_ARRAY_DATA;
            ...
            case FieldConstants.SFVEC3D:
                ...
                if((value == null) || fieldData.doubleArrayValues.length < 3)
                    fieldData.numElements = 0;
                else
                    fieldData.numElements = 1;
            ...
            case FieldConstants.SFVEC4D:
                fieldData.doubleArrayValues = (double[])value;
                if((value == null) || fieldData.doubleArrayValues.length < 4)
                    fieldData.numElements = 0;
                else
                    fieldData.numElements = 1;
                fieldData.dataType = VRMLFieldData.DOUBLE_ARRAY_DATA;
            ...
            case FieldConstants.MFIMAGE:
                ...
                fieldData.numElements = (value != null) ?
                    fieldData.intArrayValues.length :
                    0;
                fieldData.dataType = VRMLFieldData.INT_ARRAY_DATA;
                break;
        }
        ...
        switch(data.dataType) {
            ...
            case VRMLFieldData.BOOLEAN_ARRAY_DATA:
                ...
                if(data.numElements != 0) {
                    ...
                    System.arraycopy(data.booleanArrayValues,
                                     0,
                                     bools,
                                     0,
                                     data.numElements);
            ...
            case VRMLFieldData.INT_ARRAY_DATA:
                ...
                if(data.numElements != 0) {
                    num_items = data.intArrayValues.length;
                    ints = new int[num_items];
                    System.arraycopy(data.intArrayValues,
                                     0,
                                     ints,
                                     0,
                                     num_items);
            ...
            case VRMLFieldData.LONG_ARRAY_DATA:
                ...
                if(data.numElements != 0) {
                    num_items = data.longArrayValues.length;
                    longs = new long[num_items];
                    System.arraycopy(data.longArrayValues,
                                     0,
                                     longs,
                                     0,
                                     num_items);
...
// for the numElements - eg 3 for vec3f, 9 for matrix3f etc.
            case VRMLFieldData.FLOAT_ARRAY_DATA:
                ...
                if(data.numElements != 0) {
                    num_items = data.floatArrayValues.length;
                    floats = new float[num_items];
                    System.arraycopy(data.floatArrayValues,
                                     0,
                                     floats,
                                     0,
                                     num_items);
...
// for the numElements - eg 3 for vec3d, 9 for matrix3d etc.
            case VRMLFieldData.DOUBLE_ARRAY_DATA:
                ...
                if(data.numElements != 0) {
                    num_items = data.doubleArrayValues.length;
                    dbles = new double[num_items];
                    System.arraycopy(data.doubleArrayValues,
                                     0,
                                     dbles,
                                     0,
                                     num_items);
            ...
            case VRMLFieldData.STRING_ARRAY_DATA:
                ...
                if(data.numElements != 0) {
                    ...
                    System.arraycopy(data.stringArrayValues,
                                     0,
                                     strs,
                                     0,
                                     data.numElements);
            ...
            // the code wanting to make the copy is doing the scene graph
