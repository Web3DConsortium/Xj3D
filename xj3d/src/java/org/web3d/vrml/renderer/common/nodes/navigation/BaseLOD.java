...
import static org.web3d.vrml.lang.TypeConstants.ViewDependentNodeType;
...
public abstract class BaseLOD extends BaseGroupingNode
    ...
    protected BaseLOD(VRMLNodeType node) {
        ...
        try {
            ...
            if(field.floatArrayValues.length > rangeLen) {
                vfRange = new float[field.floatArrayValues.length];
                rangeSquared = new float[field.floatArrayValues.length];
            }

            rangeLen = field.numElements;

            System.arraycopy(field.floatArrayValues, 0, vfRange, 0, rangeLen);
    ...
    public int getFieldIndex(String fieldName) {
        ...
        if((ret_val == FIELD_CHILDREN) &&
           ...
           (fieldName.equals("level")))) {
            ...
            // Removed this check as it keeps filter chain content from converting
            // VRML to X3D.
    ...
    public VRMLFieldData getFieldValue(int index)
        ...
        switch(index) {
            ...
            case FIELD_RANGE:
                fieldData.clear();
                fieldData.floatArrayValues = vfRange;
                fieldData.dataType = VRMLFieldData.FLOAT_ARRAY_DATA;
