...
public abstract class BaseCollisionCollection extends AbstractNode
    ...
    static {
        ...
        parameterIdMap.put("BOUNCE", OdeConstants.dContactBounce);
        parameterIdMap.put("USER_FRICTION", OdeConstants.dContactFDir1);
        parameterIdMap.put("FRICTION_COEFFICIENT-2", OdeConstants.dContactMu2);
        parameterIdMap.put("ERROR_REDUCTION", OdeConstants.dContactSoftERP);
        parameterIdMap.put("CONSTANT_FORCE", OdeConstants.dContactSoftCFM);
        parameterIdMap.put("SPEED-1", OdeConstants.dContactMotion1);
        parameterIdMap.put("SPEED-2", OdeConstants.dContactMotion2);
        parameterIdMap.put("SLIP-1", OdeConstants.dContactSlip1);
        parameterIdMap.put("SLIP-2", OdeConstants.dContactSlip2);

        internalParamIdMap = new HashMap<>();
        internalParamIdMap.put("BOUNCE", PARAM_BOUNCE);
        internalParamIdMap.put("USER_FRICTION", PARAM_USER_FRICTION);
        internalParamIdMap.put("FRICTION_COEFFICIENT-2", PARAM_FRICTION_COEFFICIENT_2);
        internalParamIdMap.put("ERROR_REDUCTION", PARAM_ERROR_REDUCTION);
        internalParamIdMap.put("CONSTANT_FORCE", PARAM_CONSTANT_FORCE);
        internalParamIdMap.put("SPEED-1", PARAM_SPEED_1);
        internalParamIdMap.put("SPEED-2", PARAM_SPEED_2);
        internalParamIdMap.put("SLIP-1", PARAM_SLIP_1);
        internalParamIdMap.put("SLIP-2", PARAM_SLIP_2);
    }
...
// So, we have to manually do this calling the check on all contained spaces.
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        switch(index) {
            ...
            case FIELD_FRICTION_COEFFICIENTS:
                fieldData.clear();
                fieldData.floatArrayValues = vfFrictionCoefficients;
            ...
            case FIELD_SLIP_COEFFICIENTS:
                fieldData.clear();
                fieldData.floatArrayValues = vfSlipCoefficients;
                fieldData.dataType = VRMLFieldData.FLOAT_ARRAY_DATA;
                fieldData.numElements = 1;
                break;
            ...
            case FIELD_SURFACE_SPEED:
                fieldData.clear();
                fieldData.floatArrayValues = vfSurfaceSpeed;
                fieldData.dataType = VRMLFieldData.FLOAT_ARRAY_DATA;
                fieldData.numElements = 1;
                break;
            ...
            case FIELD_APPLIED_PARAMETERS:
                fieldData.clear();
                fieldData.stringArrayValues = vfAppliedParameters;
    ...
    private void setAppliedParameters(String[] value, int numValid) {
        ...
        for(int i = 0; i < numValid; i++) {
            ...
            if(val != null) {
                surfaceModeFlags |= val;

                val = internalParamIdMap.get(value[i]);
                appliedParamFlags[i] = val;
            }
