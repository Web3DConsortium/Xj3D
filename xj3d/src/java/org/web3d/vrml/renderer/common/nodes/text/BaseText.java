...
public abstract class BaseText extends AbstractNode
    ...
    /**
     * The stringTextLayoutArray matching the appropriate string in the order list depending
 on the fontstyle topToBottom field.
     */
    protected TextLayout[] stringTextLayoutArray;
    ...
    static {
        ...
        ANTIALIAS = AccessController.doPrivileged(
                (PrivilegedAction<Boolean>) () -> 
                        // privileged code goes here.
                        Boolean.getBoolean(ANTIALIAS_PROP)
        );
    }
    ...
    protected BaseText(boolean useTexture) {
        ...
        vfSolid = false; // changed in X3D 4.0, much improved usability/visibility
    ...
    public void setFontStyle(VRMLNodeType newFont)
        ...
        VRMLFontStyleNodeType fontStyleNodeType;

        if(newFont instanceof VRMLProtoInstance) {
            fontStyleNodeType = (VRMLFontStyleNodeType)
                ((VRMLProtoInstance)newFont).getImplementationNode();
            pFontStyle = (VRMLProtoInstance)newFont;
            int[] fontSecondaryType = newFont.getSecondaryType();

            boolean type_found = false;

            for(int i = 0; i < fontSecondaryType.length && !type_found; i++) {
                if(fontSecondaryType[i] == TypeConstants.FontStyleNodeType)
                    type_found = true;
            }
            if(!type_found)
            ...
            pFontStyle = null;
            fontStyleNodeType = (VRMLFontStyleNodeType)newFont;
        }
        vfFontStyle = fontStyleNodeType;
...
/***************/
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        switch(index) {
            case FIELD_STRING:
                fieldData.stringArrayValues = vfString;
            ...
            case FIELD_LENGTH:
                fieldData.floatArrayValues = vfLength;
...
//        if(numString >= 0)
        ...
        float spacing = fontStyle.getSpacing();
        int i;
        ...
        stringTextLayoutArray = new TextLayout[vfString.length]; // for each string in the Text node MFString string field
        ...
        boolean topToBottom = (vfFontStyle == null) || vfFontStyle.isTopToBottom();

        if (topToBottom)
        {
            if(vfString[0].length() == 0)
                stringTextLayoutArray[0] = new TextLayout(" ", font, frc);
            else
                stringTextLayoutArray[0] = new TextLayout(vfString[0], font, frc);
        } 
        else 
        {
            if(vfString[0].length() == 0)
                ...
                stringTextLayoutArray[0] = new TextLayout(vfString[numString - 1], font, frc);
        }
        Rectangle2D stringTextLayoutBounds = stringTextLayoutArray[0].getBounds();

        int char_height = (int)charBounds.getHeight();
        int max_len = (int)stringTextLayoutBounds.getWidth();
        ...
        if (topToBottom) 
        {
            for (i = 1; i < numString; i++) 
            {
                ...
                {
                    stringTextLayoutArray[i] = new TextLayout(" ", font, frc);
                } 
                else 
                {
                    stringTextLayoutArray[i] = new TextLayout(vfString[i], font, frc);

                    stringTextLayoutBounds = stringTextLayoutArray[i].getBounds();
                    double w = stringTextLayoutBounds.getWidth();
        ...
        else 
        {
            for (i = 1; i < numString; i++)
            {
                ...
                {
                    stringTextLayoutArray[i] = new TextLayout(vfString[numString - i - 1], font, frc);
                    stringTextLayoutBounds = stringTextLayoutArray[i].getBounds();
                    double w = stringTextLayoutBounds.getWidth();
        ...
        // so now we have our max size of the string in texture coords.
        // Let's create the minimum size texture width

        if (max_len > 1_024) {
            System.out.println("Clamping max_len to 1024, was: " + max_len);
            max_len = 512;
        }
        if (total_height > 1_024) {
            System.out.println("Clamping total_height to 512 was: " + total_height);
            total_height = 512;
        }
        if (max_len < 0)
            max_len = 1;
        ...
        // clear the background pixels matching CLEAR_COLOR to transparent again
        // TODO what about intentionally black text, is that cleared?
        ...
        switch(fontStyle.getHorizontalJustification()) 
        {
            ...
            case VRMLFontStyleNodeType.FIRST_JUSTIFY:
                stringTextLayoutBounds = stringTextLayoutArray[numString - 1].getBounds();
                // TODO The start_height for a single line of text to be positioned
                // as ParallelGrpahics does is
                // 44, but its being calculated as 64,
                // and the start_x is too small
                start_height = imgHeight; // - stringTextLayoutArray[numString - 1].getDescent();

                stringTextLayoutArray[numString - 1].draw(g,
                                            0 /*(float)bounds.getX()*/,
                                            start_height);
                for (i = numString - 2; i >= 0; i--) 
                {
                    start_height = imgHeight - char_height * spacing * (numString - i - 1);
                    start_height -= stringTextLayoutArray[i].getDescent();

                    stringTextLayoutBounds = stringTextLayoutArray[i].getBounds();
                    stringTextLayoutArray[i].draw(g, (float)stringTextLayoutBounds.getX(), start_height);
                }
                break;
            ...
            case VRMLFontStyleNodeType.MIDDLE_JUSTIFY:
                ...
                start_height = imgHeight - stringTextLayoutArray[numString - 1].getDescent();
                start_x = (usedPixelWidth >> 1) - ((float)stringTextLayoutBounds.getWidth() / 2);

                stringTextLayoutArray[numString - 1].draw(g, start_x, start_height);
                ...
                for (i = numString - 2; i >= 0; i--) 
                {
                    ...
                    start_x = (usedPixelWidth >> 1) - ((float)stringTextLayoutBounds.getWidth() / 2);
                    stringTextLayoutArray[i].draw(g, start_x, start_height);
                }
                break;
            ...
            case VRMLFontStyleNodeType.END_JUSTIFY:
                ...
                stringTextLayoutBounds = stringTextLayoutArray[numString - 1].getBounds();
                start_x = usedPixelWidth - (float)stringTextLayoutBounds.getWidth();
                start_height = imgHeight - stringTextLayoutArray[numString - 1].getDescent();
    ...
    private int smallestPower (int value)
    {
        int n = 1;
        while (n < value)
            n <<= 1; // n = n << 1; // binary left-shift operator, multiplies by 2 each time
