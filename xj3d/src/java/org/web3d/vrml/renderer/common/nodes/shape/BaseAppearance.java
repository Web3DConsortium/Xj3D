...
public abstract class BaseAppearance extends AbstractNode
    ...
    /** Index of the textureProperties field value */
    protected static final int FIELD_TEXTURE_PROPERTIES = LAST_NODE_INDEX + 7;

    /** Index of the acousticProperties field value */
    protected static final int FIELD_ACOUSTIC_PROPERTIES = LAST_NODE_INDEX + 8;

    /** Index of the alphaCutoff field value */
    protected static final int FIELD_ALPHACUTOFF = LAST_NODE_INDEX + 9;

    /** Index of the alphaMode field value */
    protected static final int FIELD_ALPHAMODE = LAST_NODE_INDEX + 10;

    /** Index of the backMaterial field value */
    protected static final int FIELD_BACKMATERIAL = LAST_NODE_INDEX + 11;

    /** Index of the backMaterial field value */
    protected static final int FIELD_SHADERS = LAST_NODE_INDEX + 12;

    /** Number of fields constant */
    private static final int NUM_FIELDS = FIELD_SHADERS + 1;
    ...
    protected VRMLProtoInstance pAcousticProperties;

    /** SFNode acousticProperties. */
    protected VRMLNodeType vfAcousticProperties;

    /** SFFloat alphaCutoff. */
    protected float vfAlphaCutoff;

    /** SFString alphaMode. */
    protected String vfAlphaMode;

    /** Proto version of the backMaterial */
    protected VRMLProtoInstance pBackMaterial;

    /** SFNode backMaterial. */
    protected VRMLNodeType vfBackMaterial;

    /** Proto version of the shaders */
    protected VRMLProtoInstance pShaders;

    /** SFNode shaders. */
    protected VRMLNodeType vfShaders;
    ...
    static {
        ...
        fieldDecl[FIELD_TEXTURE_PROPERTIES] =
            ...
            "SFNode",
            "textureProperties");
        fieldDecl[FIELD_ACOUSTIC_PROPERTIES] =
            new VRMLFieldDeclaration(FieldConstants.EXPOSEDFIELD,
            "SFNode",
            "acousticProperties");
        fieldDecl[FIELD_ALPHACUTOFF] =
            new VRMLFieldDeclaration(FieldConstants.EXPOSEDFIELD,
            "SFFloat",
            "alphaCutoff");
        fieldDecl[FIELD_ALPHAMODE] =
            new VRMLFieldDeclaration(FieldConstants.EXPOSEDFIELD,
            "SFString",
            "alphaMode");
        fieldDecl[FIELD_BACKMATERIAL] =
            new VRMLFieldDeclaration(FieldConstants.EXPOSEDFIELD,
            "SFNode",
            "backMaterial");
        fieldDecl[FIELD_SHADERS] =
            new VRMLFieldDeclaration(FieldConstants.EXPOSEDFIELD,
            "SFNode",
            "shaders");
        ...
        idx = FIELD_TEXTURE_TRANSFORM;
        fieldMap.put("textureTransform", idx);
        fieldMap.put("set_textureTransform", idx);
        fieldMap.put("textureTransform_changed", idx);

        idx = FIELD_LINE_PROPERTIES;
        fieldMap.put("lineProperties", idx);
        fieldMap.put("set_lineProperties", idx);
        fieldMap.put("lineProperties_changed", idx);

        idx = FIELD_POINT_PROPERTIES;
        fieldMap.put("pointProperties", idx);
        fieldMap.put("set_pointProperties", idx);
        fieldMap.put("pointProperties_changed", idx);

        idx = FIELD_FILL_PROPERTIES;
        fieldMap.put("fillProperties", idx);
        fieldMap.put("set_fillProperties", idx);
        fieldMap.put("fillProperties_changed", idx);

        idx = FIELD_TEXTURE_PROPERTIES;
        fieldMap.put("textureProperties", idx);
        fieldMap.put("set_textureProperties", idx);
        fieldMap.put("textureProperties_changed", idx);

        idx = FIELD_ACOUSTIC_PROPERTIES;
        fieldMap.put("acousticProperties", idx);
        fieldMap.put("set_acousticProperties", idx);
        fieldMap.put("acousticProperties_changed", idx);

        idx = FIELD_ALPHACUTOFF;
        fieldMap.put("alphaCutoff", idx);
        fieldMap.put("set_alphaCutoff", idx);
        fieldMap.put("alphaCutoff_changed", idx);

        idx = FIELD_ALPHAMODE;
        fieldMap.put("alphaMode", idx);
        fieldMap.put("set_alphaMode", idx);
        fieldMap.put("alphaMode_changed", idx);

        idx = FIELD_BACKMATERIAL;
        fieldMap.put("backMaterial", idx);
        fieldMap.put("set_backMaterial", idx);
        fieldMap.put("backMaterial_changed", idx);

        idx = FIELD_SHADERS;
        fieldMap.put("shaders", idx);
        fieldMap.put("set_shaders", idx);
        fieldMap.put("shaders_changed", idx);
    ...
    public void notifyExternProtoLoaded(int index, VRMLNodeType node)
        ...
        default:
            System.err.println("BaseAppearance: " +
    ...
    /**
     * Set the acousticProperties that should be used for this appearance. Setting a
     * value of null will clear the current texture Properties.
     *
     * @param prop The new acousticProperties instance to be used.
     * @throws InvalidFieldValueException The node does not match the required
     *    type.
     */
    public void setAcousticProperties(VRMLNodeType prop)
        throws InvalidFieldValueException {
        
        VRMLAppearanceChildNodeType node;
        VRMLNodeType old_node;

        if(pAcousticProperties != null)
            old_node = pAcousticProperties;
        else
            old_node = vfAcousticProperties;

        if (prop instanceof VRMLProtoInstance) {
            node = (VRMLAppearanceChildNodeType)
                ((VRMLProtoInstance)prop).getImplementationNode();
            pAcousticProperties = (VRMLProtoInstance) prop;
            if ((node != null) && !(node instanceof VRMLAppearanceChildNodeType)) {
                throw new InvalidFieldValueException(POINT_PROP_PROTO_MSG);
            }
        }
            ...
            pAcousticProperties = null;
            node = (VRMLAppearanceChildNodeType) prop;
        }

        vfAcousticProperties = (VRMLNodeType)node; // TODO
     ...
     * Set the alphaCutoff
     *
     * @param alphaCutoff The new alphaCutoff to use
     * @throws InvalidFieldValueException Radius value was negative
     */
    protected void setAlphaCutoff(float alphaCutoff)
        throws InvalidFieldValueException {

        if ((alphaCutoff < 0) ||(alphaCutoff > 1))
            throw new InvalidFieldValueException("alphaCutoff must be [0,1] not " + alphaCutoff);

        vfAlphaCutoff = alphaCutoff;
        if(!inSetup) {
            hasChanged[FIELD_ALPHACUTOFF] = true;
            fireFieldChanged(FIELD_ALPHACUTOFF);
        }
    }
     ...
     * Set the alphaMode
     *
     * @param alphaMode The new alphaMode to use
     * @throws InvalidFieldValueException Radius value was negative
     */
    protected void setAlphaMode (String alphaMode)
        throws InvalidFieldValueException {

        vfAlphaMode = alphaMode;
        if(!inSetup) {
            hasChanged[FIELD_ALPHAMODE] = true;
            fireFieldChanged(FIELD_ALPHAMODE);
        }
    }

    /**
     * Set the backMaterial that should be used for this appearance. Setting a
     * value of null will clear the current texture Properties.
     *
     * @param prop The new backMaterial instance to be used.
     * @throws InvalidFieldValueException The node does not match the required
     *    type.
     */
    public void setBackMaterial(VRMLNodeType prop)
        throws InvalidFieldValueException {
        // TODO
    }
    /**
     * Set the shaders that should be used for this appearance. Setting a
     * value of null will clear the current texture Properties.
     *
     * @param prop The new shaders instance to be used.
     * @throws InvalidFieldValueException The node does not match the required
     *    type.
     */
    public void setShaders(VRMLNodeType prop)
        throws InvalidFieldValueException {
        // TODO
    }
    ...
    /**
     * Get the current acousticProperties used by the appearance. If none has
     * been set this will return null.
     *
     * @return The currently set AcousticProperties
     */
    public VRMLNodeType getAcousticProperties() {
        if (pAcousticProperties != null)
            return pAcousticProperties;
        else
            return vfAcousticProperties;
    }

    /**
     * Get the current alphaCutoff used by the appearance. If none has
     * been set this will return null.
     *
     * @return The currently set AlphaCutoff
     */
    public float getAlphaCutoff() {
        return vfAlphaCutoff;
    }

    /**
     * Get the current alphaMode used by the appearance. If none has
     * been set this will return null.
     *
     * @return The currently set alphaMode
     */
    ...
    /**
     * Get the current backMaterial used by the appearance. If none has
     * been set this will return null.
     *
     * @return The currently set BackMaterial
     */
    public VRMLNodeType getBackMaterial() {
        if (pBackMaterial != null)
            return pBackMaterial;
        else
            return vfBackMaterial;
    }

    /**
     * Get the current shaders used by the appearance. If none has
     * been set this will return null.
     *
     * @return The currently set Shaders
     */
    public VRMLNodeType getShaders() {
        if (pShaders != null)
            return pShaders;
        else
            return vfShaders;
    ...
    public void setupFinished() {
        ...
        if(pAcousticProperties != null)
            pAcousticProperties.setupFinished();
        else if(vfAcousticProperties != null)
            vfAcousticProperties.setupFinished();
        if(pBackMaterial != null)
            pBackMaterial.setupFinished();
        else if(vfBackMaterial != null)
            vfBackMaterial.setupFinished();
        if(pShaders != null)
            pShaders.setupFinished();
        else if(vfShaders != null)
            vfShaders.setupFinished();
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        case FIELD_ACOUSTIC_PROPERTIES:
            fieldData.clear();
            if(pAcousticProperties != null)
                fieldData.nodeValue = pAcousticProperties;
            else
                fieldData.nodeValue = vfAcousticProperties;
            fieldData.dataType = VRMLFieldData.NODE_DATA;
            break;
            
        case FIELD_ALPHACUTOFF:
            fieldData.clear();
            fieldData.floatValue = vfAlphaCutoff;
            fieldData.dataType = VRMLFieldData.FLOAT_DATA;
            break;
            
        case FIELD_ALPHAMODE:
            fieldData.clear();
            fieldData.stringValue = vfAlphaMode;
            fieldData.dataType = VRMLFieldData.STRING_DATA;
            break;

        case FIELD_BACKMATERIAL:
            fieldData.clear();
            if(pBackMaterial != null)
                fieldData.nodeValue = pBackMaterial;
            else
                fieldData.nodeValue = vfBackMaterial;
            fieldData.dataType = VRMLFieldData.NODE_DATA;
            break;
        ...
        case FIELD_SHADERS:
            fieldData.clear();
            if(pShaders != null)
                fieldData.nodeValue = pShaders;
            else
                fieldData.nodeValue = vfShaders;
            fieldData.dataType = VRMLFieldData.NODE_DATA;
            break;
    ...
    public void sendRoute(double time,
        ...
        try {
            ...
            case FIELD_ACOUSTIC_PROPERTIES:
                if(pAcousticProperties != null)
                    destNode.setValue(destIndex, pAcousticProperties);
                else
                    destNode.setValue(destIndex, vfAcousticProperties);
                break;

            case FIELD_ALPHACUTOFF:
                destNode.setValue(destIndex, vfAlphaCutoff);
                break;

            case FIELD_ALPHAMODE:
                destNode.setValue(destIndex, vfAlphaMode);
                break;

            case FIELD_BACKMATERIAL:
                if(pBackMaterial != null)
                    destNode.setValue(destIndex, pBackMaterial);
                else
                    destNode.setValue(destIndex, vfBackMaterial);
                break;

            case FIELD_SHADERS:
                if(pShaders != null)
                    destNode.setValue(destIndex, pShaders);
                else
                    destNode.setValue(destIndex, vfShaders);
                break;
    ...
    public void setValue(int index, VRMLNodeType child)
        ...
        switch(index) {
            ...
            case FIELD_ACOUSTIC_PROPERTIES:
                setAcousticProperties(node);
                break;

            case FIELD_BACKMATERIAL:
                setBackMaterial(node);
                break;
