...
public abstract class BaseFontStyle extends AbstractNode
    ...
    static {
        ...
        fieldMap.put("family", FIELD_FAMILY);
        fieldMap.put("horizontal", FIELD_HORIZONTAL);
        fieldMap.put("justify", FIELD_JUSTIFY);
        fieldMap.put("language", FIELD_LANGUAGE);
        fieldMap.put("leftToRight", FIELD_LEFTTORIGHT);
        fieldMap.put("size", FIELD_SIZE);
        fieldMap.put("spacing", FIELD_SPACING);
        fieldMap.put("style", FIELD_STYLE);
        fieldMap.put("topToBottom", FIELD_TOPTOBOTTOM);

        justifyMap = new HashMap<>();
        justifyMap.put("BEGIN", BEGIN_JUSTIFY);
        justifyMap.put("END", END_JUSTIFY);
        justifyMap.put("FIRST", FIRST_JUSTIFY);
        justifyMap.put("MIDDLE", MIDDLE_JUSTIFY);

        styleMap = new HashMap<>();
        styleMap.put("PLAIN", PLAIN_STYLE);
        styleMap.put("BOLD", BOLD_STYLE);
        styleMap.put("ITALIC", ITALIC_STYLE);
        styleMap.put("BOLDITALIC", BOLDITALIC_STYLE);
        ...
        for (String all_font : all_fonts) {
            systemFontFamilies.add(all_font);
        }
    ...
    public BaseFontStyle() {
        ...
        vfHorizontal = true;
        vfJustify = new String[] { "BEGIN", "FIRST" };
        vfLeftToRight = true;
        vfSize = 1;
        vfSpacing = 1;
        vfStyle = "PLAIN";
        ...
        verticalJustification = FIRST_JUSTIFY;
    ...
    public BaseFontStyle(VRMLNodeType node) {
        ...
        try {
            ...
            if(field.numElements != 0) {
                String[] tmp = field.stringArrayValues;
            ...
            vfJustify[0] = field.stringArrayValues[0];
            vfJustify[1] = field.stringArrayValues[1];
    ...
    public void setupFinished() {
        ...
        for (String vfFamily1 : vfFamily) {
            if (systemFontFamilies.contains(vfFamily1)) {
                selectedFamilyFont = vfFamily1;
                break;
            } else if (fontMap.containsKey(vfFamily1)) {
                selectedFamilyFont = fontMap.get(vfFamily1);
                break;
            }
        ...
        Map<TextAttribute, Object> font_attr = new HashMap<>();
        font_attr.put(TextAttribute.FAMILY, selectedFamilyFont);
        font_attr.put(TextAttribute.SIZE, vfSize * FONT_SIZE);
        ...
        if(vfHorizontal) // horizontal font layout
        {
            if(vfJustify[0] != null) {
                tmp = justifyMap.get(vfJustify[0]);
                horizontalJustification = tmp;
            }

            if(vfJustify[1] != null) {
                tmp = justifyMap.get(vfJustify[1]);
                verticalJustification = tmp;
            }
        } 
        else // vertical font layout
        {
            if(vfJustify[1] != null) {
                tmp = justifyMap.get(vfJustify[1]);
                horizontalJustification = tmp;
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        switch(index)
        {
            case FIELD_FAMILY:
                ...
                fieldData.stringArrayValues = vfFamily;
            ...
            case FIELD_JUSTIFY:
                fieldData.dataType = VRMLFieldData.STRING_ARRAY_DATA;
                fieldData.stringArrayValues = vfJustify;
