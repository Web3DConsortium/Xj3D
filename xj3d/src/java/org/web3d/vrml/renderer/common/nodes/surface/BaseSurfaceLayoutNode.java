...
public abstract class BaseSurfaceLayoutNode extends BaseSurfaceChildNode
    ...
    public void setChildren(VRMLNodeType[] kids)
        ...
        if(kids == null)
            ...
            for (VRMLNodeType kid : kids) {
                if (!(kid instanceof VRMLSurfaceChildNodeType)) {
                    if (!(kid instanceof VRMLProtoInstance)) {
                        throw new InvalidFieldValueException(BAD_NODE_MSG);
                    }
                    // check the proto for the correct type.
                    VRMLProtoInstance proto = (VRMLProtoInstance) kid;
     ...
     * be a {@link org.web3d.vrml.nodes.VRMLProtoInstance}.
     *
   ...
   public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        switch(index) {
            case FIELD_CHILDREN:
                fieldData.clear();
                fieldData.nodeArrayValues = vfChildren;
    ...
    private void updateVisibility() {
        ...
        for (VRMLNodeType vfChildren1 : vfChildren) {
            if (!(vfChildren1 instanceof VRMLSurfaceChildNodeType)) {
                VRMLProtoInstance proto = (VRMLProtoInstance) vfChildren1;
                impl = (VRMLSurfaceChildNodeType)proto.getImplementationNode();
            } else {
                impl = (VRMLSurfaceChildNodeType) vfChildren1;
            }
