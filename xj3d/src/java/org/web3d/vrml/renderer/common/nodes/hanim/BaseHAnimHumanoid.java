...
public abstract class BaseHAnimHumanoid extends AbstractNode
    ...
    protected static final int FIELD_BBOX_DISPLAY = LAST_NODE_INDEX + 19;
    
    /** Index of the visible field */
    protected static final int FIELD_VISIBLE = LAST_NODE_INDEX + 20;

    /** The last field index used by this class */
    protected static final int LAST_HUMANOID_INDEX = FIELD_VISIBLE;
    ...
    /** Message for when the node is not an HAnimSite instance */
    protected static final String HANIMSITE_NODE_MSG =
        "Node does not describe a HAnimSite object";
    ...
    /** field SFBool bboxDisplay */
    protected boolean vfBboxDisplay;
    
    /** field SFBool visible */
    ...
    public void setHAnimFactory(HAnimFactory fac) {
        ...
        vfSkeleton.forEach(skel -> {
            skel.setHAnimFactory(fac);
        });

        vfSites.forEach(site -> {
        ...
        HAnimObject[] skel_list = new HAnimObject[vfSkeleton.size()];

        int i = 0;
        for(VRMLHAnimNodeType skel : vfSkeleton)
        {
            skel_list[i++] = skel.getHAnimObject();
        }

        hanimImpl.setSkeleton(skel_list, vfSkeleton.size());

        HAnimObject[] site_list = new HAnimObject[vfSites.size()];

        i = 0;
        for(VRMLHAnimNodeType site : vfSites)
        {
            site_list[i++] = site.getHAnimObject();
        }

        hanimImpl.setSites(site_list, vfSites.size());

        // The viewpoint field needs to be handled by the derived class.

        // Don't go passing the segment or joint list along. Doesn't do
        // anything useful.
    }
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        switch(index) {
            ...
            case FIELD_INFO:
                fieldData.stringArrayValues = vfInfo;
     ...
     * implementation too.
     *
	 * An HAnimSite gets added in the viewpoints field that can contain Viewpoint nodes
	 * 
     * @param node The node to add
     * @throws InvalidFieldValueException This is a bindable node shared
     */
    protected void addViewpointNode(VRMLNodeType node)
        ...
        } else if(node != null && !(node instanceof VRMLHAnimNodeType) && (!node.getVRMLNodeName().equals("HAnimSite"))) {
            throw new InvalidFieldValueException(HANIMSITE_NODE_MSG);
        }
