...
public abstract class CRProtoCreator
    ...
    protected CRProtoCreator(VRMLNodeFactory fac,
        ...
        traverser.setObserver(CRProtoCreator.this);
     ...
     * scene graph and then after the EP is loaded they fill in the details.
     *
    ...
    protected void finishCreate(PrototypeDecl proto) {
        ...
        for (VRMLNodeType children1 : children) {
            traverser.traverseGraph(protoInstance, children1);
        }
    ...
    private void processProtoDecls(List<VRMLNodeTemplate> protos) {
        for (VRMLNodeTemplate proto : protos) {
            scene.addTemplate(proto);
        }
    }
    ...
    protected void setProtoField(VRMLNodeType node,
        ...
        switch(data.dataType) {
            ...
            case VRMLFieldData.BOOLEAN_ARRAY_DATA:
                node.setValue(field, data.booleanArrayValues, data.numElements * fieldSize);
                break;
            ...
            case VRMLFieldData.INT_ARRAY_DATA:
                node.setValue(field, data.intArrayValues, data.numElements * fieldSize);
                break;
            ...
            case VRMLFieldData.LONG_ARRAY_DATA:
                node.setValue(field, data.longArrayValues, data.numElements * fieldSize);
                break;
            ...
            case VRMLFieldData.FLOAT_ARRAY_DATA:
                node.setValue(field, data.floatArrayValues, data.numElements * fieldSize);
                break;
            ...
            case VRMLFieldData.DOUBLE_ARRAY_DATA:
                node.setValue(field, data.doubleArrayValues, data.numElements * fieldSize);
                break;
            ...
            case VRMLFieldData.STRING_ARRAY_DATA:
                node.setValue(field, data.stringArrayValues, data.numElements * fieldSize);
                break;
            ...
            case VRMLFieldData.NODE_ARRAY_DATA:
                if((data.nodeArrayValues != null) && (data.numElements * fieldSize != 0))
                    node.setValue(field,
                            processNodeField(data.nodeArrayValues,
