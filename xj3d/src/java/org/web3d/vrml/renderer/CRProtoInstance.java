 ...
 * Then, to add the real object to the scene graph you call either
...
public class CRProtoInstance
    ...
    private final IntHashMap<Object> userData;
     ...
     */
    protected final IntHashMap<ProtoFieldInfo[]> isMap;
     ...
     */
    protected final IntHashMap<Object> fieldValueMap;
     ...
     */
    protected final IntHashMap<Integer> fieldSizeMap;

    /** Map of whether the field index has been changed since last check */
    protected final IntHashMap<Boolean> fieldChangedMap;
    ...
    private IntHashMap<List<VRMLNodeType>> lostList;

    /** Flag for when the scene is set before the frame state manager*/
    private boolean delayedAdd;

    /** Prevents double calls to propagateSetupFinish for externprotos */
    private boolean propagated;

    private Set<Integer> activeInputFields;
    ...
    private class ISFieldAdapter implements VRMLNodeListener {
        ...
        public void fieldChanged(int index) {
            if((index != sourceIndex) || activeInputFields.contains(index))
                return;
            ...
            Integer curr_size = fieldSizeMap.get(isFieldIndex);

            if((curr_size == null) || curr_size != data.numElements) {

                VRMLFieldDeclaration decl = getFieldDeclaration(isFieldIndex);

                int size_mult = decl.getFieldSize();

                fieldSizeMap.put(isFieldIndex, data.numElements * size_mult);
            }

            switch(data.dataType) {
                case VRMLFieldData.BOOLEAN_DATA:
                    fieldValueMap.put(isFieldIndex, data.booleanValue);
                    break;

                case VRMLFieldData.INT_DATA:
                    fieldValueMap.put(isFieldIndex, data.intValue);
                    break;

                case VRMLFieldData.LONG_DATA:
                    fieldValueMap.put(isFieldIndex, data.longValue);
                    break;

                case VRMLFieldData.FLOAT_DATA:
                    fieldValueMap.put(isFieldIndex, data.floatValue);
                    break;

                case VRMLFieldData.DOUBLE_DATA:
                    fieldValueMap.put(isFieldIndex, data.doubleValue);
                    break;
                ...
                case VRMLFieldData.BOOLEAN_ARRAY_DATA:
                    fieldValueMap.put(isFieldIndex, data.booleanArrayValues);
                    break;

                case VRMLFieldData.INT_ARRAY_DATA:
                    fieldValueMap.put(isFieldIndex, data.intArrayValues);
                    break;

                case VRMLFieldData.LONG_ARRAY_DATA:
                    fieldValueMap.put(isFieldIndex, data.longArrayValues);
                    break;

                case VRMLFieldData.FLOAT_ARRAY_DATA:
                    fieldValueMap.put(isFieldIndex, data.floatArrayValues);
                    break;

                case VRMLFieldData.DOUBLE_ARRAY_DATA:
                    fieldValueMap.put(isFieldIndex, data.doubleArrayValues);
                    break;

                case VRMLFieldData.NODE_ARRAY_DATA:
                    fieldValueMap.put(isFieldIndex, data.nodeArrayValues);
                    break;

                case VRMLFieldData.STRING_ARRAY_DATA:
                    fieldValueMap.put(isFieldIndex, data.stringArrayValues);
                    break;
    ...
    public synchronized void updateRefCount(int layer, boolean add) {
        ...
        if (bodyNodes.length == 0) {
            ...
            for (VRMLNodeType bodyNode : bodyNodes) {
                bodyNode.updateRefCount(layer, add);
            }
        }
    }
    ...
    public void setupFinished() {
        ...
        for(int i=0; i < keySet.length; i++) {
            list = lostList.get(keySet[i]);

            for (VRMLNodeType node : list) {
                // now set the value to all the fields that are ISd
                ProtoFieldInfo[] is_list = isMap.get(keySet[i]);
                if (is_list != null) {
                    for (ProtoFieldInfo _list : is_list) {
                        _list.node.notifyExternProtoLoaded(_list.field, node);
                    }
                }
            }
    ...
    public boolean hasFieldChanged(int index) {
        ...
        Boolean changed = fieldChangedMap.get(index);
    ...
    public VRMLFieldData getFieldValue(int index) throws InvalidFieldException {
        ...
        Integer size = fieldSizeMap.get(index);

        fieldData.numElements = (size != null) ? size: 0;
    ...
    public void notifyExternProtoLoaded(int index, VRMLNodeType node)
        ...
        if (inSetup) {
            ...
            List<VRMLNodeType> list = lostList.get(index);
        ...
        // now set the value to all the fields that are ISd
        ProtoFieldInfo[] is_list = isMap.get(index);
    ...
    public void sendRoute(double time,
        ...
        if(value == null) {
            ...
            ProtoFieldInfo[] is_list = isMap.get(srcIndex);
...
// Not the right size. Should multiply by the raw data size
                    ...
                    case VRMLFieldData.NODE_ARRAY_DATA:
                        destNode.setValue(destIndex, val.nodeArrayValues,
                                          val.numElements);
                        break;
                }
            }
    ...
    public void setValue(int index, int value)
        ...
        activeInputFields.add(index);

        for (ProtoFieldInfo _list : is_list) {
            _list.node.setValue(_list.field, value);
        }

        activeInputFields.remove(index);
    }
    ...
    public void setValue(int index, int[] value, int numValid)
        ...
        if (backFill) {
            ...
            if (old_value != null) {
                value = (int[]) old_value;
                numValid = fieldSizeMap.get(index);
            } else {
            ...
            Integer curr_size = fieldSizeMap.get(index);
            if(curr_size == null || curr_size != numValid)
                fieldSizeMap.put(index, numValid);
        ...
        for (ProtoFieldInfo _list : is_list) {
            _list.node.setValue(_list.field, value, numValid);
        }
        ...
        if (backFill) {
            ...
            if (decl.getFieldType() == FieldConstants.MFNODE && inSetup && !resendingIS) {
                ...
                if (old_value == null) {
                    ...
                    fieldSizeMap.put(index, childs.length);
                }
...
*/
        ...
        for (ProtoFieldInfo _list : is_list) {
            _list.node.setValue(_list.field, child);
        }
...
/*
             ...
             */
            _list.node.setValue(_list.field, children, numValid);
        }
    ...
    protected int appendField(VRMLFieldDeclaration field)
        ...
        fieldIndexMap.put(name, lastFieldIndex);
    ...
    public void setComplete() {
        ...
        if (bodyNotCounted) {
            ...
            for(int i = 0; i < layerIds.length; i++) {
                for (VRMLNodeType bodyNode : bodyNodes) {
                    bodyNode.updateRefCount(i, true);
                }
            }
        }
    ...
    void resendIS() {
        ...
        while(itr.hasNext()) {
            ...
            Integer size = fieldSizeMap.get(idx);

            try {
                sendValue(this, idx, field_val, size);
            } catch (InvalidFieldValueException | InvalidFieldException ifve) {
    ...
    void propagateSetupFinished() {
        ...
        if(propagated || !completeBody || (rootNode == null))
            return;
        ...
        for(i = 0; i < num_fields; i++) {
            ...
            if(value instanceof VRMLNodeType) {
                ...
                for (VRMLNodeType m_node1 : m_node) {
