...
public class CodeInputUnpacker {
     ...
     */
    public CodeInputUnpacker(InputStream stream) {
        this.stream = stream;
    }
     ...
     *
     * @param bytes the data array.
     * @param N the expected code width in bits, which of course must be what
     * was actually packed.
     */
     ...
     * returned as ints.
     *
     * @param maxcodes maximum number of codes expected. Any codes beyond this
     ...
     * returned as ints.
     *
     * @param codearray array to fill sized for maximum number of codes
     * expected. Any codes beyond this length will be discarded!
    ...
    /**
     * Doing reset () on Unpacker means use buffer from the top, the assumption
     * is that it has different data in it now. If the Unpacker was constructed
     * on an input stream, and the stream supports reset, that meaning will be
     * used.
     */
    public void reset() {
        ctr = 0;
    ...
    /**
     * True when no more codes available Note that for some code widths, packing
     * may result in the appearance of one or more final codes, which actually
     * are empty fill in the final byte. A caller must be able to recognize
     * these by a special value, or otherwise know how many valid codes are
     * expected.
     *
     * @return
     * @exception IOException possible only if based on a stream.
