...
public class CompressionTools {
    ...
    private static byte[] outputBuff = new byte[1_024];
    private static byte[] temp_result = new byte[1_024];
...
//System.out.println("delta values:");
            ...
            for(int i=0; i < len; i++) {
                hold = data[i];
                delts[i] = (short)(data[i] - lastVal[scnt]);
//System.out.print(data[i] + "=" + delts[i] + " ");
                if (delts[i] > max)
                    max = delts[i];
                else if (delts[i] < min)
...
//System.out.println("encoded size: " + size);
    ...
    /**
     * Compress an array of integers by determining the number of bits needed to
...
//System.out.println("len: " + (ival.length*4) + " int size: " + (dos.size() - tsize) + " bits: " + bits);
    ...
    /**
     * Decompress an array of integers compressed by determining the number of bits needed to
     * express the range of values.  Optional delta coding is provided.
     *
     *  Format:
...
System.out.println("final mantissa: " + mantissa);
    ...
    public static void findFloatParams(float origval, float tolerance, int[] output) {
        ...
        long MANTISSA_MASK_32 = 0x007f_ffff;
        long EXPONENT_MASK_32 = 0x7f80_0000;
...
//        float origval = 1.23456789f;
        ...
        testAllWholeFloatsSize(1_000, -500, 1f);
    }
