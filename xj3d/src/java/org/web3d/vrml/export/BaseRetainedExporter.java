 ...
 *
 * In the future we'd like to perform this across a SAI graph (i.e. no SAV stream
 * to sniff), this block will document what problems that will entail:
 *
...
public abstract class BaseRetainedExporter extends Exporter implements BinaryContentHandler {
    ...
    /**
     * Should old optional content like HAnim and GeoVRML be converted
     * to native nodes.
     *
    ...
    protected abstract void printExporterInfo();
    ...
    public void endDocument() throws SAVException, VRMLException {
        ...
        if (convertOldContent) {
            if (hanimFound)
                componentList.add(new ComponentInfo("HAnim",1)); // TODO HAnim in X3Dv4
            if (geospatialFound)
     ...
     */
    @Override
    @SuppressWarnings("unchecked") // casts from a home rolled Stack derivative
     ...
     */
    @SuppressWarnings("unchecked") // generic array type Entry[]
    protected String findIS(VRMLNodeType node, int idx, Map<Integer, List<ProtoFieldInfo>> isMap) {
        ...
        for (Entry<Integer, List<ProtoFieldInfo>> map1 : map) {
            list = map1.getValue();
            for (ProtoFieldInfo list1 : list) {
                pfi = list1;
                if (pfi.node == node && pfi.field == idx) {
                    VRMLFieldDeclaration decl = currentPrototypeDecl.getFieldDeclaration(map1.getKey());
                    return decl.getName();
                }
            }
        }
    ...
    /**
     * Get the https publicId for this spec version
     *
     * @param major The major version
     * @param minor The minor version
     * @return The spec'd public id
    ...
    protected String getSystemId(int major, int minor) {
        switch(minor) {
            case 0:
                return X3DConstants.GENERAL_SYSTEM_ID_3_0.replaceAll("http:", "https:");
            case 1:
                return X3DConstants.GENERAL_SYSTEM_ID_3_1.replaceAll("http:", "https:");
            case 2:
                return X3DConstants.GENERAL_SYSTEM_ID_3_2.replaceAll("http:", "https:");
            case 3:
                return X3DConstants.GENERAL_SYSTEM_ID_3_3.replaceAll("http:", "https:");
            case 4:
                return X3DConstants.GENERAL_SYSTEM_ID_4_0.replaceAll("http:", "https:");
            default:
                throw new UnsupportedSpecVersionException("Unhandled minor version: " + minor);
        }
    }
    ...
    private void initOldProtos() {
        ...
        Integer DOUBLE_DATA = (int) VRMLFieldData.DOUBLE_DATA;
        Integer DOUBLE_ARRAY_DATA = (int) VRMLFieldData.DOUBLE_ARRAY_DATA;
    ...
    protected void convertFieldData(int newType, VRMLFieldData data, VRMLFieldDeclaration decl) {
        ...
        switch(newType) {
            case VRMLFieldData.DOUBLE_DATA:
                switch(data.dataType) {
                    ...
                    case VRMLFieldData.FLOAT_DATA:
                        data.doubleValue = data.floatValue;
                        break;
                    case VRMLFieldData.FLOAT_ARRAY_DATA:
                        len = data.floatArrayValues.length;
                        data.doubleArrayValues = new double[len];

                        for(int i=0; i < len; i++) {
                            data.doubleArrayValues[i] = data.floatArrayValues[i];
                        }
                        break;
                    default:
                        StringBuilder buf =
            ...
            case VRMLFieldData.DOUBLE_ARRAY_DATA:
                switch(data.dataType) {
                    case VRMLFieldData.STRING_DATA:
                        data.doubleArrayValues = fieldReader.MFDouble(data.stringValue);
                        break;
                    case VRMLFieldData.STRING_ARRAY_DATA:
                        data.doubleArrayValues = MFDouble(data.stringArrayValues);
                        break;
                    case VRMLFieldData.FLOAT_ARRAY_DATA:
                        len = data.floatArrayValues.length;
                        data.doubleArrayValues = new double[len];
