...
public class JavaSAIScriptWrapper
    ...
    public void initialize(VRMLScriptNodeType node) {
        ...
        List<BaseField> updatables = new ArrayList<>();
        List<BaseField> node_fields = new ArrayList<>();
    ...
    public boolean prepareEvents() {
        if(perFrameScript != null) {
            ...
            if(directOutput) {
                for (BaseField usedNode : usedNodes) {
                    ((NodeField) usedNode).updateFieldAndChildren();
                }
            }
            ...
            if(directOutput) {
                for (BaseField usedNode : usedNodes) {
                    ((NodeField) usedNode).updateNodeAndChildren();
                }
            }
    ...
    public void updateEventOuts() {
        ...
        if(!directOutput) {
            for (BaseField updatableField : updatableFields) {
                if (updatableField.hasChanged()) {
                    updatableField.updateNode();
                }
            }
        } else {
            for (BaseField updatableField : updatableFields) {
                if (updatableField instanceof NodeField) {
                    ((NodeField) updatableField).updateNodeAndChildren();
                } else if (updatableField.hasChanged()) {
                    updatableField.updateNode();
                }
            }
    ...
    private void enableFields() {
        for (BaseField allField : allFields) {
            allField.setAccessValid(true);
        }
    }
