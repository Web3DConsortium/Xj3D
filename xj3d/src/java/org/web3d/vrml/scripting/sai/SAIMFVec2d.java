 ...
 */
class SAIMFVec2d extends BaseMField implements MFVec2d {
     ...
     */
    SAIMFVec2d(VRMLNodeType n, int field, boolean internal) {
        super(n, field, internal);
    }

    //----------------------------------------------------------
    // Methods defined by MFVec2d
    //----------------------------------------------------------
    ...
    public void append(double value[]) {
        ...
        if((localValue == null) || (localValue.length == numElements * 2)) {
            double[] tmp = new double[numElements * 2 + ARRAY_INC];
    ...
    /**
     * Set the value of the array of vectors. Input is an array of doubles
     ...
     * @param value The array of vec2d values where <br>
     *    value[i] = X <br>
     *    value[i+1] = Y
     *
    ...
    void updateField() {
        ...
        if(numElements != 0)
            System.arraycopy(data.floatArrayValues, 0, localValue, 0, req_size);
