 ...
 */
class SAIMFImage extends BaseMField implements MFImage {

    /** Local array of data */
    private int[] localValue;

    /**
     * Index offsets into the main array of values to allow quick finding of
     * details
     */
     ...
     */
    SAIMFImage(VRMLNodeType n, int field, boolean internal) {
        super(n, field, internal);
    }

    //----------------------------------------------------------
    // Methods defined by MFImage
    //----------------------------------------------------------
    ...
    public int getWidth(int imgIndex) {
        checkAccess(false);
        return localValue[imageOffsets[imgIndex]];
    }
    ...
    public int getHeight(int imgIndex)  {
        checkAccess(false);
        return localValue[imageOffsets[imgIndex] + 1];
    }
    ...
    public int getComponents(int imgIndex) {
        checkAccess(false);
        return localValue[imageOffsets[imgIndex] + 2];
    }
    ...
    public void getPixels(int imgIndex, int[] pixels) {
        checkAccess(false);
//        localValue[imageOffsets[imgIndex]];
    }
