...
class MFNodeWrapper extends BaseFieldWrapper
    ...
    public X3DNode get1Value(int index) {
        if (storedOutput)
            ...
            // nodeArrayValues may be null if numElements == 0
            if (index<0 || index>=value.numElements)
                throw new ArrayIndexOutOfBoundsException();
            else
                return nodeFactory.getSAINode(value.nodeArrayValues[index]);
        }
    }
    ...
    public void getValue(X3DNode[] nodes) {
        if (storedOutput)
            ...
            for (int counter=0; counter<data.numElements; counter++)
                nodes[counter]=nodeFactory.getSAINode(data.nodeArrayValues[counter]);
        }
    }
    ...
    private void loadInputValue() {
        ...
        for (int counter=0; counter<value.numElements; counter++)
            storedInputValue[counter]=(value.nodeArrayValues[counter]);
        storedInput=true;
    ...
    public void loadOutputValue() {
        ...
        // Found value.nodeArrayValues to be null when loading a primative Box onto a Grid in ChefX3D 07 MAY 2014 TDN
        if ((storedOutputValue == null || storedOutputValue.length!=value.numElements) && value.nodeArrayValues != null) {
            storedOutputValue=new X3DNode[value.numElements];
        } else {
            return; // nothing to load
        }
        for (int counter=0; counter<value.numElements; counter++)
            storedOutputValue[counter]=nodeFactory.getSAINode(value.nodeArrayValues[counter]);
        storedOutput=true;
    }
