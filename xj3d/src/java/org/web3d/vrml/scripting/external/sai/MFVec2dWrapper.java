 ...
 *
 */
class MFVec2dWrapper extends BaseFieldWrapper
    implements MFVec2d, ExternalEvent, ExternalOutputBuffer {
     ...
     */
    MFVec2dWrapper(VRMLNodeType node, int field, ExternalEventQueue aQueue,
    ...
    /**
     * @see org.web3d.x3d.sai.MFVec2d#append(float[])
     */
    @Override
    public void append(double[] value) {
        ...
        synchronized(theEventQueue.eventLock) {
            MFVec2dWrapper queuedElement=
                (MFVec2dWrapper) theEventQueue.getLast(this);
            ...
            if (queuedElement==null || !queuedElement.isSetOneValue) {
                ...
                if (!storedInput && !storedOutput) {
                    ...
                    queuedElement=new MFVec2dWrapper(
    ...
    public void get1Value(int index, double[] vec) {
        if (storedOutput) {
            ...
            if (index<0 || index>=value.numElements)
                ...
                vec[0]=value.doubleArrayValues[index];
                vec[1]=value.doubleArrayValues[index+1];
            }
        }
    }
    ...
    public void getValue(double[] vec) {
        if (storedOutput)
            ...
            if (data.numElements!=0)
                System.arraycopy(data.doubleArrayValues,0,vec,0,data.numElements*2);
        }
    }
    ...
    public void getValue(double[][] vec) {
        if (storedOutput)
            ...
            VRMLFieldData data=theNode.getFieldValue(fieldIndex);
            ArrayUtils.raise2(data.doubleArrayValues,data.numElements,vec);
        }
    }
    ...
    /**
     * @see org.web3d.x3d.sai.MFVec2d#insertValue(int, boolean)
     */
    @Override
    public void insertValue(int index, double value[]) throws ArrayIndexOutOfBoundsException {
        ...
        synchronized(theEventQueue.eventLock) {
            ...
            System.arraycopy(queuedElement.storedInputValue,
            		...
            		index*2+2,
            		queuedElement.storedInputLength-index*2);
    ...
    private void loadInputValue() {
        ...
        if (storedInputValue == null || storedInputValue.length!=value.numElements)
            storedInputValue=new double[value.numElements*2];
        if (value.numElements!=0)
            System.arraycopy(value.doubleArrayValues,0,storedInputValue,0,value.numElements*2);
        storedInput=true;
    ...
    public void loadOutputValue() {
        ...
        if (storedOutputValue == null || storedOutputValue.length!=value.numElements)
            storedOutputValue=new double[value.numElements*2];
        if (value.numElements!=0)
            System.arraycopy(value.doubleArrayValues,0,storedOutputValue,0,value.numElements*2);
        storedOutput=true;
    }

    /**
     * @see org.web3d.x3d.sai.Vec2d#removeValue(int)
     */
    @Override
    ...
    /**
     * Set the value of the array of 2D vectors. Input is an array of doubles
