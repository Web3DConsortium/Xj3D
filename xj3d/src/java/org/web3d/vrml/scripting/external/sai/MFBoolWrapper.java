...
class MFBoolWrapper extends BaseFieldWrapper
    ...
    public boolean get1Value(int index) {
        if (storedOutput)
            ...
            // booleanArrayValues may be null if numElements == 0
            if (index<0 || index>=value.numElements)
                throw new ArrayIndexOutOfBoundsException();
            else
                return value.booleanArrayValues[index];
        }
    }
    ...
    public void getValue(boolean[] values) {
        if (storedOutput)
            ...
            if (data.numElements!=0)
                System.arraycopy(data.booleanArrayValues,0,values,0,data.numElements);
        }
    }
    ...
    private void loadInputValue() {
        ...
        if (value.numElements!=0)
            System.arraycopy(value.booleanArrayValues,0,storedInputValue,0,value.numElements);
        storedInput=true;
    ...
    public void loadOutputValue() {
        ...
        if (value.numElements!=0)
            System.arraycopy(value.booleanArrayValues,0,storedOutputValue,0,value.numElements);
        storedOutput=true;
    }
