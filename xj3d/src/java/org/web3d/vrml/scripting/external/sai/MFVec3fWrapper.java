...
class MFVec3fWrapper extends BaseFieldWrapper
    ...
    public void get1Value(int index, float[] vec) {
        if (storedOutput)
            ...
            if (index<0 || index>=data.numElements)
                ...
                System.arraycopy(data.floatArrayValues,index*3,vec,0,3);
        }
    }
    ...
    public void getValue(float[] vec) {
        if (storedOutput) {
            ...
            if (data.numElements!=0)
                System.arraycopy(data.floatArrayValues,0,vec,0,data.numElements*3);
        }
    }
    ...
    public void getValue(float[][] vec) {
        if (storedOutput) {
            ...
            VRMLFieldData data=theNode.getFieldValue(fieldIndex);
            if (data.numElements!=0)
                ArrayUtils.raise3(data.floatArrayValues,data.numElements,vec);
        }
    }
