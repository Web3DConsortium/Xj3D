 ...
 *
 */
class SFVec2fWrapper extends BaseFieldWrapper implements SFVec2f, ExternalEvent, ExternalOutputBuffer {
     ...
     */
    SFVec2fWrapper(VRMLNodeType node, int field, ExternalEventQueue aQueue,
        SAIEventAdapterFactory factory) {
        super(node,field,aQueue,factory);
        storedInputValue=new float[2];
        storedOutputValue=new float[2];
    }
    ...
    public void doEvent() {
        try {
            theNode.setValue(fieldIndex,storedInputValue,2);
        } finally {
            storedInput=false;
        }
    }
    ...
    public void getValue(float[] vec) {
        if (storedOutput) {
            ...
            if (data.floatArrayValues==null)
                ...
                System.arraycopy(data.floatArrayValues,0,vec,0,2);
        }
    }
    ...
    private void loadInputValue() {
        ...
        if (value.floatArrayValues==null)
            ...
            System.arraycopy(value.floatArrayValues,0,storedInputValue,0,2);
        storedInput=true;
    }
    ...
    public void loadOutputValue() {
        ...
        if (value.floatArrayValues==null)
            ...
            System.arraycopy(value.floatArrayValues,0,storedOutputValue,0,2);
        storedOutput=true;
    }
    ...
    public void setValue(float[] value) {
    	checkWriteAccess();
        SFVec2fWrapper queuedElement=this;
        // Input and output buffers do not mix
        if (storedInput || storedOutput)
            queuedElement=new SFVec2fWrapper(theNode, fieldIndex, theEventQueue, theEventAdapterFactory);
