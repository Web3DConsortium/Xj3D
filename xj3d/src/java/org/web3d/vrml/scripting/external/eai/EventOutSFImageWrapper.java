...
implements ExternalOutputBuffer, EventWrapper {
    ...
    public int getComponents() {
        if (isStored)
            ...
            try {
            	...
            	if (value.intArrayValues==null)
            		return 0;
            	else
            		return value.intArrayValues[2];
    ...
    public void getPixels(int[] dest) {
        if (isStored) {
            ...
            try {
                ...
                System.arraycopy(imageData.intArrayValues,3,dest,0,imageData.numElements-3
                );
            } catch (InvalidFieldException ife) {
                throw new RuntimeException("Error getting value.");
            }
    ...
    public int[] getPixels() {
        ...
        if (isStored) {
            ...
            try {
                ...
                System.arraycopy(imageData.intArrayValues,3,pixels,0,imageData.numElements-3
                );
                return pixels;
            } catch (InvalidFieldException ife) {
                throw new RuntimeException("Error getting value.");
    ...
    public void loadOutputValue() {
        ...
        try {
            ...
            storedComponents=imageData.intArrayValues[2];
            if (storedHeight!=0 & storedWidth!=0) {
                System.arraycopy(imageData.intArrayValues,3,storedPixels,0,
                    storedPixels.length
                );
            } else {
