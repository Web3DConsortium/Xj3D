 ...
 *
 */
class MFVec3dWrapper extends BaseFieldWrapper
    implements MFVec3d, ExternalEvent, ExternalOutputBuffer {
     ...
     */
    MFVec3dWrapper(VRMLNodeType node, int field, ExternalEventQueue aQueue,
    ...
    /**
     * @see org.web3d.x3d.sai.MFVec3d#append(float[])
     */
    @Override
    public void append(double[] value) {
        ...
        synchronized(theEventQueue.eventLock) {
            MFVec3dWrapper queuedElement=
                (MFVec3dWrapper) theEventQueue.getLast(this);
            ...
            if (queuedElement==null || !queuedElement.isSetOneValue) {
                ...
                if (!storedInput && !storedOutput) {
                    ...
                    queuedElement=new MFVec3dWrapper(
    ...
    /**
     * @see org.web3d.x3d.sai.MFVec3d#clear()
     */
    @Override
    public void clear() {
    ...
    public void get1Value(int index, double[] vec) {
        if (storedOutput)
            ...
            if (index<0 || index>=data.numElements)
                ...
                System.arraycopy(data.doubleArrayValues,index*3,vec,0,3);
        }
    }
    ...
    public void getValue(double[] vec) {
        if (storedOutput) {
            ...
            if (data.numElements!=0)
                System.arraycopy(data.doubleArrayValues,0,vec,0,data.numElements*3);
        }
    }
    ...
    public void getValue(double[][] vec) {
        if (storedOutput) {
            ...
            checkReadAccess();
            VRMLFieldData data=theNode.getFieldValue(fieldIndex);
            ArrayUtils.raise3(data.doubleArrayValues,data.numElements,vec);
        }
    }
    ...
    /**
     * @see org.web3d.x3d.sai.MFVec3d#insertValue(int, double[])
     */
    @Override
    ...
    private void loadInputValue() {
        ...
        if (storedInputValue == null || storedInputValue.length!=value.numElements*3)
            storedInputValue=new double[value.numElements*3];
        if (value.numElements!=0)
            System.arraycopy(value.doubleArrayValues,0,storedInputValue,0,value.numElements*3);
        storedInput=true;
    ...
    public void loadOutputValue() {
        ...
        if (storedOutputValue == null || storedOutputValue.length!=value.numElements)
            storedOutputValue=new double[value.numElements*3];
        if (value.numElements!=0)
            System.arraycopy(value.doubleArrayValues,0,storedOutputValue,0,value.numElements*3);
        storedOutput=true;
    }

    /**
     * @see org.web3d.x3d.sai.MFVec3d#removeValue(int)
     */
    @Override
     ...
     * world this will generate a full MFVec3d event with the nominated index
     * value changed.
     *  <p>
     ...
     * elements for that data type. So for an MFdouble the size would be the
     * number of double values, but for an MFVec3d, it is the number of vectors
