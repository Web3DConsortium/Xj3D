...
class NavigationUIManagerAdapter
    ...
    NavigationUIManagerAdapter(ViewpointManager vpm,
        ...
        viewpointMap = new HashMap<>();
        viewpointWrappers = new ArrayList<>();
    ...
    public void availableViewpointsChanged(VRMLViewpointNodeType[] viewpoints) {
        ...
        if(viewpoints != null) {
            for (VRMLViewpointNodeType viewpoint : viewpoints) {
                Xj3DViewpoint vp = new ViewpointWrapper(viewpoint);
                viewpointMap.put(viewpoint, vp);
