 ...
 * Images are represented as arrays of integers as per the VRML IS specification
 * Section 5.5 SFImage. Pixel values are between 0 and 256 and represented as
 * integers to maintain consistency with java's ImageConsumer interface and
 * PixelGrabber class.
 *
 * @author Brad Vendor
...
class SFImageWrapper extends BaseFieldWrapper implements SFImage, ExternalEvent, ExternalOutputBuffer {
    ...
    /**
     * Get the number of colour components in the image. The value will always
     * be between 1 and 4 indicating the number of components of the colour
     * specification to be read from the image pixel data.
     *
    ...
    public int getComponents() {
        if (storedOutput) {
            ...
            if (imageData.intArrayValues != null && imageData.intArrayValues.length > 1) {
                return imageData.intArrayValues[2];
            } else {
                return 0;
            }
        }
    ...
    public int getHeight() {
        if (storedOutput) {
            ...
            checkReadAccess();
            VRMLFieldData imageData = theNode.getFieldValue(fieldIndex);
            if (imageData.intArrayValues != null && imageData.intArrayValues.length > 0) {
                return imageData.intArrayValues[1];
            } else {
                return 0;
            }
        }
     ...
     * ArrayIndexOutOfBoundsException will be generated. The integer values are
     * represented according to the number of components.
    ...
    public void getPixels(int[] pixels) {
        if (storedOutput) {
            ...
            VRMLFieldData imageData = theNode.getFieldValue(fieldIndex);
            if (imageData.intArrayValues != null && imageData.intArrayValues.length > 1) {
                int numPixels = imageData.intArrayValues[0] * imageData.intArrayValues[1];
                if (numPixels > 0) {
                    System.arraycopy(imageData.intArrayValues, 3, pixels, 0, numPixels);
                }
            }
        }
    }
    ...
    public int getWidth() {
        if (storedOutput) {
            ...
            VRMLFieldData imageData = theNode.getFieldValue(fieldIndex);
            if (imageData.intArrayValues != null && imageData.intArrayValues.length > 0) {
                return imageData.intArrayValues[0];
            } else {
                return 0;
            }
        }
    ...
    /**
     * @see
     * org.web3d.vrml.scripting.external.buffer.ExternalOutputBuffer#initialize(org.web3d.vrml.nodes.VRMLNodeType,
     * int)
     */
    @Override
    ...
    /**
     * @see
     * org.web3d.vrml.scripting.external.buffer.ExternalEvent#isConglomerating()
     */
    @Override
    ...
    private void loadInputValue() {
        ...
        System.arraycopy(value.intArrayValues, 0, storedInputValue, 0, value.numElements);
        storedInput = true;
    }
    ...
    public void loadOutputValue() {
        VRMLFieldData imageData = theNode.getFieldValue(fieldIndex);
        int newWidth = imageData.intArrayValues[0];
        int newHeight = imageData.intArrayValues[1];
        if (storedOutputValue == null || storedOutputValue.length < imageData.numElements) {
            storedOutputValue = new int[imageData.numElements];
        }
        System.arraycopy(imageData.intArrayValues, 0, storedOutputValue, 0, imageData.numElements);
        storedOutput = true;
    }

    /**
     * @see
     * org.web3d.vrml.scripting.external.buffer.ExternalOutputBuffer#reset()
     */
    @Override
    public void reset() {
        theNode = null;
     ...
     * RenderedImages are row order from top to bottom. A 4x8 RenderImage is
     * indexed as follows:
     *
     * <pre>
     *
     ...
     *  </pre>
     *
     * SFImages are row order from bottom to top. A 4x8 RenderImage is indexed
     * as follows:
     *
     * <pre>
     *
     ...
     * <br>
     * The parameter destYOffset is referenced to the SFImage object (indexed
     * bottom to top).
     *
     * @param img The new image to use as the source
     ...
     * <code>width * height</code>. If there are less items than this an
     * ArrayIndexOutOfBoundsException will be generated. The integer values are
     * represented according to the number of components. If the integer
     ...
     * @exception ArrayIndexOutOfBoundsException The number of pixels provided
     * by the caller is not enough for the width * height.
     */
    @Override
