...
public class ExternalEventQueue implements ExternalView, NodeRealizationProcessor {
    ...
    /**
     * Used for matching set1Value calls. Since EventIn's which are targeted at
     * the same (node,id) pair are equal and have the same hashCode, this is
     * used to find matching set1Value calls.
     */
    private HashSet<ExternalEvent> conglomerativeEvents;

    /**
     * Used for recording which nodes have been marked as realizing. Nodes get
     * removed from this list as they complete realization.
     */
    private HashSet<VRMLNodeType> realizingNodes;

    /**
     * External lock for the event list. This has to be public because the logic
     * for merging set1Value calls isn't thread safe otherwise.
     */
    ...
    /**
     * The queue for events since the last beginUpdate. These are the events
     * which will be transferred to the waitingQueue when endUpdate is called.
     */
    ObjectArray eventQueue;

    /**
     * The events which will be processed at the end of the frame. These are the
     * events waiting for the end of frame.
     */
    ...
    /**
     * Start queuing updates. It isn't an error to call this multiple times, but
    ...
    /**
     * End queuing of updates, and send the current set to be processed. It
     * isn't an error to call this multiple times, but extra calls are ignored.
     */
    ...
    /**
     * Return the ExternalEvent which is equal to the sent type. This relies on
     * the trick that EventIn instances are equal based on node and field ID
     * properties.
     *
     * @param type
     * @return
    ...
    public ExternalEvent getLast(ExternalEvent type) {
        ...
        ExternalEvent e = null;

        for (Object o : conglomerativeEvents.toArray()) {
            if (o.equals(type)) {
                e = (ExternalEvent) o;
            }
        }
    ...
    /**
     * Process a browser.deleteRoute request. The EAI specification doesn't
     * specifically state that this should be buffered, but it is a world state
     * modification.
     *
     * @param browser
     * @param core
    ...
    /**
     * Process an addition to the event queue. This will either add the event to
     * the queue, or execute it immediately depending on the queuing state
     *
     * @param event
     */
