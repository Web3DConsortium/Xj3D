...
class MFStringWrapper extends BaseFieldWrapper
    ...
    public String get1Value(int index) {
        if (storedOutput)
            ...
            // stringArrayValues may be null if numElements == 0
            if (index<0 || index>=value.numElements)
                throw new ArrayIndexOutOfBoundsException();
            else
                return value.stringArrayValues[index];
        }
    }
    ...
    public void getValue(String[] value) {
        if (storedOutput)
            ...
            if (data.numElements!=0)
                System.arraycopy(data.stringArrayValues,0,value,0,data.numElements);
        }
    }
    ...
    private void loadInputValue() {
        ...
        if (value.numElements!=0)
            System.arraycopy(value.stringArrayValues,0,storedInputValue,0,value.numElements);
        storedInput=true;
    ...
    public void loadOutputValue() {
        ...
        if (value.numElements!=0)
            System.arraycopy(value.stringArrayValues,0,storedOutputValue,0,value.numElements);
        storedOutput=true;
    }
