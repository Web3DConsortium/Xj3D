 ...
 *
 */
class SFStringWrapper extends BaseFieldWrapper implements SFString, ExternalEvent, ExternalOutputBuffer {

	/** The value stored in this buffer iff storedInput */
	String storedInputValue;

	/** The value stored in this buffer iff storedOutput */
	String storedOutputValue;
	 ...
	 */
    SFStringWrapper(VRMLNodeType node, int field, ExternalEventQueue aQueue,
	...
	private void loadInputValue() {
		...
		storedInputValue=value.stringValue;
		storedInput=true;
	}
	...
	public void loadOutputValue() {
		VRMLFieldData value=theNode.getFieldValue(fieldIndex);
		storedOutputValue=value.stringValue;
		storedOutput=true;
	}
    ...
    public void setValue(String value) {
    	checkWriteAccess();
    	SFStringWrapper queuedElement=this;
		// Input and output buffers do not mix
		if (storedInput || storedOutput)
			queuedElement=new SFStringWrapper(theNode, fieldIndex, theEventQueue, theEventAdapterFactory);
