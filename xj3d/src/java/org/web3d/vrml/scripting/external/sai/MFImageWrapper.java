...
class MFImageWrapper extends MFInt32Wrapper
    ...
    /** Ensures that there is at least a certain number of elements
    ...
    public int getComponents(int imgIndex) {
        if (storedOutput) {
            ...
            if (imgIndex<0 || imgIndex>=countActualImages(data.intArrayValues,data.numElements))
                throw new ArrayIndexOutOfBoundsException();
            return data.intArrayValues[findStartOfImage(imgIndex,data.intArrayValues)+2];
        }
    }
    ...
    public int getHeight(int imgIndex) {
        ...
        if (storedOutput) {
            ...
            if (imgIndex<0 || imgIndex>=countActualImages(data.intArrayValues,data.numElements))
                throw new ArrayIndexOutOfBoundsException();
            return data.intArrayValues[findStartOfImage(imgIndex,data.intArrayValues)+1];
        }
    }
    ...
    public void getPixels(int imgIndex, int[] pixels) {
        ...
        if (storedOutput) {
            ...
            if (imgIndex<0 || imgIndex>=countActualImages(data.intArrayValues,data.numElements))
                throw new ArrayIndexOutOfBoundsException();
            pixelSource=data.intArrayValues;
        }
    ...
    public int getWidth(int imgIndex) {
        if (storedOutput) {
            ...
            if (imgIndex<0 || imgIndex>=countActualImages(data.intArrayValues,data.numElements))
                throw new ArrayIndexOutOfBoundsException();
            return data.intArrayValues[findStartOfImage(imgIndex,data.intArrayValues)];
        }
    }
    ...
    private void loadInputValue() {
        ...
        //calculateRequiredStorage(value.numElements,value.intArrayValues);
        if (storedInputValue == null || storedInputValue.length < requiredSize)
            storedInputValue=new int[requiredSize];
        System.arraycopy(value.intArrayValues,0,storedInputValue,0,requiredSize);
        storedInput=true;
    ...
    public void loadOutputValue() {
        ...
        System.arraycopy(value.intArrayValues,0,storedOutputValue,0,requiredSize);
        storedOutput=true;
    }
    ...
    public int getSize() {
        if (storedOutput) {
            ...
            VRMLFieldData value=theNode.getFieldValue(fieldIndex);
            return countActualImages(value.intArrayValues,value.numElements);
        }
    }
...
//        if (counter-1!=rawData.length) // <- Really?
//            throw new RuntimeException("Corrupted MFImage field.");
    ...
    int calculateRequiredStorage(int numFields, int rawData[]) {
        ...
        for (int fieldCounter=0; fieldCounter<numFields; fieldCounter++) {
            currentImageSize=rawData[actualSize]*rawData[actualSize+1]+3;
    ...
    static int findStartOfImage(int index, int rawData[]) {
        ...
        int sizeOfImage;
        while (index>0) {
            sizeOfImage=rawData[startOfImage]*rawData[startOfImage+1]+3;
    ...
    void rewriteImageToSize(int imageIndex, int newWidth, int newHeight) {
        ...
        // Prevent AIOOB Ex
        if (startOfImage+oldSize <= storedInputValue.length && startOfImage+newSize <= newArray.length && storedInputValue.length-startOfImage-oldSize > -1)
