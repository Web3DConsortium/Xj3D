 ...
 *
 */
class SFVec2dWrapper extends BaseFieldWrapper implements SFVec2d, ExternalEvent, ExternalOutputBuffer {

    /** Default field value to use for null field values */
    static final double DEFAULT_FIELD_VALUE[]=new double[]{0.0,0.0};
     ...
     */
    SFVec2dWrapper(VRMLNodeType node, int field, ExternalEventQueue aQueue,
        SAIEventAdapterFactory factory) {
        super(node,field,aQueue,factory);
        storedInputValue=new double[2];
        storedOutputValue=new double[2];
    }
    ...
    public void getValue(double[] vec) {
        if (storedOutput) {
            System.arraycopy(storedOutputValue,0,vec,0,2);
        } else {
        	checkReadAccess();
        	VRMLFieldData data=theNode.getFieldValue(fieldIndex);
            if (data.doubleArrayValues==null)
                System.arraycopy(DEFAULT_FIELD_VALUE,0,vec,0,2);
            else
                System.arraycopy(data.doubleArrayValues,0,vec,0,2);
        }
    }
    ...
    private void loadInputValue() {
        ...
        if (value.doubleArrayValues==null)
            System.arraycopy(DEFAULT_FIELD_VALUE,0,storedInputValue,0,2);
        else
            System.arraycopy(value.doubleArrayValues,0,storedInputValue,0,2);
        storedInput=true;
    }
    ...
    public void loadOutputValue() {
        VRMLFieldData value=theNode.getFieldValue(fieldIndex);
        if (value.doubleArrayValues==null)
            System.arraycopy(DEFAULT_FIELD_VALUE,0,storedOutputValue,0,2);
        else
            System.arraycopy(value.doubleArrayValues,0,storedOutputValue,0,2);
        storedOutput=true;
    }
    ...
    public void setValue(double[] value) {
    	checkWriteAccess();
    	SFVec2dWrapper queuedElement=this;
        // Input and output buffers do not mix
        if (storedInput || storedOutput)
            queuedElement=new SFVec2dWrapper(theNode, fieldIndex, theEventQueue, theEventAdapterFactory);
        System.arraycopy(value,0,queuedElement.storedInputValue,0,2);
