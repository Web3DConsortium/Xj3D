...
class ECMAScriptWrapper implements ScriptWrapper {
    ...
    public void initialize(VRMLScriptNodeType node) {
        ...
        List<String> eo_names = new ArrayList<>(field_list.size() - 3);
            ...
            try {
                ...
                switch(decl.getFieldType()) {
                    ...
                    case FieldConstants.SFCOLOR:
                        f_arg = (data == null) ? null : data.floatArrayValues;
                    ...
                    case FieldConstants.SFVEC2D:
                        d_arg = (data == null) ? null : data.doubleArrayValues;
                    ...
                    case FieldConstants.SFIMAGE:
                        i_arg = (data == null) ? null : data.intArrayValues;
                    ...
                    case FieldConstants.SFNODE:
                        ...
                        nodeEventOuts.put(name, (NodeFieldObject)field_wrapper);
                        break;
                    ...
                    case FieldConstants.MFBOOL:
                        boolean[] b_arg =
                            (data == null) ? null : data.booleanArrayValues;
                    ...
                    case FieldConstants.MFSTRING:
                        s_arg = (data == null) ? null : data.stringArrayValues;
                    ...
                    default:
                        field_wrapper = null;
                        errorReporter.errorReport("Unknown X3D field type " +
                                           decl.getFieldTypeString(), null);
                }
            } catch(InvalidFieldException ife) {
    ...
    public void queueEvent(int type, String name, float value) {
          callScriptFunction(name, (double) value);
    }
    ...
    private void callScriptFunction(String name, Object value) {
        ...
        if(value instanceof FieldScriptableObject)
            ((Scriptable)value).setParentScope(scope);

        enterContext();

        try {
            // call a function corresponding to an eventIn on the script.
...
//                    eventOutMap.put(name, functionArgs[0]);
        ...
        switch(decl.getFieldType()) {
            case FieldConstants.SFBOOL:
                if(!(value instanceof Boolean))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFBool field",
                                                null);
                else
                    node.setValue(index, ((Boolean)value));
                break;

            case FieldConstants.SFINT32:
                if(!(value instanceof Number))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFInt32 field",
                                                null);
                else
            ...
            case FieldConstants.SFFLOAT:
                if(!(value instanceof Number))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFFloat field",
                                                null);
                else
            ...
            case FieldConstants.SFTIME:
                if(!(value instanceof Number))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFTime field",
                                                null);
                else
            ...
            case FieldConstants.SFSTRING:
                if(!(value instanceof String) && !(value instanceof org.mozilla.javascript.ConsString))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value \"" + value + "\" to " + node.getVRMLNodeName() + " " + decl.getName() + " SFString field",
                                                null);
                else
                    node.setValue(index, value.toString()); // apparently can't cast ConsString to (String)
                break;
            ...
            case FieldConstants.SFCOLOR:
                if(!(value instanceof SFColor))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFColor field",
                                                null);
                else {
            ...
            case FieldConstants.SFCOLORRGBA:
                if(!(value instanceof SFColorRGBA))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFColorRGBA field",
                                                null);
                else {
            ...
            case FieldConstants.SFROTATION:
                if(!(value instanceof SFRotation))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFRotation field",
                                                null);
                else {
            ...
            case FieldConstants.SFVEC2F:
                if(!(value instanceof SFVec2f))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFVec2f field",
                                                null);
                else {
            ...
            case FieldConstants.SFVEC3F:
                if(!(value instanceof SFVec3f))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFVec3f field",
                                                null);
                else {
            ...
            case FieldConstants.SFVEC2D:
                if(!(value instanceof SFVec2d))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFVec2d field",
                                                null);
                else {
            ...
            case FieldConstants.SFVEC3D:
                if(!(value instanceof SFVec3d))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFVec3d field",
                                                null);
                else {
            ...
            case FieldConstants.SFIMAGE:
                if(!(value instanceof SFImage))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFImage field",
                                                null);
                else {
            ...
            case FieldConstants.MFBOOL:
                if(!(value instanceof MFBool))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFBool field",
                                                null);
                else {
            ...
            case FieldConstants.MFCOLOR:
                if(!(value instanceof MFColor))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFColor field",
                                                null);
                else {
            ...
            case FieldConstants.MFCOLORRGBA:
                if(!(value instanceof MFColorRGBA))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFColorRGBA field",
                                                null);
                else {
            ...
            case FieldConstants.MFDOUBLE:
                if(!(value instanceof MFDouble))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFDouble field",
                                                null);
                else {
            ...
            case FieldConstants.MFINT32:
                if(!(value instanceof MFInt32))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFInt32 field",
                                                null);
                else {
            ...
            case FieldConstants.MFFLOAT:
                if(!(value instanceof MFFloat))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFFloat field",
                                                null);
                else {
            ...
            case FieldConstants.MFTIME:
                if(!(value instanceof MFTime))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFTime field",
                                                null);
                else {
            ...
            case FieldConstants.MFSTRING:
                if(!(value instanceof MFString))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value [" + value + "] to " + node.getVRMLNodeName() + " " + decl.getName() + " MFString field",
                                                null);
                else {
            ...
            case FieldConstants.MFROTATION:
                if(!(value instanceof MFRotation))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFRotation field",
                                                null);
                else {
            ...
            case FieldConstants.MFVEC2F:
                if(!(value instanceof MFVec2f))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFVec2f field",
                                                null);
                else {
            ...
            case FieldConstants.MFVEC3F:
                if(!(value instanceof MFVec3f))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFVec3f field",
                                                null);
                else {
            ...
            case FieldConstants.MFVEC2D:
                if(!(value instanceof MFVec2d))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFVec2d field",
                                                null);
                else {
            ...
            case FieldConstants.MFVEC3D:
                if(!(value instanceof MFVec3d))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " MFVec3d field",
                                                null);
                else {
            ...
            case FieldConstants.SFNODE:
                if((value != null) && !(value instanceof SFNode))
                    errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                                                "value " + value + " to " + node.getVRMLNodeName() + " " + decl.getName() + " SFNode field",
                                                null);
                else {
                    if(value != null)
                        node.setValue(index, ((NodeImplSource)value).getImplNode());
                    else
            ...
            case FieldConstants.MFNODE:
                if (value == null) {
                    ...
                    // NOTE: Here is the missing call to get an ecmascript function to
                    // finally work (tdn)
                    } else if (value instanceof Scene) {
                        Scene scene = (Scene) value;
                        MFNode mfn = (MFNode) scene.get("rootNodes", null);
                        VRMLNodeType[] n_list = mfn.getRawData();
                        node.setValue(index, n_list, n_list.length);
                    } else {
                        errorReporter.warningReport("Ecmascript attempting to set invalidly typed " +
                            "value [" + value + "] to " + node.getVRMLNodeName() + " " + decl.getName() +
                            " MFNode field",
                            null);
                    }
                }
            ...
            default:
                // unknown value, node value not set.  hopefully not reachable!  here just in case.

                errorReporter.warningReport("Ecmascript attempting to set unknown type (" + decl.getFieldTypeString() + ") having " +
                    "value [" + value + "] to " + node.getVRMLNodeName() + " " + decl.getName() +
                    " MFNode field",
                    null);
        }
    ...
    private boolean sendNodeOutput(String fieldName, boolean changeChecked) {
        ...
        if(changed_items instanceof NodeFieldData) {
            ...
            @SuppressWarnings("unchecked") // cast from an object type
