...
public class MFString extends FieldScriptableObject {
     ...
     * @param numberValid number of valid String values to use from array
     */
    public MFString(String[] strings, int numberValid) {
        this(); // invoke default constructor

        if(numberValid > 0) {
            stringList.ensureCapacity(numberValid);

            for(int i = 0; i < numberValid; i++)
                stringList.add(strings[i]);

            sizeInt.setValue(numberValid);
        } else {
            sizeInt.setValue(0);
        }
    }
...
//     * @param nodes The list of nodes to use
    ...
    public String toString() {
        ...
        for(int i = 0; i < size; i++) {
            ...
            if ((size > 1))
               buf.append("  "); // indent if multiple entries
            buf.append('\"');
            buf.append(str);
            buf.append("\"");
            if ((size > 1) && (i < size - 1)) // no newline after single entry or last entry
               buf.append("\n");
        }
