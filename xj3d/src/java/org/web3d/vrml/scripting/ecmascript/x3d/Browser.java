...
public class Browser extends AbstractScriptableObject {
    ...
    /**
     * Static initialization block to set up the basic ECMAScript definitions.
     */
    static {
    ...
    public Browser(VRMLExecutionSpace space,
        ...
        for(org.web3d.vrml.lang.ProfileInfo pi : p_list) {
            ProfileInfo p_info = new ProfileInfo(pi);
            String p_name = pi.getName();
        ...
        if(propertyNames.contains(name)) {
            ...
            switch(prop) {
                ...
                case 'c':
                    switch (name) {
                        case "currentSpeed":
                            ret_val = realBrowser.getCurrentSpeed();
                            break;
                        case "currentFrameRate":
                            ret_val = realBrowser.getCurrentFrameRate();
                            break;
                        default:
                            ret_val = ecmaScene;
                    }
     ...
     * @return the Scene created from the given nodes
     */
    ...
    protected FunctionObject locateFunction(String methodName) {
        ...
        try {
          function = new FunctionObject(methodName, method, this);
            ...
            org.j3d.util.DefaultErrorReporter.getDefaultReporter().errorReport("Unknown function: " + real_name +
