...
public class ListsRouter implements Router {
    ...
    private Map<EventOutHolder, List<RouteHolder>> sourceMap;
    ...
    public boolean processRoutes(double timestamp) {
        ...
        for(int i = 0; i < sourceCount; i++) {
            if(routeList[i].needsProcessing()) {
                List<RouteHolder> l = sourceMap.get(routeList[i]);
     ...
     */
    @Override
    public void addRoutes(List<ROUTE> routeSet) {
    ...
    public void updateRoutes() {
        ...
        if(routesToRemove.isEmpty() && routesToAdd.isEmpty())
            return;
        ...
        for(int i = 0; i < size; i++) {
            ...
            List<RouteHolder> l = sourceMap.get(tester);
