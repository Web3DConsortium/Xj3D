...
public class DefaultRouteManager implements RouteManager {
    ...
    private final Map<VRMLExecutionSpace, Router> spaceMap;
    ...
    private final HashSet<VRMLExecutionSpace> spacesToAdd;

    /** Set of all spaces waiting to be removed */
    private final HashSet<VRMLExecutionSpace> spacesToRemove;
    ...
    public void removeRoute(VRMLExecutionSpace space,
        ...
        Router router = spaceMap.get(space);

        if(router != null)
            router.removeRoute(srcNode, srcIndex, destNode, destIndex);
        else
            errorReporter.warningReport("DefaultRouteManager is attempting to remove route from a " +
    ...
    public void removeRoute(VRMLExecutionSpace space, ROUTE route) {
        ...
        if(router != null) {
            ...
            errorReporter.warningReport("DefaultRouteManager is attempting to remove route (object) " +
    ...
    public synchronized void addSpace(VRMLExecutionSpace space) {
        if(spaceMap.containsKey(space) || spacesToAdd.contains(space)) {
            errorReporter.warningReport("DefaultRouteManager is trying to add duplicate execution space=" + space + " (likely from duplicate Prototype or Inline), ignored.",
                                        null);
// TODO
// perhaps from different scenes that are separately using the same prototype?
// Each contained prototype ought to be separate and unique, not dependent on any other scenes...
// Probably the key for a prototype ought to be a unique hash/key?
// Probably the same uniqueness rationale needs to exist for duplicated Inline scenes as well
            return;
        }
    ...
    public synchronized void removeSpace(VRMLExecutionSpace space) {
        if(!spaceMap.containsKey(space)) {
            errorReporter.warningReport("DefaultRouteManager is trying to remove non-existent execution space, ignored.",
                                        null);
            return;
    ...
    private void delayedAddSpace(VRMLExecutionSpace space) {
        ...
        // TODO is this for one scene or for all collected scenes that are handled by the manager??
        List<ROUTE> route_list = scene.getRoutes();

        router.addRoutes(route_list);

        // Recursively add contained spaces

        // Handle Prototypes
        // TODO is this for one scene or for all collected scenes that are handled by the manager??
        List<VRMLNode> node_list = scene.getByPrimaryType(TypeConstants.ProtoInstance);
        size = node_list.size();
    ...
    private void delayedRemoveSpace(VRMLExecutionSpace space) {
        ...
        Router router = spaceMap.remove(space);
