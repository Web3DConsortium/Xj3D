...
import org.web3d.vrml.renderer.ogl.nodes.networking.OGLInline;
...
public class DefaultFrameStateManager
    ...
    private HashSet<FrameStateListener>[] registeredFrameStateListeners;
    ...
    private IntHashMap<NodeArray[]> removedNodes;

    /** Map of the basic removed index (true/false) listing */
    private IntHashMap<Boolean> removedIndex;

    /** Map of the basic added nodes listing */
    private IntHashMap<NodeArray[]> addedNodes;

    /** Map of the basic added index (true/false) listing */
    private IntHashMap<Boolean> addedIndex;
    ...
    public DefaultFrameStateManager() {
        ...
        traverser.setObserver(DefaultFrameStateManager.this);
    ...
    public void clear() {
        ...
        for(int i = 0; i < size; i++) {
            tmp = addedNodes.get(nodeTypeList[i]);
            tmp[0].clear();
            tmp[1].clear();

            tmp = removedNodes.get(nodeTypeList[i]);
            tmp[0].clear();
            tmp[1].clear();
    ...
    public synchronized void registerRemovedNode(VRMLNode node) {
        ...
        // NPS XMSF Bug fix 439
        // https://www.movesinstitute.org/bugzilla/show_bug.cgi?id=439
        // Idea queued up from AbstractLoadManager.queueSceneLoad
        if (node instanceof OGLInline) {

            OGLInline oglInline = (OGLInline) node;
            BasicScene scene = oglInline.getContainedScene();
            List<VRMLNode> node_list = scene.getBySecondaryType(TypeConstants.SingleExternalNodeType);

            // Same value as BaseInline.FIELD_NODE == 4.
            // False forces a registerRemoveScene call for orig. inline
            oglInline.setValue(4, false);

            for (VRMLNode vsent : node_list) {

                // Now remove any other embedded inlines
                oglInline = (OGLInline) vsent;
                oglInline.setValue(4, false);
            }
        }
    }
    ...
    public void clearRemovedNodes() {
        ...
        for(int i = 0; i < size; i++) {
            tmp = removedNodes.get(nodeTypeList[i]);
            val = removedIndex.get(nodeTypeList[i]);

            NodeArray clear_target = tmp[val ? 1 : 0];
    ...
    public void clearAddedNodes() {
        ...
        for(int i = 0; i < size; i++) {
            tmp = addedNodes.get(nodeTypeList[i]);
            val = addedIndex.get(nodeTypeList[i]);
    ...
    public NodeArray getRemovedNodes(int type) {
        ...
        Boolean idx = removedIndex.get(type);

        NodeArray[] tmp = removedNodes.get(type);
    ...
    public NodeArray getAddedNodes(int type) {
        Boolean idx = addedIndex.get(type);

        NodeArray[] tmp = addedNodes.get(type);
        if (!idx) {
    ...
    public void observedNode(VRMLNodeType parent,
        ...
        switch(child.getPrimaryType()) {
            ...
            default:
                ...
                if(addedNodes.containsKey(type)) {
                    if(addTraversal) {
                        Boolean val = addedIndex.get(type);
                        NodeArray[] tmp = addedNodes.get(type);

                        tmp[val ? 1 : 0].add(child);
                    } else {
                        Boolean val = removedIndex.get(type);
                        NodeArray[] tmp = removedNodes.get(type);
                        tmp[val ? 1 : 0].add(child);
                    }
                }
        ...
        for(int i = 0; i < secondary_types.length; i++) {
            switch(secondary_types[i]) {
				...
				case TypeConstants.OriginManagedNodeType:
                    if (addTraversal)
