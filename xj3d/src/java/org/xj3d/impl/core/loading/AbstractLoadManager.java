...
public abstract class AbstractLoadManager
    ...
    private Map<String[], LoadRequest> inProgress;
    ...
    public synchronized void queueExternProtoLoad(VRMLExternProtoDeclare proto) {
        ...
        LoadRequest lrq = inProgress.get(urls);

        if(lrq != null) {
            if(!lrq.loadDetailsList.contains(details))
                lrq.loadDetailsList.add(details);
        } else {
    ...
    public void queueSceneLoad(BasicScene scene) {
        ...
        for(VRMLNode obj : node_list) {

            // Scripts we ignore because they get punted to the script loader
            if(obj instanceof VRMLScriptNodeType || obj == null)
                continue;
        ...
        for(VRMLNode node : node_list) {
            
            if (node instanceof VRMLMultiExternalNodeType) {

                VRMLMultiExternalNodeType ext_node = (VRMLMultiExternalNodeType) node;
    ...
    public void stopSceneLoad(BasicScene scene) {
        ...
        for(int i = 0; i < size; i++) {
            ...
            if(inProgress.containsKey(urls)) {
                LoadRequest lrq = inProgress.get(urls);
                lrq.loadDetailsList.remove(details);
            } else
    ...
    /**
     * Request to fetch the cache used by the derived loadConstantsType.
     *
    ...
    /**
     * Return the given load loadConstantsType constant that represents the given external
 node.
     *
    ...
    private String findLoadConstant(VRMLNodeType node) {
        ...
        switch(node.getPrimaryType()) {
            ...
            case TypeConstants.TextureNodeType:
                // May be a movietexture, so check the secondary loadConstantsType for
                // audio capabilities
                int[] other = node.getSecondaryType();

                if(other != null) {
                    for(int i : other) {
                        if(i == TypeConstants.AudioClipNodeType)
