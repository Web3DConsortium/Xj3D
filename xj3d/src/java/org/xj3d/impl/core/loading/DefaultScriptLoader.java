...
public class DefaultScriptLoader implements ScriptLoader {
    ...
    /** Mapping of content loadConstantsType (key) to the supporting engine (value) */
    private IntHashMap<Map<String, ScriptEngine>> engineMap;
    ...
    public void registerScriptingEngine(ScriptEngine engine) {
        ...
        Map<String, ScriptEngine> spec_map = engineMap.get(spec_version);
    ...
    public void unregisterScriptingEngine(ScriptEngine engine) {
        ...
        for (String type : types) {
            Object suspect = spec_map.get(type);
            if (suspect == engine) {
                spec_map.remove(type);
            }
        }
    }
    ...
    public void loadScript(VRMLScriptNodeType script) {
        ...
        if(script.getLoadState() != VRMLExternalNodeType.NOT_LOADED)
            return;
