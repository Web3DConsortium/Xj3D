...
import static org.ietf.uri.URIUtils.createURI;
...
class ScriptLoadHandler extends BaseLoadHandler
    ...
    public void processLoadRequest(ErrorReporter reporter,
        ...
        for (LoadDetails loadList1 : loadList) {
            details = (ScriptLoadDetails) loadList1;
            VRMLScriptNodeType node = details.node;
        ...
        for(int i = 0; !content_found && (i < url.length); i++) {
            ...
            VRMLScriptNodeType s_node = details.node;
            ...
            Map<String, ScriptEngine> engine_map = details.engineMap.get(spec_version);

            if(cached_version != null) {
                mime_type = cached_version.getContentType();

                ScriptEngine engine =
                    engine_map.get(mime_type);
    ...
    private boolean loadExternal(ErrorReporter reporter,
                                 String fileUri,
                                 Vector<LoadDetails> loadList,
                                 Map<String, ScriptEngine> engineMap) {
        ...
        if(fileUri.endsWith(".class")) {
            ...
            ScriptEngine eng = engineMap.get(JAVA_MIME);
            ...
            ScriptEngine engine = engineMap.get(mime_type);

            for (LoadDetails loadList1 : loadList) {
                ScriptLoadDetails details = (ScriptLoadDetails) loadList1;
                VRMLScriptNodeType node = details.node;
                if(!node.checkValidContentType(mime_type))
                    continue;
                content_found = true;
        ...
        // for use.
        for (URL source_url : source_urls) {
            try {
                currentConnection = source_url.getResource();
            } catch (IOException ioe) {
                String msg = "Can't find resource " + source_url;
                reporter.warningReport(msg, ioe);
                continue;
            } catch (UnsupportedServiceException e) {
                String msg = "Can't run script: " + source_url;
    ...
    private boolean loadClass(ErrorReporter reporter,
        ...
        System.out.println("ScriptLoadHandler.loadClass");
        ...
        try {
            ...
            java.net.URL[] url_list;

            // try to load the parent class loader's
            // list so the user does not have to specify
            // as much from used projects
            if (this.getClass().getClassLoader() instanceof URLClassLoader) {

                URLClassLoader parent_loader = (URLClassLoader)this.getClass().getClassLoader();
                java.net.URL[] loader_list = parent_loader.getURLs();


                url_list = new java.net.URL[loader_list.length + 1];
                url_list[0] = new java.net.URL(url_str);
                System.arraycopy(loader_list, 0, url_list, 1, loader_list.length);

            } else {
                url_list = new java.net.URL[1];
                url_list[0] = new java.net.URL(url_str);
            }

// debug
//for (int i = 0; i < url_list.length; i++) {
//    System.out.println("url: " + url_list[i]);
//}
            ...
            try {
                ...
                // WebStart fallback
//System.out.println("WebStart fallback");

                ClassLoader cl = this.getClass().getClassLoader();
                script_class = cl.loadClass(class_name);
            }

//System.out.println("script class: " + script_class);
