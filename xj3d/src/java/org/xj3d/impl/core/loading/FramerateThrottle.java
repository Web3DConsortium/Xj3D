...
public class FramerateThrottle extends Thread
    ...
    private static final int FRAME_CYCLE_MINIMUM_STARTUP = 1_000;
    ...
    /** Time to wait between queries in milliseconds*/
    ...
    private static final boolean DISPLAY_LOAD_TIME = false;
    ...
    public FramerateThrottle(BrowserCore core, ErrorReporter reporter) {
        super(NAME);
        ResourceConnection.addGlobalProgressListener(FramerateThrottle.this);
        universe = core;
        ...
        universe.addCoreListener(FramerateThrottle.this);
        ...
        while (doRun) {
            ...
            if (mainFileLoading) {
                ...
                if (total == lastTotal) {
                    ...
                    if (lastTotalTimes > 2_000 / WAIT_TIME) {
                        if (!reportedError) {
                            reportedError = true;
                ...
                if (DISPLAY_LOAD_TIME && initialLoadDone) {
