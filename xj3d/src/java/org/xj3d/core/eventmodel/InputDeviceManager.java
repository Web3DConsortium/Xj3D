...
public class InputDeviceManager {
    ...
    private Map<String, List<VRMLDeviceSensorNodeType>> controllerMap;
    ...
    *  the UI systems event listeners */
    ...
    public void initialize() {
        if ( deviceFactory != null ) {
            ...
            List<InputDevice> newControllers = new ArrayList<>();
            List<InputDevice> newTrackers = new ArrayList<>();
            List<MidiDevice> newMidis = new ArrayList<>();
            InputDevice dev;
            ...
            for(int i = 0; i < len; i++) {
                ...
                for (InputDevice newDevice : newDevices) {
                    dev = newDevice;
    ...
    public void addX3DNode(VRMLDeviceSensorNodeType node) {
        ...
        List<VRMLDeviceSensorNodeType> list;
        int len;

        list = controllerMap.get(name);
        if(list == null) {
            list = new ArrayList<>();
            list.add(node);
    ...
    public void processTrackers(int layerId, boolean navigate,
        ...
        if (navigate) {
            ...
            for(int i = 0; i < len; i++) {
                ...
                List<VRMLDeviceSensorNodeType> list =
                    controllerMap.get(controllerDevices[i].getName());
                int x3dLen;
                ...
                if(list != null) {
                    ...
                    for(int j = 0; j < x3dLen; j++) {
                        VRMLDeviceSensorNodeType device = list.get(j);
