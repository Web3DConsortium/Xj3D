...
class LoadPriorityComparator implements Comparator<LoadRequest> {
    ...
    /**
     * Mapping of the sort loadConstantsType string to the Integer representing its
 priority.
     */
    ...
    public int compare(LoadRequest o1, LoadRequest o2) {
        ...
        Integer i1 = priorityMap.get(l1.loadConstantsType);
        Integer i2 = priorityMap.get(l2.loadConstantsType);
...
//       on compare to return the same this over time.  loadDetailsList.size changes

/*
        int s1 = l1.loadDetailsList.size();
        int s2 = l2.loadDetailsList.size();
        if(s1 != s2)
            return s1 < s2 ? s1 : s2;
*/
    ...
    private void setupPriorities() {
        String prop = (String)AccessController.doPrivileged((PrivilegedAction<Object>) () -> System.getProperty(LoadConstants.SORT_ORDER_PROP,
                LoadConstants.DEFAULT_SORT_ORDER) // privileged code goes here, for example:
        );
