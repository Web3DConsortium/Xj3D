 ...
 * of the largest component in that row or column.  Alternatively, individual
 * rows or columns can be set to a fixed percentage of the container or their
 * minimum size with various options. Components can also be set to fill the
    ...
    implements LayoutManager, java.io.Serializable
{
  ...
  /**
   * Resets the grid parameters.  All styles, alignments and sizes are reset
  ...
  /**
   * Sets the style of this grid.  This provides a shortcut to producing several
   * standard styles of grid layout by setting various row alignments and sizes.
   * The style can be tweaked by using the set(Row|Column)Alignment and set(Row|Column)Size
   * methods after a call to this one.<p>
   * The style parameter has the following parameters:
   * <ul>
   * <li>FILL to use all available space, with component heights and widths relative to
   * their preferred size.
   * <li>CENTER to use the same spacing as FILL but with each component being it's preferred
   * size and centered in the space.
   * <li>LABEL_FILL is a simple layout where the 1st, 3rd, etc. columns are considered to
   * be labels and the intervening ones are the data components (TextFields, ComboBoxes).
   * The labels are minimum size and right aligned and the others fill all available space.
   * <li>LABEL_MIN is the same as LABEL_FILL but the data components are minimum too and
   * aligned left.
   * </ul>
   ...
   * which may cause the layout to be smaller than the allocated space.  The horizontal
   * alignment of the grid in this space can then be specified using this method.
   ...
   * which may cause the layout to be smaller than the allocated space.  The vertical
   * alignment of the grid in this space can then be specified using this method.
   * The default is centered.
   * The available alignments are:
   * <ul>
  ...
  /**
   * Sets the alignment for a particular row.  If the alignment is not FILL, the
   * preferred size of the component will be used unless it is greater than the available
   * space, in which case it will be truncated.  The default alignment is FILL for all rows.
   * The available alignments are:
   * <ul>
  ...
  /**
   * Sets the alignment of multiple rows.  See setRowAlignment for details of alignment
   * values.
  ...
  /**
   * Sets the alignment for a particular column.  If the alignment is not FILL, the
   * preferred size of the component will be used unless it is greater than the available
   * space, in which case it will be truncated.  The default alignment is FILL for all
   * columns.
   * The available alignments are:
   * <ul>
   * <li>FILL - fill the entire vertical space available.
  ...
  /**
   * Sets the alignment of multiple columns.  See setColumnAlignment for details of alignment
   * values.
   ...
   * <ul>
   * <li>percentage - a value between 1 and 100 for the percentage of the width of the
   * grid that this row should take up.
   * <li>MINIMUM - make sure that the height of this row is always the minimum height of
   * of the largest component in the row no matter what it does to the formatting.  It can look
   * ugly if you resize the window too small but is good for labels which must always
   * display all their text.
   * <li>DYNAMIC - After the percentages and minimum components are removed from the grid
   * width the rows with dynamic sizes are allocated a proportion of the remaining space
   * based on the minimum height of the largest component in the row.  This is the default
   * size for all rows.
   * <li>BELOW_MINIMUM - the height is set to MINIMUM unless this minimum size would be
   * smaller than a dynamically allocated one, in which case DYNAMIC allocation is used.
   * This setting is useful if you want a component to stick to it's minimum size in most
   * situations but go even smaller if the panel size gets really small.
   * <li>ABOVE_MINIMUM - the height is allocated using the DYNAMIC setting unless it would
   * make the component smaller than it's minimum size, where MINIMUM is used.  This
   * option is useful if you want a component to take up any available space but never
   * go smaller than it's minimum size.
   * </ul>
     * @param value
   */
   ...
   * <ul>
   * <li>percentage - a value between 1 and 100 for the percentage of the width of the
   * grid that this column should take up.
   * <li>MINIMUM - make sure that the width of this column is always the minimum width of
   * the largest component in the column no matter what it does to the formatting.  It can
   * look ugly if you resize the window too small but is good for labels which must always
   * display all their text.
   * <li>DYNAMIC - After the percentages and minimum components are removed from the grid
   * width the columns with dynamic sizes are allocated a proportion of the remaining space
   * based on the minimum width of the largest component in the row.  This is the default
   * size for all column.
   * <li>BELOW_MINIMUM - the height is set to MINIMUM unless this minimum size would be
   * smaller than a dynamically allocated one, in which case DYNAMIC allocation is used.
   * This setting is useful if you want a component to stick to it's minimum size in most
   * situations but go even smaller if the panel size gets really small.
   * <li>ABOVE_MINIMUM - the width is allocated using the DYNAMIC setting unless it would
   * make the component smaller than it's minimum size, where MINIMUM is used.  This
   * option is useful if you want a component to take up any available space but never
   * go smaller than it's minimum size.
   * </ul>
  ...
  /**
   * Sets the size of multiple columns.  See setColumnSize for details of alignment
   * values.
   * @param start the inclusive start column to set
   * @param end the inclusive end column to set
   * @param value the size to set
  ...
  /**
   * Calculate the largest either minimum or preferred size for the components in
   * each row and column.
  ...
  private int spaceOut(int len,int[] perc,int[] value)
  {
    ...
    for(int i=0;i<size;i++)
    {
      ...
      if (perc[i]==DYNAMIC||perc[i]==ABOVE_MINIMUM||perc[i]==ABOVE_MARKER)
      {
  ...
  public void layoutContainer(Container parent)
  {
    ...
    for (int c = 0, x = insets.left+col_indent ; c < cols ; x += col_width[c++] + hgap)
    {
      for (int r = 0, y = insets.top+row_indent ; r < rows ; y += row_height[r++] + vgap)
      {
        int i = r * cols + c;
        if (i < ncomponents)
          placeComponent(parent.getComponent(i),x,y,col_width[c],row_height[r],row_align[r],col_align[c]);
      }
    }
  }
