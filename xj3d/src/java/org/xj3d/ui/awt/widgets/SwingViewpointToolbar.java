...
public class SwingViewpointToolbar extends JPanel
     ...
     */
    private JComboBox<Object> viewpoints;

    /**
     * The model used by the combo box to handle viewpoint data
     */
    private DefaultComboBoxModel<Object> viewpointModel;
     ...
     * valid. Each of these nested arrays contains a list of the Viewpoint nodes
     * in that layer.
     */
    ...
    public SwingViewpointToolbar(BrowserCore core,
        ...
        vpManager = vpMgr;
        browserCore = core;
        core.addNavigationStateListener(SwingViewpointToolbar.this);
        core.addViewpointStatusListener(SwingViewpointToolbar.this);
        ...
        viewpoints.addItemListener(SwingViewpointToolbar.this);
...
//        int HORIZ_POS = SwingConstants.CENTER;
     ...
     *
     * @param idx The new state expressed as an index into the current navModes
     * list.
     */
    @Override
     ...
     * This layer is currently empty of viewpoints. Calls to
    ...
    /**
     * Notification that a Layer ID is no longer valid. Any viewpoints that have
     * been made available for that layer should now be removed from the layer.
     *
     * @param layerId The ID of the layer to be added
     */
    @Override
    ...
    public void viewpointLayerActive(int layerId) {
        ...
        if (layerId == -1) {
            ...
            if ((l == null) || l.isEmpty()) {
                ...
                for (VRMLViewpointNodeType l1 : l) {
                    Object vp = l1;
                    if (vp != def_vp) {
...
//System.out.println( "removing default: "+ def_vp );
     ...
     * disabled state. This is used when we only have the default viewpoint, but
     * still want to allow the home, fit and lookat capabilities.
     *
     * @param enabled true if this component is enabled
     */
