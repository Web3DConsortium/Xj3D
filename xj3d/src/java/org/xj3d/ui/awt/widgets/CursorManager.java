...
public class CursorManager
    ...
    /** Property to define fly mode cursor */
    private static final String FLY_LEVEL_CURSOR_PROPERTY = "FLY.cursor";
    ...
    /** Default image name for fly mode cursor */
    private static final String DEFAULT_FLY_LEVEL_CURSOR_VALUE =
        "images/navigation/CursorFly.gif";
    ...
    private Map<String, Cursor> navCursors;
    ...
    private Map<Cursor, String> cursorUrlMap;

    /** A mapping of urls to cursors */
    private Map<String, Cursor> urlCursorMap;
    ...
    /**
     * Create a new instance of the cursor manager with a customized set
     * of images.
     *
     * @param canvas The canvas to manage cursors for
...
//if (c != null) System.out.println("Cached cursor: " + c.hashCode() + " url: " + url);
        if (c == null) {
...
//System.out.println("Final cursor: " + c.hashCode());
     ...
     * @param params The list of paramters for the urls
     * @param desc The description string that accompanied the link
     */
    @Override
    ...
    public void navigationStateChanged(int idx) {
        ...
        Cursor cursor = navCursors.get(navModes[idx]);
    ...
    public void keyPressed(KeyEvent evt) {
        ...
        case KeyEvent.VK_ALT:
            altModifier = true;
            if (trackNavMode) {
                navStateCursor = navCursors.get(Xj3DConstants.TRACK_PAN_NAV_MODE);
        ...
        case KeyEvent.VK_CONTROL:
            ctrlModifier = true;
            if (trackNavMode) {
                navStateCursor = navCursors.get(Xj3DConstants.FLY_NAV_MODE);
                boolean sensorIsActive = !sensorStack.isEmpty();
                if (!sensorIsActive || (sensorIsActive && shiftModifier)) {
                    setCursor(navStateCursor);
                }
    ...
    public void keyReleased(KeyEvent evt) {
        ...
        case KeyEvent.VK_ALT:
            altModifier = false;
            if(trackNavMode) {
                navStateCursor = navCursors.get(Xj3DConstants.TRACK_EXAMINE_NAV_MODE);
                boolean sensorIsActive = !sensorStack.isEmpty();
                if (!sensorIsActive || (sensorIsActive && shiftModifier)) {
                    setCursor(navStateCursor);
                }
    ...
    private void loadCursors(Properties cursorProperties) {
        ...
        String fly_level = cursorProperties.getProperty(
            FLY_LEVEL_CURSOR_PROPERTY,
            DEFAULT_FLY_LEVEL_CURSOR_VALUE
            );
        ...
        Image img13 = IconLoader.loadImage(fly_level, errorReporter);
        ...
        if(img13 != null) {
            center.x = img13.getWidth(null) / 2;
            center.y = img13.getHeight(null) / 2;

            Cursor c = tk.createCustomCursor(img13, center ,null);
            navCursors.put(Xj3DConstants.FLY_LEVEL_NAV_MODE, c);

            cursorUrlMap.put(c, fly_level);
            urlCursorMap.put(fly_level, c);
        } else {
            errorReporter.warningReport("Unable to load FLY LEVEL cursor image: " + fly_level, null);
            navCursors.put(Xj3DConstants.FLY_LEVEL_NAV_MODE,
                Cursor.getPredefinedCursor( Cursor.CROSSHAIR_CURSOR ));
        }
    ...
    private void setCursor(Cursor cursor) {
        ...
        if (c == null) {
            if (newUrl.startsWith("PREDEFINED.")) {
                ...
                Image img = IconLoader.loadImage(newUrl, errorReporter);
