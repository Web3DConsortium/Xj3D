/*****************************************************************************
 *                        Web3d.org Copyright (c) 2001 - 2026 - 2008
 *                               Java Source
 *
 ...
 * DEF name is programmatically configurable in this module.</li>
 ...
 * <li>The image capture is initiated immediately if the DEF'd Viewpoint is not found,
 ...
 * DEF'd Viewpoint has been bound.</li>
 * </ul>
 *
 * @author Rex Melton
    ...
    *  named viewpoint to be bound */
    ...
    public void screenCaptured( Buffer buffer, int width, int height ) {
        ...
        if ( listener != null ) {
            listener.recorderStatusChanged(
                new RecorderEvent(this, RecorderEvent.COMPLETE, number) 
            );
        }
    }
    ...
    public void allEventsComplete( ) {
        if ( addNodes ) {
            ...
            if (navInfo != null) // not created if Core profile
            {
    ...
    public boolean setOutputFile( File file ) {
        ...
        System.out.println ("ThumbnailRecorder setOutputFile " + filename);
        int extensionIndex = filename.lastIndexOf( "." );
        if ( extensionIndex == -1 ) {
            errorReporter.errorReport(
                LOG_NAME +": Invalid output file: Unknown image "+
                "encoding type for file name: "+ filename, null );
        } else if ( setEncoding( filename.substring( extensionIndex+1 ) ) ) {
            outputFile = file;
        }
    ...
    public void start( RecorderListener listener ) {
        ...
        if (replaceNavInfo) {
            ...
            if (node != null) {
                ...
                // avoid throwing exception if Core profile
//              String profileName=scene.getMetaData().getProfileName(); // original method
                String profileName=scene.getProfileName();               // utility method
                if (!profileName.equalsIgnoreCase("Core"))
                {
...
//                  String profileName=scene.getMetaData().getProfileName(); // original method
    ...
    public void saveScreen( Buffer buffer, int width, int height ) {
        ...
        try {
            ...
            System.out.println ("ThumbnailRecorder saveScreen " + outputFile);
    ...
    private void initiateCapture() {
        ...
        if ( listener != null ) {
            listener.recorderStatusChanged(
                new RecorderEvent(this, RecorderEvent.ACTIVE, number)
            );
        }
    }
}
