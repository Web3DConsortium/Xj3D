...
class Xj3DModel implements AVModel {
    ...
    private Map<String, SceneGraphObject> namedObjects;

    /** Mapping of objects to their externally named resources */
    private Map<SceneGraphObject, Object> externalObjects;
    ...
    private List<AVRuntimeComponent> runtimes;
    ...
    void setValues(boolean keepScene) {
        ...
        Map<String, VRMLNode> vrml_map = realScene.getDEFNodes();

        Set<String> key_set = vrml_map.keySet();
        ...
        for(i = 0; i < size; i++) {
            ...
            switch(p_type) {
                case TypeConstants.BackgroundNodeType:
                    backgrounds.add((Background)node.getSceneGraphObject());
                    break;

                case TypeConstants.FogNodeType:
                    fogs.add((Fog)node.getSceneGraphObject());
                    break;

                case TypeConstants.ViewpointNodeType:
                    OGLViewpointNodeType jvn = (OGLViewpointNodeType)node;
                    viewpoints.add((Viewpoint)jvn);
                    break;
            }
        }
        ...
        ((ArrayList<Light>) lights).ensureCapacity(size);

        for(i = 0; i < size; i++) {
            node = (OGLVRMLNode)list.get(i);
            lights.add((Light)node.getSceneGraphObject());
        }
        ...
        for(i = 0; i < size; i++) {
            node = (OGLVRMLNode)list.get(i);
            layers.add((Layer)node.getSceneGraphObject());
        }
