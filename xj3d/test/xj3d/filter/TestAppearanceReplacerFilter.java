...
/**
 * Test AppearanceReplacer Filter
 *
 * @author Rex Melton
 * @version $Revision: 1.0 $
 */
public class TestAppearanceReplacerFilter extends BaseTestFilter {

    /**
     * The filter argument name
     */
    public static final String FILTER_NAME = "AppearanceReplacer";

    /**
     * The directory containing the files to pass through the filter
     */
    public static final String PARSETEST = "parsetest/filter/" + FILTER_NAME + "/";
    ...
    public static Test suite() {
        return new TestSuite(TestAppearanceReplacerFilter.class);
    }

    /**
     * AppearanceReplacer Test Case.
	 * <p>
	 * This does nothing more than pass the test file set through the filter.
	 * Whether the filter does what it's supposed to other than 'not crash',
	 * is not determined.
     */
    public void testAppearanceReplacer() {

        List<String> filters = new ArrayList<>();
        filters.add(FILTER_NAME);

        List<String> args = new ArrayList<>();
        args.add("-appearanceFile");
        args.add(URLChecker.prependFileScheme(new File(PARSETEST + "appearance.x3dv").getAbsolutePath())); // <- what??
