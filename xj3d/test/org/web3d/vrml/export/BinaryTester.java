...
/** Not sure what this was meant to test. A X3D file size of 668K causes an out
 * of memory error unless -Xmx8g of heap size is allocated. Even then, an EOF
 * exception is thrown. (tdn) 10/04/24
 */
public class BinaryTester extends TestCase {

    private static final int STATE_NODE = 0;
    private static final int STATE_FIELDS = 1;
    private final int state = STATE_NODE;
    ...
    private Map<Integer, String>[] nodeFieldDict;
    ...
    public static Test suite() {
        return new TestSuite(BinaryTester.class);
    }
    ...
    public static void main(String args[]) {
        ...
        if (args.length < 1) {
            ...
            args = new String[] {"examples/dis/HelloXj3D.x3dv"};
        }
    ...
    public void parseFile(InputStream dis) throws IOException {
        floatArray = new float[4];
        stringArray = new String[1];
        intArray = new int[1];
        currentNode = new SimpleStack();
        initCompressors();
        this.dis = (DataInputStream) dis;
        readHeader();
...
System.out.println("Fields: " + dictSize);
        for(int i=0; i < dictSize; i++) {
            ...
            for(int j=0; j < size; j++) {
                ...
                nodeFieldDict[i+1].put(j,fieldName);
        ...
        while(true) {
            ...
            switch(read) {
                case BinaryExporter.OP_NODE:
                    ...
                    break;
                default:
                    System.err.println("Unknown OP code: " + read);
            }
        }
   }
...
System.out.println("rn eof");
    ...
    public void readFields(VRMLNode node) throws IOException {
        int nodeId = ((Integer)currentNode.peek());
...
System.out.println("EOFields");
        ...
        String name = nodeFieldDict[nodeId].get(fieldNum-1);
