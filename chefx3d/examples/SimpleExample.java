...
public class SimpleExample extends BaseExample {
    ...
    public SimpleExample() {
        super(WorldModelFactory.NETWORK_NONE, "", "", "", "");
    }
    ...
    protected void createTools() {
        ...
        // Solve URL issues
        URL url = loader.getResource(cpath + "common/x3d_default_world.xslt");
        File file = null;
        try {
            file = new File(url.toURI());
        } catch (URISyntaxException ex) {
            errorReporter.errorReport(ex.getMessage(), ex);
        }

        styles.put("x3d", file.getPath());

        String[] interfaceIcons = new String[] {ipath + "Grid16x16.png",
                ipath + "Grid32x32.png", ipath + "Grid64x64.png"};

        // Solve URL issues
        url = loader.getResource(cpath + "Locations/Grid/Grid.x3dv");

        tool = new Tool("Grid", ipath + "Grid.png", interfaceIcons, false, Tool.TYPE_WORLD,
                new String[] { url.toString() }, Classification.FREE, "Grid",
        ...
        // Solve URL issues
        url = loader.getResource(cpath + "Primitives/Box/Box.xslt");
        try {
            file = new File(url.toURI());
        } catch (URISyntaxException ex) {
            errorReporter.errorReport(ex.getMessage(), ex);
        ...
        styles.put("x3d", file.getPath());
        styles.put("x3d_view", file.getPath());
        toolParams = null;

        interfaceIcons = new String[] {ipath + "Box16x16.png", ipath + "Box32x32.png",
                ipath + "/Box64x64.png"};

        url = loader.getResource(ipath + "Box.png");
        try {
            file = new File(url.toURI());
        } catch (URISyntaxException ex) {
            errorReporter.errorReport(ex.getMessage(), ex);
        ...
        url = loader.getResource(cpath + "Primitives/Box/Box.x3d");

        tool = new Tool("Box", file.getPath(), interfaceIcons, false, Tool.TYPE_MODEL,
                new String[] { url.toString() }, Classification.FREE, "Box",
    ...
    private ToolGroup createWaypointTools() {
        ToolGroup root;
        List<ToolGroupChild> tools = new ArrayList<ToolGroupChild>();
        List<ToolGroupChild> chapters;
        Document properties;
        Map<String, String> styles = new HashMap<String, String>();
        Tool tool;
