...
public class RemoveVertexCommand implements Command {
     ...
     */
    public RemoveVertexCommand(WorldModel model, int entityID, int vertexID) {
        ...
        if (model.getEntity(entityID) instanceof SegmentableEntity) {
            vertex = ((SegmentableEntity)model.getEntity(entityID)).getSelectedVertex();
        }
    ...
    @Override
    public void undo() {
        model.addSegmentVertex(local, entityID, vertexID,
