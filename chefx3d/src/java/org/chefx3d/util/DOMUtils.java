...
public class DOMUtils {
    ...
    private static final String IDENTITY_TRANSFORM = "<xsl:stylesheet version = '2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'><xsl:output method='xml' omit-xml-declaration='yes' indent='yes'/><xsl:template match='node()|@*'> <xsl:copy>   <xsl:apply-templates select='@*'/>   <xsl:apply-templates/>   </xsl:copy> </xsl:template></xsl:stylesheet>";
     ...
     * @return The element or null
     */
    public static Element getSingleElement(String name, Document doc, boolean issueError, String errorMessage, ErrorReporter errorReporter) {
