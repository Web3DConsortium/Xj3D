 ...
 */
public class GTPanZoomControl implements MouseListener, MouseMotionListener, ImageObserver {
    ...
    *  over the zoom knob. */
    ...
    /** Default Constructor */
    public GTPanZoomControl( Component parent ) {
        this( parent, 0, 0 );
    }

    /** Constructor */
    public GTPanZoomControl( Component parent, Point p ) {
        this( parent, p.x, p.y );
    }

    /** Constructor */
    public GTPanZoomControl( Component parent, int offsetX, int offsetY ) {
        ...
        image = toolkit.createImage(
            ClassLoader.getSystemResource( DEFAULT_CONTROL_IMAGE ) );

        zoom_knob_image = toolkit.createImage(
            ClassLoader.getSystemResource( DEFAULT_ZOOM_KNOB_IMAGE ) );

        handCursor = Cursor.getPredefinedCursor( Cursor.HAND_CURSOR );

        Image img = toolkit.createImage(
            ClassLoader.getSystemResource( DEFAULT_OPEN_HAND_CURSOR_IMAGE ) );
        openHandCursor = toolkit.createCustomCursor( img, new Point( 6, 4 ), null );

        img = toolkit.createImage(
            ClassLoader.getSystemResource( DEFAULT_CLOSED_HAND_CURSOR_IMAGE ) );
     ...
     * @return The function identifier corresponding to the selected
