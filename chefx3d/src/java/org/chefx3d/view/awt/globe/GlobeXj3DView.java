...
public class GlobeXj3DView extends JPanel implements ViewX3D, ViewConfig, Runnable, BrowserListener,
    WorldLoaderListener, X3DFieldEventListener, ModelListener, EntityChangeListener, ActionListener,
    Xj3DCursorFilter {

    private static enum MouseMode {NONE, NAVIGATION, PLACEMENT, SELECTION, ASSOCIATE};

    private static double[] GEO_ORIGIN = new double[] { 0.0, 0.0, -6_378_137};
    ...
    private Map<X3DField, Integer> tsMap;

    /** Map between entityID and the enabled field */
    private Map<Integer, X3DField> tsEnabledMap;
    ...
    *  world loads to manage initialization of scene dependent objects. */
    ...
    public GlobeXj3DView(WorldModel model, String initialWorld, String globeURL) {
        ...
        model.addModelListener(GlobeXj3DView.this);
        this.model = model;
...
//        ViewManager.getViewManager(model).addView(this);
        ViewManager.getViewManager().setWorldModel(model);
        ViewManager.getViewManager().addView(GlobeXj3DView.this);
        // TDN
...
System.out.println("***SetTool: " + currentTool + " icon: " + tool.getIcon() + " mode: " + currentMode);
    ...
    private void init(Map<String, Object> params) {
        ...
        Image image = tk.createImage(ClassLoader.getSystemResource("images/2d/selectIcon.png"));
        ...
        image = tk.createImage(ClassLoader.getSystemResource("images/2d/panIcon.png"));
        ...
        image = tk.createImage(ClassLoader.getSystemResource("images/2d/openHandIcon.png"));
        ...
        image = tk.createImage(ClassLoader.getSystemResource("images/2d/boundIcon.png"));
        ...
        tsMap = new HashMap<>(100);
        tsEnabledMap = new HashMap<>(100);

        exporter = new X3DExporter("3.3", "Immersive", new String[] {"Geospatial"}, new int[] {1});
        ViewManager.getViewManager().setWorldModel(model);
...
System.out.println("Placing entity at: " + lastGeoPos[0] + " " + lastGeoPos[1] + " " + lastGeoPos[2]);
        ...
        if (mainScene != null) {
            ...
            tsMap.put(tisActive, entityID);
            tsEnabledMap.put(entityID, ts.getField("enabled"));
...
//                ViewManager.getViewManager(model).disableAssociateMode();
            ...
            Integer entityID = tsMap.get((Object)src);
            if (entityID != null) {
                ...
                entityDragging = true;
                currentEntity = model.getEntity(entityID);
            }
        }
...
System.out.println("Done X3D");
        ...
        try {
            ...
            System.err.println("Error parsing x3d: " + x3d);
            e.printStackTrace();
        }
